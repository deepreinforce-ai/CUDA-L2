...
__global__ void __launch_bounds__(128, 3)
    hgemm_extreme_saturation_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_buf1 = make_fragment_like(tCrA_buf0);
  auto tCrB_buf1 = make_fragment_like(tCrB_buf0);
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      int buf_idx_compute = ik & 1;
      int buf_idx_prefetch = 1 - buf_idx_compute;
  ...
  auto tCrD_half = make_fragment_like<T>(tCrD);
...
template <typename T>
void launch_hgemm_extreme_saturation(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  constexpr int kShmSize = cute::max(shm_size_AB, shm_size_C) * sizeof(T);

  cudaFuncSetAttribute(
      hgemm_extreme_saturation_kernel<T, BM, BN, BK, kStage, MMA, G2SCopyA,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_extreme_saturation<half>(a_ptr, b_col_ptr, c_ptr, M, N, K);
}