...
static cutlass::Status try_run_gemm(
    int M, int N, int K,
    ...
    const cutlass::KernelHardwareInfo& hw_info)
{
  ...
  return status;
#else
  return cutlass::Status::kErrorNotImplemented;
#endif
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  static cutlass::KernelHardwareInfo hw_info = []() {
    ...
    info.device_id = cached_device_id;
    info.sm_count = cutlass::KernelHardwareInfo::query_device_multiprocessor_count(cached_device_id);
    return info;
  }();
  ...
  cutlass::Status status = try_run_gemm<HgemmPrimary>(M, N, K, ptr_A, ptr_B, ptr_C, ptr_D, hw_info);

  if (status != cutlass::Status::kSuccess) {
    status = try_run_gemm<HgemmFallback>(M, N, K, ptr_A, ptr_B, ptr_C, ptr_D, hw_info);
    if (status != cutlass::Status::kSuccess) {
      throw std::runtime_error("CUTLASS GEMM failed on both primary and fallback configurations");
    }
  }
