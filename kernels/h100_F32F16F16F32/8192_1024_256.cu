...
#pragma unroll
  ...
  const int total_elements = size(tCrD);
  const int vec_iters = total_elements / VEC_SIZE;
...
#pragma unroll
    for (int j = 0; j < VEC_SIZE; ++j) {
      tCrD_half(base + j) = static_cast<T>(tCrD(base + j));
    }
  }

  const int remainder_start = vec_iters * VEC_SIZE;
#pragma unroll
  for (int i = remainder_start; i < total_elements; ++i) {
