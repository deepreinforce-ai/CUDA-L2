...
void launch_hgemm_optimized_v2(T *a, T *b, T *c, int M, int N, int K) {
  ...
  constexpr bool use_block_swizzle = true;
  
  int BX = (N + BN - 1) / BN;
  int BY = (M + BM - 1) / BM;
  int BZ = use_block_swizzle ? (N + swizzle_stride - 1) / swizzle_stride : 1;
  BX = use_block_swizzle ? (BX + BZ - 1) / BZ : BX;
  ...
  cudaFuncSetAttribute(
      hgemm_kernel_optimized_v2<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
