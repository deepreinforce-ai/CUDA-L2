...
hgemm_optimized_v7_kernel(const T * __restrict__ Aptr,
  ...
  cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, 0), tCrA_view(_, _, 0));
  cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, 0), tCrB_view(_, _, 0));

  cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, 1), tCrA_view(_, _, 1));
  cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, 1), tCrB_view(_, _, 1));

  cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, 2), tCrA_view(_, _, 2));
  cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, 2), tCrB_view(_, _, 2));
  cute::gemm(tiled_mma, tCrD, tCrA(_, _, 0), tCrB(_, _, 0), tCrD);

  cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, 3), tCrA_view(_, _, 3));
  cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, 3), tCrB_view(_, _, 3));
...
template <typename T>
void launch_hgemm_optimized_v7(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static constexpr int shm_size_A = cute::cosize(SmemLayoutA{}) * sizeof(T);
  static constexpr int shm_size_B = cute::cosize(SmemLayoutB{}) * sizeof(T);
  static constexpr int kShmSize = shm_size_A + shm_size_B;

  cudaFuncSetAttribute(
      hgemm_optimized_v7_kernel<T, 128, 128, 64, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_v7<half>(
