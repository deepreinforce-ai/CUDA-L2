...
__global__ void __launch_bounds__(128, 2) cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr,
                                                  T *Dptr, int m,
                                                  int n, int k) {
...
#pragma unroll
  ...
  const int vec4_iters = total_elems / 4;
  const int remainder = total_elems % 4;

#pragma unroll
  for (int i = 0; i < vec4_iters; ++i) {
    const int base_idx = i * 4;
    float f0 = tCrD(base_idx + 0);
    float f1 = tCrD(base_idx + 1);
    float f2 = tCrD(base_idx + 2);
    float f3 = tCrD(base_idx + 3);
    
    tCrD_half(base_idx + 0) = static_cast<T>(f0);
    tCrD_half(base_idx + 1) = static_cast<T>(f1);
    tCrD_half(base_idx + 2) = static_cast<T>(f2);
    tCrD_half(base_idx + 3) = static_cast<T>(f3);
  }

#pragma unroll
  for (int i = vec4_iters * 4; i < total_elems; ++i) {
  ...
  const int num_batches = size<1>(tCrC_r2sx);

#pragma unroll
  for (int i = 0; i < num_batches; i += step) {
#pragma unroll
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory exceeds A's single stage");
