...
hgemm_optimized_epilogue_kernel(const T * __restrict__ Aptr,
                                ...
                                int m, int n, int k)
{
    ...
    auto tCrA = thr_mma.partition_fragment_A(gA(_, _));
    auto tCrB = thr_mma.partition_fragment_B(gB(_, _));
    ...
    int nk = size<2>(tCrA_view);

    cute::copy(s2r_copy_a, tAsA_s2r(_, _, 0), tCrA_view(_, _, 0));
    cute::copy(s2r_copy_b, tBsB_s2r(_, _, 0), tCrB_view(_, _, 0));

#pragma unroll
    for (int ik = 0; ik < nk - 1; ++ik) {
        cute::copy(s2r_copy_a, tAsA_s2r(_, _, ik + 1), tCrA_view(_, _, ik + 1));
        cute::copy(s2r_copy_b, tBsB_s2r(_, _, ik + 1), tCrB_view(_, _, ik + 1));
        cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik), tCrB(_, _, ik), tCrD);
    }

    cute::gemm(tiled_mma, tCrD, tCrA(_, _, nk - 1), tCrB(_, _, nk - 1), tCrD);
...
void launch_optimized_hgemm(T *a, T *b_col_major, T *c, int M, int N, int K) {
    ...
    static constexpr int shm_size = (shm_AB > shm_C ? shm_AB : shm_C) * sizeof(T);

    cudaFuncSetAttribute(
        hgemm_optimized_epilogue_kernel<T, BM, BN, BK, MMA,
                                        G2SCopyA, G2SCopyB,
    ...
    hgemm_optimized_epilogue_kernel<T, BM, BN, BK, MMA,
                                    ...
                                    S2RCopyAtomA, S2RCopyAtomB,
                                    R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>
        <<<grid, block, shm_size>>>(a, b_col_major, c, M, N, K);
}
