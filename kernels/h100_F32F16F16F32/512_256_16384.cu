                           ...
                           Alpha alpha, Beta beta)
{
  ...
  int prefetch_depth = cute::min(int(K_PIPE_MAX), total_ktiles);
  ...
  int ktiles_processed = 0;

  CUTE_NO_UNROLL
  while (ktiles_processed < total_ktiles)
  {
    ...
    if ((warp_idx == 0) && lane_predicate && k_tile < total_ktiles)
    {
