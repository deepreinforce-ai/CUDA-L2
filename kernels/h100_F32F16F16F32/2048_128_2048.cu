...
hgemm_tuned_optimized_kernel(T *__restrict__ Aptr,
                              T *__restrict__ Bptr,
                              T *__restrict__ Dptr,
                              int m, int n, int k) {
...
void launch_tuned_optimized(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_tuned_optimized<half, 7>(a_ptr, b_col_ptr, c_ptr, M, N, K);
}