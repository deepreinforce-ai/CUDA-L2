...
hgemm_optimized_prefetch_kernel(T *__restrict__ Aptr, T *__restrict__ Bptr,
...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  ...
  int BX = (N + int(BN) - 1) / int(BN);
  int BY = (M + int(BM) - 1) / int(BM);
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_prefetch_kernel<T, int(BM), int(BN), int(BK), Stages,
                                    MMA, G2SCopyA, G2SCopyB, SmemLayoutA,
                                    SmemLayoutB, SmemLayoutC, S2RCopyAtomA,
