...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename HgemmUltraFineGrouped::Gemm;
  using StrideA = typename HgemmUltraFineGrouped::StrideA;
  using StrideB = typename HgemmUltraFineGrouped::StrideB;
  using StrideC = typename HgemmUltraFineGrouped::StrideC;
  using StrideD = typename HgemmUltraFineGrouped::StrideD;
  using ElementA = typename HgemmUltraFineGrouped::ElementA;
  using ElementB = typename HgemmUltraFineGrouped::ElementB;
  using ElementC = typename HgemmUltraFineGrouped::ElementC;
  ...
  if (cuda_status != cudaSuccess) {
    std::string err_msg = "CUDA device synchronization error: ";
    err_msg += cudaGetErrorString(cuda_status);
    throw std::runtime_error(err_msg);
  }
#else
  throw std::runtime_error("CUTLASS SM90 MMA operations not supported on this architecture");
#endif
}