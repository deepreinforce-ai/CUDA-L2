...
#pragma unroll
  ...
  constexpr int num_regs = decltype(size(tCrD))::value;

#pragma unroll
  for (int i = 0; i < num_regs / 2; ++i) {
    half2 h2 = __floats2half2_rn(tCrD(2 * i), tCrD(2 * i + 1));
    reinterpret_cast<half2 &>(tCrD_half(2 * i)) = h2;
  }
  if constexpr (num_regs % 2 == 1) {
    tCrD_half(num_regs - 1) = __float2half(tCrD(num_regs - 1));
  }
...
void launch_hgemm_bk128(T *a, T *b_col_major, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void launch_hgemm_bn128(T *a, T *b_col_major, T *c, int M, int N, int K) {
  using namespace cute;
