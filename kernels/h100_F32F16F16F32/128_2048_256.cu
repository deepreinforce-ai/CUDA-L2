...
hgemm_optimized_bk32_kernel(const T * __restrict__ Aptr, 
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik == 0 && itile_to_read < ntile) {
        const int next_stage = ismem_write % kStage;
        cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, itile_to_read),
      ...
      if (ik < nk - 1) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 1, ismem_read),
                   tCrA_view(_, _, ik + 1));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 1, ismem_read),
                   tCrB_view(_, _, ik + 1));
      }
...
void launch_hgemm_bk32_4stage(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                "C smem size exceeds A pipe stage size");
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_bk32_kernel<T, BM, BN, BK, kStage, MMA,
                                  G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_bk32_4stage<half, 4, true>(
