...
template <typename T, int BM, int BN, int BK, int kStage, int SPLIT_K, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void hgemm_split_k_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  int split_k_id = blockIdx.z % SPLIT_K;
  int k_start = (k / SPLIT_K) * split_k_id;
  int k_end = (split_k_id == SPLIT_K - 1) ? k : k_start + (k / SPLIT_K);
  ...
  Tensor B = make_tensor(make_gmem_ptr(Bptr), make_shape(n, k),
                         make_stride(k, Int<1>{}));
  Tensor D = make_tensor(make_gmem_ptr(Dptr), make_shape(m, n, Int<SPLIT_K>{}),
...
#pragma unroll
  for (int istage = 0; istage < kStage - 1 && istage < ntile; ++istage) {
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, k_tile_offset + istage),
               tAsA_copy(_, _, _, istage));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, k_tile_offset + istage),
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik == 0) {
        if (itile_to_read < ntile) {
          cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, k_tile_offset + itile_to_read),
                     tAsA_copy(_, _, _, ismem_write));
          cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, k_tile_offset + itile_to_read),
...
__global__ void __launch_bounds__(512) reduce_split_k_vectorized_kernel(
    T *partial_C, T *C, int m, int n) {
  int total = m * n;
  ...
  if (idx + 1 < total) {
    ...
    for (int k = 0; k < SPLIT_K; ++k) {
      half2 vals = *reinterpret_cast<half2*>(&partial_C[idx + k * total]);
      sum0 += __half2float(vals.x);
      sum1 += __half2float(vals.y);
    }
    
    half2 result;
    result.x = __float2half(sum0);
    result.y = __float2half(sum1);
    *reinterpret_cast<half2*>(&C[idx]) = result;
    
  } else if (idx < total) {
    float sum = 0.0f;
    #pragma unroll
    for (int k = 0; k < SPLIT_K; ++k) {
      sum += __half2float(partial_C[idx + k * total]);
    }
    C[idx] = __float2half(sum);
  }
}

template <typename T, const int Stages = 8, const int SPLIT_K = 4>
void launch_hgemm_split_k_cute(T *a, T *b, T *c, T *partial_c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_split_k_kernel<
          T, BM, BN, BK, KStage, SPLIT_K, MMA, G2SCopyA, G2SCopyB, SmemLayoutA,
          SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC,
          S2GCopyAtomC, S2GCopyC>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);

  hgemm_split_k_kernel<
      T, BM, BN, BK, KStage, SPLIT_K, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB,
      SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC,
      S2GCopyC><<<grid, block, shm_size>>>(a, b, partial_c, M, N, K);

  int total_elements = M * N;
  int reduce_threads = 512;
  int reduce_blocks = ((total_elements + 1) / 2 + reduce_threads - 1) / reduce_threads;
  reduce_split_k_vectorized_kernel<T, SPLIT_K><<<reduce_blocks, reduce_threads>>>(
      partial_c, c, M, N);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  const int SPLIT_K = 4;
  auto partial_c = torch::empty({M, N, SPLIT_K}, torch::TensorOptions().dtype(torch::kHalf).device(a.device()));
  
  launch_hgemm_split_k_cute<half, 8, SPLIT_K>(
      reinterpret_cast<half *>(a.data_ptr()),
      reinterpret_cast<half *>(b_col_major.data_ptr()),
      reinterpret_cast<half *>(c.data_ptr()),
      reinterpret_cast<half *>(partial_c.data_ptr()),
      M, N, K);
}