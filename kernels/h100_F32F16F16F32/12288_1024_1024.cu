...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  CHECK_TORCH_TENSOR_SHAPE(b_col_major, N, K)
  CHECK_TORCH_TENSOR_SHAPE(c, M, N)

#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = StandardH100GemmConfig::Gemm;
  using StrideA = StandardH100GemmConfig::StrideA;
  using StrideB = StandardH100GemmConfig::StrideB;
  using StrideC = StandardH100GemmConfig::StrideC;
  using StrideD = StandardH100GemmConfig::StrideD;
  using ElementA = StandardH100GemmConfig::ElementA;
  using ElementB = StandardH100GemmConfig::ElementB;
  using ElementC = StandardH100GemmConfig::ElementC;
...
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported - H100 or newer GPU required");
#endif
}