...
struct VerifiedFP16HgemmKernel {
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = VerifiedFP16HgemmKernel::Gemm;
  using StrideA = VerifiedFP16HgemmKernel::StrideA;
  using StrideB = VerifiedFP16HgemmKernel::StrideB;
  using StrideC = VerifiedFP16HgemmKernel::StrideC;
  using StrideD = VerifiedFP16HgemmKernel::StrideD;
  using ElementA = VerifiedFP16HgemmKernel::ElementA;
  using ElementB = VerifiedFP16HgemmKernel::ElementB;
  using ElementC = VerifiedFP16HgemmKernel::ElementC;
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS Verified FP16 GEMM cannot implement this problem size");
  }

  status = gemm.initialize(arguments, workspace.get());
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS Verified FP16 GEMM initialization failed");
  }
  ...
  if (cuda_status != cudaSuccess) {
    std::string err_msg = "CUDA error after Verified FP16 kernel launch: ";
    err_msg += cudaGetErrorString(cuda_status);
    throw std::runtime_error(err_msg);
  }

  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS Verified FP16 GEMM execution failed");
  }
