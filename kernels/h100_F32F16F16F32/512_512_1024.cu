...
__global__ void __launch_bounds__(128, 2)
    hgemm_ultra_optimized_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n,
                                 int k) {
...
void launch_hgemm_ultra_optimized(T *a, T *b, T *c, int M, int N, int K,
                                   int swizzle_stride) {
  auto BM = Int<128>{};
  auto BN = Int<128>{};
  auto BK = Int<64>{};
  ...
  cudaFuncSetAttribute(
      hgemm_ultra_optimized_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA,
  ...
  hgemm_ultra_optimized_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_ultra_optimized<half, 4, true>(
