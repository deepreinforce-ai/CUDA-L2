...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 1) hgemm_pipelined_8stage_kernel(
...
void launch_hgemm_pipelined_8stage(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_pipelined_8stage_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_pipelined_8stage<half, 8>(
