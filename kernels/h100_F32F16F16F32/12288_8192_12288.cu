...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  using ElementA = typename HgemmBreakthrough::ElementA;
  using ElementB = typename HgemmBreakthrough::ElementB;
  using ElementC = typename HgemmBreakthrough::ElementC;
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("Breakthrough GEMM cannot implement");
  }

  status = gemm.initialize(arguments, workspace.get());
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("Breakthrough GEMM initialization failed");
  }
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("Breakthrough GEMM execution failed");
  }
...
#else
  throw std::runtime_error("Requires SM90");
#endif
}