...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void hgemm_cute_optimal_kernel(T *Aptr, T *Bptr,
                                          T *Dptr, int m,
                                          int n, int k) {
...
#pragma unroll
  for (int i = 0; i < total_elems; i += 2) {
    float2 f2;
    f2.x = float(tCrD(i));
    f2.y = (i + 1 < total_elems) ? float(tCrD(i + 1)) : 0.0f;
    half2 h2 = __float22half2_rn(f2);
    half* h2ptr = reinterpret_cast<half*>(&h2);
    tCrD_half(i) = static_cast<T>(h2ptr[0]);
    if (i + 1 < total_elems) {
      tCrD_half(i + 1) = static_cast<T>(h2ptr[1]);
    }
  }
...
void launch_hgemm_bn64_bk64(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "SmemLayoutC exceeds available A pipe size");
