...
template <typename T, int BM, int BN, int BK, int kStage, int kSplits, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void hgemm_splitk_compute_kernel(T *Aptr, T *Bptr, T *Dptr, 
                                           int m, int n, int k) {
  using namespace cute;
  ...
  int k_per_split = (k + kSplits - 1) / kSplits;
  ...
  T* A_start = Aptr + iy * BM * k + k_start;
  T* B_start = Bptr + ix * BN * k + k_start;
  
  Tensor A_split = make_tensor(make_gmem_ptr(A_start), 
                               make_shape(Int<BM>{}, k_chunk),
                               make_stride(k, Int<1>{}));
  Tensor B_split = make_tensor(make_gmem_ptr(B_start), 
                               make_shape(Int<BN>{}, k_chunk),
                               make_stride(k, Int<1>{}));
  
  int out_offset = split_idx * m * n + iy * BM * n + ix * BN;
  Tensor D_split = make_tensor(make_gmem_ptr(Dptr + out_offset),
                               make_shape(Int<BM>{}, Int<BN>{}),
                               make_stride(n, Int<1>{}));

  Tensor gA = local_tile(A_split, make_tile(Int<BM>{}, Int<BK>{}), make_coord(0, _));
  Tensor gB = local_tile(B_split, make_tile(Int<BN>{}, Int<BK>{}), make_coord(0, _));
  Tensor gD = D_split;
  ...
  int ismem_read = 0;
  int ismem_write = 0;
  int ntile = (k_chunk + BK - 1) / BK;

  int prefetch_stages = min(kStage - 1, ntile);
#pragma unroll
...
template <typename T, int kSplits>
__global__ void reduce_splitk_simple_kernel(T *partial, T *output, int m, int n) {
  ...
  for (int k = 0; k < kSplits; ++k) {
    int offset = k * m * n + row * n + col;
    sum += __half2float(partial[offset]);
  }
  
  output[row * n + col] = __float2half(sum);
}

template <typename T, const int Stages = 4, const int KSplits = 32>
void launch_hgemm_splitk_cute(T *a, T *b, T *c, int M, int N, int K, T *partial_buf) {
  using namespace cute;
  ...
  using G2SCopyA = decltype(make_tiled_copy(
      g2s_copy_atom{}, make_layout(make_shape(Int<32>{}, Int<4>{}),
  ...
  using S2GCopyC = decltype(make_tiled_copy(
      S2GCopyAtomC{}, make_layout(make_shape(Int<32>{}, Int<4>{}),
                                  make_stride(Int<4>{}, Int<1>{})),
      make_layout(make_shape(Int<1>{}, Int<8>{}))));
  ...
  cudaFuncSetAttribute(
      hgemm_splitk_compute_kernel<T, BM, BN, BK, KStage, KSplits, MMA, G2SCopyA, G2SCopyB,
  ...
  hgemm_splitk_compute_kernel<T, BM, BN, BK, KStage, KSplits, MMA, G2SCopyA, G2SCopyB,
                             SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA,
                             S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>
      <<<grid, block, kShmSize>>>(a, b, partial_buf, M, N, K);

  dim3 reduce_block(16, 16);
  dim3 reduce_grid((N + 15) / 16, (M + 15) / 16);
  reduce_splitk_simple_kernel<T, KSplits><<<reduce_grid, reduce_block>>>(partial_buf, c, M, N);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  constexpr int k_splits = 32;
  
  static half *partial_buf = nullptr;
  static size_t allocated_size = 0;
  size_t required_size = M * N * k_splits;
  
  if (partial_buf == nullptr || allocated_size < required_size) {
    if (partial_buf != nullptr) {
      cudaFree(partial_buf);
    }
    cudaMalloc(&partial_buf, required_size * sizeof(half));
    allocated_size = required_size;
  }

  launch_hgemm_splitk_cute<half, 4, k_splits>(
      reinterpret_cast<half *>(a.data_ptr()),
      reinterpret_cast<half *>(b_col_major.data_ptr()),
      reinterpret_cast<half *>(c.data_ptr()), M, N, K, partial_buf);
}