    ...
    Alpha alpha, Beta beta)
{
  ...
  int iterations_remaining = size<1>(tAgA);
  ...
  while (k_tile_count > -K_PIPE_MAX)
  {
    ...
    int pipeline_fill_level = k_tile_count + K_PIPE_MAX;
    
    if (pipeline_fill_level >= K_PIPE_MAX - 1) {
      warpgroup_wait<1>();
    } else if (pipeline_fill_level >= 3) {
      warpgroup_wait<1>();
    } else {
      warpgroup_wait<0>();
    }
    ...
    int M, int N, int K)
{
  ...
  auto kernel_ptr = &gemm_wgmma_tma_kernel<
