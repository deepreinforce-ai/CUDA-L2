...
using HgemmUltraMGroup = HgemmKernelUltraMGroup<
...
void run_gemm_ultra_m_group(torch::Tensor a, torch::Tensor b_col_major, torch::Tensor c, 
                               int M, int N, int K) {
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  run_gemm_ultra_m_group<HgemmUltraMGroup>(a, b_col_major, c, M, N, K);
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported");
#endif
}