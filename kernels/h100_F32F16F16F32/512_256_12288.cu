    ...
    Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorage3Stage<TA, TB, SmemLayoutA, SmemLayoutB>;
...
void launch_hgemm_ultra_deep_k_sm90(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorage3Stage<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_ultra_deep_k_kernel<
                                 ...
                                 torch::Tensor b_col_major, torch::Tensor c)
{
  ...
  launch_hgemm_ultra_deep_k_sm90<cute::half_t, cute::half_t, cute::half_t>(
