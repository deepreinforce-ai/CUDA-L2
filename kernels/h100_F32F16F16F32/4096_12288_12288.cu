...
>
struct HgemmKernelDeepPipeline {
  ...
  using GridBlockShape = cute::Shape<cute::_1, cute::_2, cute::_1>;
...
using HgemmDeepPipeline = HgemmKernelDeepPipeline<
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  run_gemm<HgemmDeepPipeline>(a, b_col_major, c, M, N, K);
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported - H100 GPU required");
#endif
}