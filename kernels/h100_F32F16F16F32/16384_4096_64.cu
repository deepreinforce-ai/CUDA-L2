...
hgemm_f16acc_kernel(T *__restrict__ Aptr,
                    T *__restrict__ Bptr,
                    T *__restrict__ Dptr,
                    int m, int n, int k) {
...
#pragma unroll
  ...
  int total_steps = size<1>(tCrC_r2sx);
...
hgemm_f16acc_wide_kernel(T *__restrict__ Aptr,
                         T *__restrict__ Bptr,
                         T *__restrict__ Dptr,
                         int m, int n, int k) {
  using namespace cute;
...
void launch_hgemm_128x128_f16acc(T *a, T *b, T *c,
                                 int M, int N, int K,
                                 int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                "C smem exceeds A's single pipeline stage");
  ...
  cudaFuncSetAttribute(
      hgemm_f16acc_kernel<T, BM, BN, BK, KStage, MMA,
...
template <typename T, const int Stages = 2, const bool BlockSwizzle = true>
void launch_hgemm_128x256_f16acc(T *a, T *b, T *c,
                                 int M, int N, int K,
                                 int swizzle_stride) {
  using namespace cute;

  constexpr auto BM = Int<128>{};
  constexpr auto BN = Int<256>{};
  constexpr auto BK = Int<64>{};
  constexpr auto KStage = Int<Stages>{};
  constexpr auto kSmemLayoutCBatch = Int<4>{};
  ...
  cudaFuncSetAttribute(
      hgemm_f16acc_wide_kernel<T, BM, BN, BK, KStage, MMA,
          G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC,
          S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC,
          BlockSwizzle>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_128x128_f16acc<half, 2, true>(
