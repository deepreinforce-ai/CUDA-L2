...
void gemm_wgmma_relaxed_sync_kernel(
    ...
    Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageRelaxed<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  int num_iterations = k_tile_total;

  CUTE_NO_UNROLL
  for (int iter = 0; iter < num_iterations; ++iter)
  {
    int read_pipe = read_stage;
    
    ProducerBarType::wait(&producer_mbar[read_pipe], read_phase ? 1 : 0);
    ...
    if (is_producer_warp && lane_predicate && k_tile < k_tile_total)
    {
      int write_pipe = write_stage;
      
      ConsumerBarType::wait(&consumer_mbar[write_pipe], write_phase ? 1 : 0);
...
void launch_hgemm_relaxed_sync(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageRelaxed<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_wgmma_relaxed_sync_kernel<
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_relaxed_sync<cute::half_t, cute::half_t, cute::half_t>(
