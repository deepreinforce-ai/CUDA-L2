...
__global__ void __launch_bounds__(256, 2)
    hgemm_optimized_k64_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
template <typename T>
void launch_hgemm_optimized_k64(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_k64_kernel<T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_optimized_k64<half>(
