...
#pragma unroll
  ...
  auto sC0 = make_tensor(sA(_, _, ismem_read).data(), SmemLayoutC{});
  constexpr int sC_offset = cute::cosize(SmemLayoutC{});
  auto sC1 = make_tensor(sA(_, _, ismem_read).data() + sC_offset, SmemLayoutC{});

  auto r2s_tiled_copy_c = make_tiled_copy_C(R2SCopyAtomC{}, tiled_mma);
  auto r2s_thr_copy_c = r2s_tiled_copy_c.get_slice(idx);
  auto tCrC_r2s = r2s_thr_copy_c.retile_S(tCrD_half);
  
  auto tCsC0_r2s = r2s_thr_copy_c.partition_D(sC0);
  auto tCsC1_r2s = r2s_thr_copy_c.partition_D(sC1);

  S2GCopyC s2g_tiled_copy_c;
  auto s2g_thr_copy_c = s2g_tiled_copy_c.get_thread_slice(idx);
  
  auto tCsC0_s2g = s2g_thr_copy_c.partition_S(sC0);
  auto tCsC1_s2g = s2g_thr_copy_c.partition_S(sC1);
  ...
  const int step = size<3>(tCsC0_r2s);
  auto warp = cg::tiled_partition<32>(cg::this_thread_block());

#pragma unroll
  for (int j = 0; j < step; ++j) {
    cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, 0 * step + j), tCsC0_r2s(_, 0, 0, j));
  }
...
#pragma unroll
  for (int j = 0; j < step; ++j) {
    cute::copy(s2g_tiled_copy_c, tCsC0_s2g(_, 0, 0, j), tCgC_s2gx(_, 0 * step + j));
  }

#pragma unroll
  for (int j = 0; j < step; ++j) {
    cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, 1 * step + j), tCsC1_r2s(_, 0, 0, j));
  }
...
#pragma unroll
  for (int j = 0; j < step; ++j) {
    cute::copy(s2g_tiled_copy_c, tCsC1_s2g(_, 0, 0, j), tCgC_s2gx(_, 1 * step + j));
  }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    2 * size(SmemLayoutC{}),
                "Need space for two C buffers in shared memory");
