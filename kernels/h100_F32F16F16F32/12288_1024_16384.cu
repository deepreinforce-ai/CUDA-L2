...
struct HgemmKernelStandardHybrid {
...
#define CHECK_TORCH_TENSOR_DTYPE(T, th_type)                                   \
  if (((T).options().dtype() != (th_type))) {                                  \
    ...
    std::cout << "  Got: " << (T).options() << std::endl;                      \
    throw std::runtime_error("Tensor dtype validation failed");                \
  }
...
#define CHECK_TORCH_TENSOR_SHAPE(T, S0, S1)                                    \
  if (((T).size(0) != (S0)) || ((T).size(1) != (S1))) {                        \
    ...
    std::cout << "  Got: [" << (T).size(0) << ", " << (T).size(1) << "]"       \
              << std::endl;                                                    \
    throw std::runtime_error("Tensor shape validation failed");                \
  }
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  using Gemm = HgemmKernelStandardHybrid::Gemm;
  using StrideA = HgemmKernelStandardHybrid::StrideA;
  using StrideB = HgemmKernelStandardHybrid::StrideB;
  using StrideC = HgemmKernelStandardHybrid::StrideC;
  using StrideD = HgemmKernelStandardHybrid::StrideD;
  using ElementA = HgemmKernelStandardHybrid::ElementA;
  using ElementB = HgemmKernelStandardHybrid::ElementB;
  using ElementC = HgemmKernelStandardHybrid::ElementC;
  ...
  if (cuda_status != cudaSuccess) {
    throw std::runtime_error(std::string("Synchronization failed: ") + 
                           cudaGetErrorString(cuda_status));
  }

#else
  throw std::runtime_error("CUTLASS SM90 not supported - requires H100 GPU");
#endif
}