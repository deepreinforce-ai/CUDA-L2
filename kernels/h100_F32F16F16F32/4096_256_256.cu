...
hgemm_6stage_optimized_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
void launch_hgemm_6stage_optimized_bk64(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_6stage_optimized_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_6stage_optimized_bk64<half, 6, true>(
