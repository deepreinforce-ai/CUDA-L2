...
struct HgemmOptimizedExplicit4Stage {
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename HgemmOptimizedExplicit4Stage::Gemm;
  using StrideA = typename HgemmOptimizedExplicit4Stage::StrideA;
  using StrideB = typename HgemmOptimizedExplicit4Stage::StrideB;
  using StrideC = typename HgemmOptimizedExplicit4Stage::StrideC;
  using StrideD = typename HgemmOptimizedExplicit4Stage::StrideD;
  using ElementA = typename HgemmOptimizedExplicit4Stage::ElementA;
  using ElementB = typename HgemmOptimizedExplicit4Stage::ElementB;
  using ElementC = typename HgemmOptimizedExplicit4Stage::ElementC;
...
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported on this platform");
#endif
}