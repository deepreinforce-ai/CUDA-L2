...
hgemm_warp_tiled_blocked_kernel(const T * __restrict__ Aptr,
...

  {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      if (ik + 1 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 1, ismem),
                   tCrA_view(_, _, ik + 1));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 1, ismem),
                   tCrB_view(_, _, ik + 1));
      }
...
void launch_hgemm_warp_tiled_blocked(T *a, T *b, T *c, int M, int N,
                                      int K, int swizzle_stride) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_warp_tiled_blocked_kernel<
...
#define LAUNCH_HGEMM_MMA_STAGES_SWIZZLE_TN(stages, stride)                     \
  launch_hgemm_warp_tiled_blocked<half, (stages), true>(                       \
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  LAUNCH_HGEMM_MMA_STAGES_SWIZZLE_TN(2, swizzle_stride);
}