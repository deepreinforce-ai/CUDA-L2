...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename OptimizedTMABoxKernel::Gemm;
  using StrideA = typename OptimizedTMABoxKernel::StrideA;
  using StrideB = typename OptimizedTMABoxKernel::StrideB;
  using StrideC = typename OptimizedTMABoxKernel::StrideC;
  using StrideD = typename OptimizedTMABoxKernel::StrideD;
  using ElementA = typename OptimizedTMABoxKernel::ElementA;
  using ElementB = typename OptimizedTMABoxKernel::ElementB;
  using ElementC = typename OptimizedTMABoxKernel::ElementC;
...
#else
  throw std::runtime_error("CUTLASS SM90 (H100) architecture required");
#endif
}