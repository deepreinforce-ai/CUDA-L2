...
void gemm_enhanced_3stage_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                 ...
                                 Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageEnhanced<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  CUTE_UNROLL
  for (int pipe = 0; pipe < K_PIPE_MAX && k_tile < size<1>(tAgA); ++pipe, ++k_tile)
  {
    if ((warp_idx == 0) && lane_predicate)
    {
  ...
  int mma_iterations = k_tile_count;
  
  int threshold_3 = mma_iterations - 3;
  int threshold_2 = mma_iterations - 2;

  CUTE_NO_UNROLL
  for (int iter = 0; iter < mma_iterations; ++iter)
  {
...
void launch_enhanced_3stage_hgemm(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageEnhanced<TA, TB, decltype(sA), decltype(sB)>;
  int smem_size = int(sizeof(SharedStorage));

  dim3 dimBlock(size(tiled_mma));
  dim3 dimCluster(4, 1, 1);
  dim3 dimGrid(round_up(size(ceil_div(N, bN)), dimCluster.x),
               round_up(size(ceil_div(M, bM)), dimCluster.y));
  
  cutlass::ClusterLaunchParams params = {dimGrid, dimBlock, dimCluster, smem_size};

  auto kernel_ptr = &gemm_enhanced_3stage_kernel<decltype(prob_shape), decltype(cta_tiler),
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_enhanced_3stage_hgemm<cute::half_t, cute::half_t, cute::half_t>(
