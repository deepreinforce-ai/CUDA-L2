...
void launch_hgemm_staged(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;

  static constexpr int BM_val = 128;
  static constexpr int BN_val = 32;
  static constexpr int BK_val = 64;
  auto BM = Int<BM_val>{};
  auto BN = Int<BN_val>{};
  auto BK = Int<BK_val>{};
  ...
  using SmemLayoutA = decltype(tile_to_shape(
      SmemLayoutAtom{}, make_shape(BM, BK, KStage)));
  using SmemLayoutB = decltype(tile_to_shape(
      SmemLayoutAtom{}, make_shape(BN, BK, KStage)));
  ...
  const int BX = (N + BN_val - 1) / BN_val;
  const int BY = (M + BM_val - 1) / BM_val;
  dim3 block(size(MMA{}));
  dim3 grid(BX, BY);
  ...
  cudaFuncSetAttribute(
      hgemm_staged_kernel<T, BM_val, BN_val, BK_val, Stages, MMA,
                          G2SCopyA, G2SCopyB,
