...
hgemm_split_k_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  int itile_to_read = k_tile_offset;
  int ismem_read = 0;
  int ismem_write = 0;

  int prefetch_depth = min(kStage - 1, k_tiles);
#pragma unroll
  for (int istage = 0; istage < prefetch_depth; ++istage) {
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik == 0) {
        if (itile_to_read < k_tile_offset + k_tiles) {
          cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, itile_to_read),
                     tAsA_copy(_, _, _, ismem_write));
          cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, itile_to_read),
                     tBsB_copy(_, _, _, ismem_write));
...
template <typename T, int SplitK>
__global__ void __launch_bounds__(256)
reduce_split_k_kernel(T *partial_results, T *output, int m, int n) {
  int tid = blockIdx.x * blockDim.x + threadIdx.x;
  int total = m * n;
  
  if (tid >= total) return;
  
  T *base = partial_results + tid;
  
  float sum = 0.0f;
#pragma unroll
  for (int k = 0; k < SplitK; ++k) {
    sum += __half2float(__ldg(base + k * total));
  }
  
  output[tid] = __float2half(sum);
}

template <typename T, const int Stages = 6, const int SplitK = 8>
void launch_hgemm_split_k_cute(T *a, T *b, T *c, T *temp, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_split_k_kernel<T, BM, BN, BK, KStage, SplitK, MMA, G2SCopyA, G2SCopyB,
  ...
  hgemm_split_k_kernel<T, BM, BN, BK, KStage, SplitK, MMA, G2SCopyA, G2SCopyB,
                      SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA,
                      S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>
      <<<grid, block, kShmSize>>>(a, b, temp, M, N, K);

  int threads = 256;
  int blocks = (M * N + threads - 1) / threads;
  reduce_split_k_kernel<T, SplitK><<<blocks, threads>>>(temp, c, M, N);
}
