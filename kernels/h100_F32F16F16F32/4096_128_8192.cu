...
hgemm_bn64_double_buffer_kernel(
...
#pragma unroll
    ...
    auto tCrD_out = make_tensor_like<T>(tCrD);
    cute::copy(tCrD, tCrD_out);

    auto sC = make_tensor(sA(_, _, ismem_read).data(), SmemLayoutC{});

    auto r2s_tiled_copy_c = make_tiled_copy_C(R2SCopyAtomC{}, tiled_mma);
    auto r2s_thr_copy_c = r2s_tiled_copy_c.get_slice(idx);
    auto tCrC_r2s = r2s_thr_copy_c.retile_S(tCrD_out);
...
void launch_hgemm_optimized(half* a, half* b_col_major, half* c, int M, int N, int K) {
    using namespace cute;
    ...
    static constexpr int kShmSize = cute::max(shm_size_AB, shm_size_C) * sizeof(half);

    cudaFuncSetAttribute(
        hgemm_bn64_double_buffer_kernel<half, BM, BN, BK, kStage, MMA,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
    ...
    launch_hgemm_optimized(A_ptr, B_ptr, C_ptr, M, N, K);
}