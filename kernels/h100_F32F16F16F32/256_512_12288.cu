...
static cudaStream_t g_memory_stream = nullptr;
static bool g_streams_initialized = false;

struct HgemmApex_DualStream_4Stage {
...
struct HgemmApex_DualStream_5Stage {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
struct HgemmApex_DualStream_3Stage {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
struct HgemmApex_Pingpong_4Stage {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
struct HgemmApex_1x2_4Stage {
  ...
  using TileShape = cute::Shape<cute::_128, cute::_128, cute::_128>;
  using GridGroupShape = cute::Shape<cute::_1, cute::_2, cute::_1>;
...
struct HgemmApex_2x2_4Stage {
  ...
  using TileShape = cute::Shape<cute::_128, cute::_128, cute::_128>;
  using GridGroupShape = cute::Shape<cute::_2, cute::_2, cute::_1>;
...
struct HgemmApex_WarpSpec_Fallback {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
cutlass::Status execute_gemm_kernel(torch::Tensor a, torch::Tensor b_col_major, torch::Tensor c, 
                                     int M, int N, int K) {
  ...
  if (!g_streams_initialized) {
    cudaStreamCreateWithPriority(&g_compute_stream, cudaStreamNonBlocking, -1);
    cudaStreamCreateWithPriority(&g_memory_stream, cudaStreamNonBlocking, 0);
  ...
  size_t A_bytes = M * K * sizeof(ElementA);
  size_t B_bytes = K * N * sizeof(ElementB);

  constexpr size_t L2_THRESHOLD = 45 * 1024 * 1024;

  if ((A_bytes + B_bytes) < L2_THRESHOLD) {
    cudaMemAdvise(ptr_A, A_bytes, cudaMemAdviseSetReadMostly, 0);
    cudaMemAdvise(ptr_B, B_bytes, cudaMemAdviseSetReadMostly, 0);
    cudaMemPrefetchAsync(ptr_A, A_bytes, 0, g_memory_stream);
    cudaMemPrefetchAsync(ptr_B, B_bytes, 0, g_memory_stream);
  }
  ...
  if (workspace_size > g_workspace_size) {
    ...
    cudaMalloc(&g_workspace_ptr, workspace_size);
    g_workspace_size = workspace_size;
  }

  status = gemm.initialize(arguments, g_workspace_ptr);
  if (status != cutlass::Status::kSuccess) {
    return status;
  }

  status = gemm.run(g_compute_stream);
  if (status != cutlass::Status::kSuccess) {
    return status;
  }
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  cutlass::Status status = execute_gemm_kernel<HgemmApex_DualStream_4Stage>(a, b_col_major, c, M, N, K);
  
  if (status != cutlass::Status::kSuccess) {
    status = execute_gemm_kernel<HgemmApex_DualStream_5Stage>(a, b_col_major, c, M, N, K);
    
    if (status != cutlass::Status::kSuccess) {
      status = execute_gemm_kernel<HgemmApex_DualStream_3Stage>(a, b_col_major, c, M, N, K);
      
      if (status != cutlass::Status::kSuccess) {
        status = execute_gemm_kernel<HgemmApex_Pingpong_4Stage>(a, b_col_major, c, M, N, K);
        
        if (status != cutlass::Status::kSuccess) {
          status = execute_gemm_kernel<HgemmApex_1x2_4Stage>(a, b_col_major, c, M, N, K);
          
          if (status != cutlass::Status::kSuccess) {
            status = execute_gemm_kernel<HgemmApex_2x2_4Stage>(a, b_col_major, c, M, N, K);
            
            if (status != cutlass::Status::kSuccess) {
              status = execute_gemm_kernel<HgemmApex_WarpSpec_Fallback>(a, b_col_major, c, M, N, K);
              
              if (status != cutlass::Status::kSuccess) {
                throw std::runtime_error("All CUTLASS GEMM kernel configurations failed");
              }
            }
          }
        }
