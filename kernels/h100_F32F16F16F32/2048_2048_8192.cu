...
void gemm_main_kernel(
    ...
    TA const* __restrict__ A, CUTLASS_GRID_CONSTANT TmaA const tma_a,
    TB const* __restrict__ B, CUTLASS_GRID_CONSTANT TmaB const tma_b,
    TC* __restrict__ C, CStride dC, TiledMma mma,
    Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageMain<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  constexpr int tma_transaction_bytes = 
    sizeof(TA) * cosize_v<SmemLayoutA> / size<2>(SmemLayoutA{}) +
    sizeof(TB) * cosize_v<SmemLayoutB> / size<2>(SmemLayoutB{});
  ...
  if (remaining_tiles > 0) {
    ...
    while (remaining_tiles > 1)
    {
      ...
      ConsumerBarType::arrive(&consumer_mbar[read_state.index()]);
      ++read_state;
  ...
  CUTE_UNROLL
  for (int drain_idx = 0; drain_idx < K_PIPE_MAX; ++drain_idx) {
    int read_pipe = read_state.index();
    ProducerBarType::wait(&producer_mbar[read_pipe], read_state.phase());
    ...
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageMain<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_main_kernel<
    ...
    torch::Tensor c)
{
  ...
  launch_hgemm_main<cute::half_t, cute::half_t, cute::half_t>(
