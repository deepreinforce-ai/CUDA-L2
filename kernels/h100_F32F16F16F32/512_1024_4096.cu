...
struct SharedStorageWarpSpecialized
{
  ...
  alignas(64) uint64_t tma_barrier[cute::size<2>(SmemLayoutA{})];
  alignas(64) uint64_t mma_barrier[cute::size<2>(SmemLayoutA{})];
};
...
void gemm_warp_specialized_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                   ...
                                   Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageWarpSpecialized<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  int total_k_tiles = k_tile_count;
  int k_tile = 0;
  ...
  constexpr int EPILOGUE_START_OFFSET = 2;
  bool epilogue_active = false;
  ...
  while (k_tile_count > -K_PIPE_MAX)
  {
    ...
    int remaining_iterations = total_k_tiles - current_iteration - 1;
    
    if (remaining_iterations > EPILOGUE_START_OFFSET) {
      warpgroup_wait<2>();
    } 
    else if (remaining_iterations == EPILOGUE_START_OFFSET) {
      warpgroup_wait<2>();
...
void launch_hgemm_warp_specialized(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageWarpSpecialized<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_warp_specialized_kernel<decltype(prob_shape), decltype(cta_tiler),
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_warp_specialized<cute::half_t, cute::half_t, cute::half_t>(
