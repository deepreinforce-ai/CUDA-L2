...
void gemm_ultimate_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                          ...
                          Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageUltimate<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  constexpr int DRAIN_ITERATIONS = 3;
  int steady_state_iterations = k_tile_count + K_PIPE_MAX - DRAIN_ITERATIONS;

  CUTE_NO_UNROLL
  for (int iter = 0; iter < steady_state_iterations; ++iter)
  {
    ...
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageUltimate<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_ultimate_kernel<decltype(prob_shape), decltype(cta_tiler),
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_ultimate<cute::half_t, cute::half_t, cute::half_t>(
