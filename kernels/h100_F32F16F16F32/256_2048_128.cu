...
hgemm_h100_kernel(T *__restrict__ Aptr, T *__restrict__ Bptr,
  ...
  auto tCrA_buf = make_tensor_like(tCrA);
  auto tCrB_buf = make_tensor_like(tCrB);
  auto tCrA_buf_view = s2r_thr_copy_a.retile_D(tCrA_buf);
  auto tCrB_buf_view = s2r_thr_copy_b.retile_D(tCrB_buf);
  ...
  for (int ik = 0; ik < nk; ++ik) {
    ...
    int stage = last_ik ? 1 : 0;
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, stage),
               tCrA_buf_view(_, _, ik_next));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, stage),
               tCrB_buf_view(_, _, ik_next));

    cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik), tCrB(_, _, ik), tCrD);

    cute::copy(tCrA_buf(_, _, ik_next), tCrA(_, _, ik_next));
    cute::copy(tCrB_buf(_, _, ik_next), tCrB(_, _, ik_next));
  }

  #pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    int ik_next = (ik + 1) % nk;
    bool not_last = (ik < nk - 1);

    if (not_last) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, 1),
                 tCrA_buf_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, 1),
                 tCrB_buf_view(_, _, ik_next));
    }
...
void launch_hgemm_h100(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C smem exceeds one A pipe stage");
  ...
  cudaFuncSetAttribute(
      hgemm_h100_kernel<T, BM, BN, BK, KStage, MMA,
                        G2SCopyA, G2SCopyB,
