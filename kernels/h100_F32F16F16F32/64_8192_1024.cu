...
void launch_hgemm_optimized_v4(T* a, T* b_col_major, T* c, int M, int N, int K) {
    using namespace cute;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
    ...
    half* b_col_major_ptr = reinterpret_cast<half*>(b_col_major.data_ptr());
    half* c_ptr = reinterpret_cast<half*>(c.data_ptr());
    
    launch_hgemm_optimized_v4<half, 5>(a_ptr, b_col_major_ptr, c_ptr, M, N, K);
}