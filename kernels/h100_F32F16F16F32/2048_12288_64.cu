...
cuda_l2_h100_fp32_kernel(
  ...
  const int ix = blockIdx.z * gridDim.x + blockIdx.x;
  ...
  for (int ik = 0; ik < nk; ++ik) {
    if (ik + 1 < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 1, 0), tCrA_view(_, _, ik + 1));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 1, 0), tCrB_view(_, _, ik + 1));
    }
  ...
  for (int ik = 0; ik < nk; ++ik) {
    if (ik + 2 < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 2, 1), tCrA_view(_, _, ik + 2));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 2, 1), tCrB_view(_, _, ik + 2));
    }
    cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik), tCrB(_, _, ik), tCrD);
  }

  auto tCrD_half = make_tensor_like<T>(tCrD);
  auto tCrD_flat = flatten(tCrD);
  auto tCrD_half_flat = flatten(tCrD_half);

  const int total_elements = size(tCrD_flat);
  const int vec_iterations = total_elements / 16;
  const int remaining = total_elements % 16;

  #pragma unroll
  for (int i = 0; i < vec_iterations; ++i) {
    const int base = i * 16;
    tCrD_half_flat(base + 0)  = static_cast<T>(tCrD_flat(base + 0));
    tCrD_half_flat(base + 1)  = static_cast<T>(tCrD_flat(base + 1));
    tCrD_half_flat(base + 2)  = static_cast<T>(tCrD_flat(base + 2));
    tCrD_half_flat(base + 3)  = static_cast<T>(tCrD_flat(base + 3));
    tCrD_half_flat(base + 4)  = static_cast<T>(tCrD_flat(base + 4));
    tCrD_half_flat(base + 5)  = static_cast<T>(tCrD_flat(base + 5));
    tCrD_half_flat(base + 6)  = static_cast<T>(tCrD_flat(base + 6));
    tCrD_half_flat(base + 7)  = static_cast<T>(tCrD_flat(base + 7));
    tCrD_half_flat(base + 8)  = static_cast<T>(tCrD_flat(base + 8));
    tCrD_half_flat(base + 9)  = static_cast<T>(tCrD_flat(base + 9));
    tCrD_half_flat(base + 10) = static_cast<T>(tCrD_flat(base + 10));
    tCrD_half_flat(base + 11) = static_cast<T>(tCrD_flat(base + 11));
    tCrD_half_flat(base + 12) = static_cast<T>(tCrD_flat(base + 12));
    tCrD_half_flat(base + 13) = static_cast<T>(tCrD_flat(base + 13));
    tCrD_half_flat(base + 14) = static_cast<T>(tCrD_flat(base + 14));
    tCrD_half_flat(base + 15) = static_cast<T>(tCrD_flat(base + 15));
  }

  if (remaining > 0) {
    const int base = vec_iterations * 16;
    #pragma unroll
    for (int i = 0; i < remaining; ++i) {
      tCrD_half_flat(base + i) = static_cast<T>(tCrD_flat(base + i));
    }
  }
...
template <typename T>
void launch_hgemm_kernel(T *a, T *b, T *c, int M, int N, int K,
                         int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C smem exceeds A pipeline stage");
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  constexpr int swizzle_stride = 2048;

  launch_hgemm_kernel<half>(
