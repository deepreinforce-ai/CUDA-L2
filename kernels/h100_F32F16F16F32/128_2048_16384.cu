...
template <typename T, int BM, int BN, int BK, int kStage,
          ...
          int MinBlocksPerSM>
__global__ void __launch_bounds__(128, MinBlocksPerSM)
...
void launch_bm32_bn64_bk64_s8(half *a, half *b, half *c, int M, int N, int K) {
  using namespace cute;
  using T = half;
  ...
  static_assert(
      ...
      "SmemLayoutC larger than one SmemLayoutA stage");
  ...
  cudaFuncSetAttribute(
      hgemm_pipelined_kernel<
          T, BM, BN, BK, KStage, kSmemLayoutCBatch, MMA,
          G2SCopyA, G2SCopyB,
...
void launch_bm32_bn64_bk64_s6(half *a, half *b, half *c, int M, int N, int K) {
  using namespace cute;
  using T = half;
...
void launch_bm16_bn64_bk64_s6(half *a, half *b, half *c, int M, int N, int K) {
  using namespace cute;
  using T = half;
...
void launch_bm32_bn128_bk64_s6(half *a, half *b, half *c, int M, int N, int K) {
  using namespace cute;
  using T = half;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_bm32_bn64_bk64_s8(a_ptr, b_col_ptr, c_ptr, M, N, K);
}