...
void gemm_wgmma_tma_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                           ...
                           Alpha alpha, Beta beta)
{
  ...
  {
    int read_pipe_1 = read_state.index();
    ProducerBarType::wait(&producer_mbar[read_pipe_1], read_state.phase());

    warpgroup_arrive();
    gemm(mma, tCrA(_,_,_,read_pipe_1), tCrB(_,_,_,read_pipe_1), tCrC);
    warpgroup_commit_batch();

    ++read_state;
    
    int read_pipe_2 = read_state.index();
    ProducerBarType::wait(&producer_mbar[read_pipe_2], read_state.phase());

    warpgroup_arrive();
    gemm(mma, tCrA(_,_,_,read_pipe_2), tCrB(_,_,_,read_pipe_2), tCrC);
    warpgroup_commit_batch();

    warpgroup_wait<3>();

    ConsumerBarType::arrive(&consumer_mbar[read_pipe_1]);
    ConsumerBarType::arrive(&consumer_mbar[read_pipe_2]);
    ++read_state;

    if ((warp_idx == 0) && lane_predicate)
    {
      int pipe_1 = write_state.index();
      ConsumerBarType::wait(&consumer_mbar[pipe_1], write_state.phase());
      ProducerBarType::arrive_and_expect_tx(&producer_mbar[pipe_1], tma_transaction_bytes);
      copy(tma_a.with(producer_mbar[pipe_1]), tAgA(_,k_tile), tAsA(_,pipe_1));
      copy(tma_b.with(producer_mbar[pipe_1]), tBgB(_,k_tile), tBsB(_,pipe_1));
      ++write_state;
      --k_tile_count;
      ++k_tile;

      if (k_tile_count > -K_PIPE_MAX) {
        int pipe_2 = write_state.index();
        ConsumerBarType::wait(&consumer_mbar[pipe_2], write_state.phase());
        ProducerBarType::arrive_and_expect_tx(&producer_mbar[pipe_2], tma_transaction_bytes);
        copy(tma_a.with(producer_mbar[pipe_2]), tAgA(_,k_tile), tAsA(_,pipe_2));
        copy(tma_b.with(producer_mbar[pipe_2]), tBgB(_,k_tile), tBsB(_,pipe_2));
        ++write_state;
        --k_tile_count;
        ++k_tile;
      }
