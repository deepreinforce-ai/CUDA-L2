    ...
    int m, int n, int k)
{
    ...
    #pragma unroll
    for (int i = 0; i < size<1>(tCrC_flat); i += step) {
        #pragma unroll
        for (int j = 0; j < step; ++j) {
            cute::copy(tCrC_flat(_, i + j), tCsC_r2s(_, 0, 0, j));
        }
        __syncthreads();

        #pragma unroll
        for (int j = 0; j < step; ++j) {
            cute::copy(s2g_copy_c, tCsC_s2g(_, 0, 0, j), tCgC_flat(_, i + j));
        }
        __syncthreads();
    }
}
...
void launch_hgemm_micro_optimized(const T *a, const T *b_col, T *c, int M, int N, int K)
{
    static constexpr int BM     = 64;
    static constexpr int BN     = 64;
    static constexpr int BK     = 64;
    static constexpr int KStage = Stages;
    ...
    using SmemLayoutA = decltype(tile_to_shape(
        SmemAtom{}, make_shape(Int<BM>{}, Int<BK>{}, Int<KStage>{})));
    using SmemLayoutB = decltype(tile_to_shape(
        SmemAtom{}, make_shape(Int<BN>{}, Int<BK>{}, Int<KStage>{})));
    ...
    static constexpr int kMmaPM = 1 * kRepM * 16;
    static constexpr int kMmaPN = 2 * kRepN * 8;
    static constexpr int kMmaPK = 1 * kRepK * 16;
    using MMA_P_T = Tile<Int<kMmaPM>, Int<kMmaPN>, Int<kMmaPK>>;
    using MMA = decltype(make_tiled_mma(mma_atom{}, MMA_EU_RepeatT{}, MMA_P_T{}));

    using g2s_op     = SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>;
    using g2s_traits = Copy_Traits<g2s_op>;
    using g2s_atom   = Copy_Atom<g2s_traits, T>;
    ...
    using s2r_op       = SM75_U32x4_LDSM_N;
    using s2r_traits   = Copy_Traits<s2r_op>;
    using s2r_atom     = Copy_Atom<s2r_traits, T>;
    using S2RCopyAtomA = s2r_atom;
    using S2RCopyAtomB = s2r_atom;
    ...
    static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                  "SmemLayoutC must fit in SmemLayoutA");
    ...
    static constexpr int shm_AB  = cute::cosize(SmemLayoutA{}) + cute::cosize(SmemLayoutB{});
    static constexpr int shm_C   = cute::cosize(SmemLayoutC{});
    static constexpr int kShmSize = cute::max(shm_AB, shm_C) * (int)sizeof(T);
    ...
    torch::Tensor c)
{
    ...
    launch_hgemm_micro_optimized<half, 5>(
