...
hgemm_kernel(T *__restrict__ Aptr, T *__restrict__ Bptr,
...
template <typename T>
void launch_optimized_hgemm(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_kernel<T, BM, BN, BK, Stages, MMA,
                   G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  half *Aptr = reinterpret_cast<half *>(a.data_ptr());
  half *Bptr = reinterpret_cast<half *>(b_col_major.data_ptr());
  half *Cptr = reinterpret_cast<half *>(c.data_ptr());

  launch_optimized_hgemm<half>(Aptr, Bptr, Cptr, M, N, K);
}