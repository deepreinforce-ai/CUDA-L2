...
template <
    typename T,
    int BM, int BN, int BK, int kStages,
    typename TiledMMA,
...
hgemm_optimized_5stage_kernel(
    T const* __restrict__ Aptr,
    ...
    Aptr = (const T*)__builtin_assume_aligned(Aptr, 16);
    ...
    cute::copy(s2r_tiled_copy_a,
               tAsA(_, _, ik_init, ismem_read),
               tCrA_view(_, _, ik_init));
    cute::copy(s2r_tiled_copy_b,
               tBsB(_, _, ik_init, ismem_read),
               tCrB_view(_, _, ik_init));
...
void launch_hgemm_optimized_5stage(T* a, T* b, T* c, int M, int N, int K) {
    ...
    static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                  "C smem larger than A's single stage");
...
void cuda_l2_h100_fp32(
    ...
    launch_hgemm_optimized_5stage<half, 5>(a_ptr, b_ptr, c_ptr, M, N, K);
}