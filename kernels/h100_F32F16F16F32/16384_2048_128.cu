...
__global__ void __launch_bounds__(256, 2)
    hgemm_k128_ultimate(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  const int swizzle_size = 8;
  const int bx_raw = blockIdx.x;
  const int by_raw = blockIdx.y;
  const int tiles_per_group = swizzle_size;
  const int group_id = bx_raw / tiles_per_group;
  const int group_offset = bx_raw % tiles_per_group;
  const int ix = group_id * tiles_per_group + group_offset;
  const int iy = by_raw;
  ...
  if (ntile == 2) {
    ...
    for (int itile = 0; itile < ntile; ++itile) {
      ...
      for (int ik = 0; ik < nk; ++ik) {
        ...
        if (ik == 0 && itile + kStage - 1 < ntile) {
          int next_tile = itile + kStage - 1;
  ...
  const int num_batches = size<1>(tCrC_r2sx) / step;

  #pragma unroll
  for (int i = 0; i < num_batches; ++i) {
    #pragma unroll
    for (int j = 0; j < step; ++j) {
      cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, i * step + j), tCsC_r2s(_, 0, 0, j));
    }
    ...
    #pragma unroll
    for (int j = 0; j < step; ++j) {
      cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), tCgC_s2gx(_, i * step + j));
    }
...
void launch_hgemm_k128_ultimate(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_k128_ultimate<T, BM, BN, BK, KStage, MMA, G2SCopyA,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_k128_ultimate<half, 2>(
