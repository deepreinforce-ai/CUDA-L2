    ...
    int m, int n, int k)
{
  ...
  int idx = threadIdx.x;
  int ix  = static_cast<int>(BlockSwizzle) * blockIdx.z * gridDim.x + blockIdx.x;
  int iy  = blockIdx.y;
  ...
  auto tAsA_r     = s2r_thr_a.partition_S(sA);
  auto tCrA_v     = s2r_thr_a.retile_D(tCrA);

  auto s2r_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_b  = s2r_copy_b.get_slice(idx);
  auto tBsB_r     = s2r_thr_b.partition_S(sB);
  auto tCrB_v     = s2r_thr_b.retile_D(tCrB);
...
#pragma unroll
  ...
  cute::copy(s2r_copy_a, tAsA_r(_, _, 0, ismem_read), tCrA_v(_, _, 0));
  cute::copy(s2r_copy_b, tBsB_r(_, _, 0, ismem_read), tCrB_v(_, _, 0));
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      cute::copy(s2r_copy_a, tAsA_r(_, _, ik_next, ismem_read),
                 tCrA_v(_, _, ik_next));
      cute::copy(s2r_copy_b, tBsB_r(_, _, ik_next, ismem_read),
                 tCrB_v(_, _, ik_next));
...
#pragma unroll
    for (int j = 0; j < step; ++j) {
      ...
      cute::copy(r2s_copy_c, t, tCsC_r2s(_, 0, 0, j));
    }
    __syncthreads();
#pragma unroll
    for (int j = 0; j < step; ++j) {
...
template <typename T, int Stages, bool UseSwizzle>
static void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K,
                                   int swizzle_stride) {
  using namespace cute;
  ...
  int BX_full = (N + BN - 1) / BN;
  int BY      = (M + BM - 1) / BM;
  int BZ      = UseSwizzle ? (N + swizzle_stride - 1) / swizzle_stride : 1;
  int BX      = UseSwizzle ? (BX_full + BZ - 1) / BZ : BX_full;

  constexpr int NumThreads = 128;
  dim3 block(NumThreads);
  dim3 grid(BX, BY, BZ);

  constexpr int shm_AB = (cute::cosize(SmemLayoutA{}) +
                          cute::cosize(SmemLayoutB{})) * sizeof(T);
  constexpr int shm_C  = cute::cosize(SmemLayoutC{}) * sizeof(T);
  constexpr int kShmSize = cute::max(shm_AB, shm_C);

  auto kernel_fn = hgemm_optimized_kernel<
      T, BM, BN, BK, KStage, MMA,
      G2SCopyA, G2SCopyB,
  ...
  kernel_fn<<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}
                                  ...
                                  torch::Tensor b_col_major, torch::Tensor c)
{
  ...
  half *pa  = reinterpret_cast<half *>(a.data_ptr());
  half *pbc = reinterpret_cast<half *>(b_col_major.data_ptr());
  half *pc  = reinterpret_cast<half *>(c.data_ptr());

  launch_hgemm_optimized<half, 4, true>(pa, pbc, pc, M, N, K, 2048);
}