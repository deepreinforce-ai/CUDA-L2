...
struct PrimaryLargeTileHgemmKernel {
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename PrimaryLargeTileHgemmKernel::Gemm;
  using StrideA = typename PrimaryLargeTileHgemmKernel::StrideA;
  using StrideB = typename PrimaryLargeTileHgemmKernel::StrideB;
  using StrideC = typename PrimaryLargeTileHgemmKernel::StrideC;
  using StrideD = typename PrimaryLargeTileHgemmKernel::StrideD;
  using ElementA = typename PrimaryLargeTileHgemmKernel::ElementA;
  using ElementB = typename PrimaryLargeTileHgemmKernel::ElementB;
  using ElementC = typename PrimaryLargeTileHgemmKernel::ElementC;
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS GEMM cannot implement - check tile/group configuration");
  }

  status = gemm.initialize(arguments, workspace.get());
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS GEMM initialization failed - check resource limits");
  }
...
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported - H100 GPU with compute capability 9.0 required");
#endif
}