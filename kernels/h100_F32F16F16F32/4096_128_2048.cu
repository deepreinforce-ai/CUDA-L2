...
__device__ __forceinline__ void fp32x4_to_fp16x4_fast(
    float a0, float a1, float a2, float a3, half* __restrict__ out) {
  unsigned r0, r1;
  asm volatile(
    "cvt.rn.f16x2.f32 %0, %2, %3;\n\t"
    "cvt.rn.f16x2.f32 %1, %4, %5;\n\t"
    : "=r"(r0), "=r"(r1)
    : "f"(a1), "f"(a0), "f"(a3), "f"(a2)
  );
  *(unsigned*)&out[0] = r0;
...
hgemm_gen5_kernel(T* __restrict__ Aptr, T* __restrict__ Bptr, 
...
#pragma unroll
  ...
  int num_elements = size(tCrD);

#pragma unroll
  for (int i = 0; i + 3 < num_elements; i += 4) {
    fp32x4_to_fp16x4_fast(tCrD(i), tCrD(i+1), tCrD(i+2), tCrD(i+3),
                           &tCrD_half(i));
  }
#pragma unroll
  for (int i = (num_elements / 4) * 4; i < num_elements; ++i) {
    tCrD_half(i) = __float2half_rn(tCrD(i));
  }
...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 2)
hgemm_gen5_bk64_kernel(T* __restrict__ Aptr, T* __restrict__ Bptr, 
                       T* __restrict__ Dptr, int m, int n, int k) {
  extern __shared__ T shm_data[];
  ...
  auto g2s_thr_copy_a = g2s_tiled_copy_a.get_slice(idx);
  auto tAgA_copy      = g2s_thr_copy_a.partition_S(gA);
  auto tAsA_copy      = g2s_tiled_copy_a.get_slice(idx).partition_D(sA);

  G2SCopyB g2s_tiled_copy_b;
  auto g2s_thr_copy_b = g2s_tiled_copy_b.get_slice(idx);
  auto tBgB_copy      = g2s_thr_copy_b.partition_S(gB);
  auto tBsB_copy      = g2s_tiled_copy_b.get_slice(idx).partition_D(sB);
...
template <typename T>
void launch_gen5_bk32(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;

  constexpr int BM_V    = 64;
  constexpr int BN_V    = 64;
  constexpr int BK_V    = 32;
  ...
  using SmemLayoutAtom = decltype(composition(
      Swizzle<3, 3, 3>{},
      make_layout(make_shape(Int<8>{}, Int<BK_V>{}),
                  make_stride(Int<BK_V>{}, Int<1>{}))));

  using SmemLayoutA = decltype(tile_to_shape(
      SmemLayoutAtom{},
      make_shape(Int<BM_V>{}, Int<BK_V>{}, Int<Stages>{})));
  using SmemLayoutB = decltype(tile_to_shape(
      SmemLayoutAtom{},
      make_shape(Int<BN_V>{}, Int<BK_V>{}, Int<Stages>{})));
  ...
  int BX = (N + BN_V - 1) / BN_V;
  int BY = (M + BM_V - 1) / BM_V;
...
template <typename T>
void launch_gen5_bk64(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;

  constexpr int BM_V   = 64;
  constexpr int BN_V   = 64;
  constexpr int BK_V   = 64;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  if (g_best_kernel < 0) {
    launch_gen5_bk32<half>(a_ptr, b_ptr, c_ptr, M, N, K);
    cudaDeviceSynchronize();
    launch_gen5_bk64<half>(a_ptr, b_ptr, c_ptr, M, N, K);
    cudaDeviceSynchronize();

    cudaEvent_t s0, e0, s1, e1;
