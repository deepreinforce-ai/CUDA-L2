...
void gemm_optimized_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                           ...
                           Alpha alpha, Beta beta)
{
  ...
  auto K_PIPE_MAX = size<2>(SmemLayoutA{});
  ...
  constexpr int WGMMA_INFLIGHT = 3;
  ...
  for (int k_tile_idx = 0; k_tile_idx < k_tile_count; ++k_tile_idx)
  {
    ...
    if (k_tile_idx + WGMMA_INFLIGHT < k_tile_count) {
      warpgroup_wait<WGMMA_INFLIGHT>();
    } else if (k_tile_idx + 1 < k_tile_count) {
      warpgroup_wait<1>();
    }
...
void launch_hgemm_optimized_sm90(TA const* A, int ldA, TB const* B, int ldB,
                                  TC* C, int ldC, int M, int N, int K)
{
  ...
  auto kernel_ptr = &gemm_optimized_kernel<decltype(prob_shape), decltype(cta_tiler),
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_optimized_sm90<cute::half_t, cute::half_t, cute::half_t>(
