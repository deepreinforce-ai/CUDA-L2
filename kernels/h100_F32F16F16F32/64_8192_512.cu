...
template <typename T, int BM, int BN, int BK, int kStage, int kNTile,
          ...
          typename S2GCopyAtomC, typename S2GCopyC, const bool BlockSwizzle>
__global__ void __launch_bounds__(128, 2)
hgemm_optimized_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
void launch_optimized_hgemm(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;

  constexpr int BM_val = 64;
  constexpr int BN_val = 128;
  constexpr int BK_val = 64;
  constexpr int ntile_val = 8;
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_kernel<T, BM_val, BN_val, BK_val, Stages, ntile_val,
  ...
  hgemm_optimized_kernel<T, BM_val, BN_val, BK_val, Stages, ntile_val, MMA,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_optimized_hgemm<half, 3, true>(a_ptr, b_ptr, c_ptr, M, N, K, 2048);
}