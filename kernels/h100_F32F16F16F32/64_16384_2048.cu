...
__global__ void __launch_bounds__(256)
  ...
  for (int ix = blockIdx.x; ix < tiles_n; ix += gridDim.x) {
...
#pragma unroll
    ...
    constexpr int num_elements = decltype(size(tCrD))::value;
    static_assert(num_elements % 2 == 0, "Must have even number of elements");

#pragma unroll
    for (int i = 0; i < num_elements; i += 2) {
      float2 f2 = make_float2(tCrD(i), tCrD(i + 1));
      half2 h2 = __float22half2_rn(f2);
      tCrD_half(i) = __low2half(h2);
      tCrD_half(i + 1) = __high2half(h2);
    }
...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  ...
  int num_sms = 132;
  int persistent_bx = min(BX, num_sms);

  dim3 block(size(MMA{}));
  dim3 grid(persistent_bx, BY, 1);

  static constexpr int shm_size_AB =
      cute::cosize(SmemLayoutA{}) + cute::cosize(SmemLayoutB{});
  static constexpr int shm_size_C = cute::cosize(SmemLayoutC{});
  static constexpr int kShmSize =
      cute::max(shm_size_AB, shm_size_C) * (int)sizeof(T);

  cudaFuncSetAttribute(
      cuda_l2_h100_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA,
