...
struct KTile80HgemmKernel {
  ...
  using TileShape = cute::Shape<cute::_128, cute::_256, cute::_80>;
  using GridShape = cute::Shape<cute::_1, cute::_1, cute::_1>;
...
struct KTile32HgemmKernel {
  ...
  using TileShape = cute::Shape<cute::_128, cute::_256, cute::_32>;
  using GridShape = cute::Shape<cute::_1, cute::_1, cute::_1>;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  bool success = try_gemm_kernel<KTile64HgemmKernel>(
      M, N, K, ptr_A, ptr_B, ptr_C, ptr_D);

  if (!success) {
    success = try_gemm_kernel<KTile80HgemmKernel>(
        M, N, K, ptr_A, ptr_B, ptr_C, ptr_D);

    if (!success) {
      success = try_gemm_kernel<KTile96HgemmKernel>(
          M, N, K, ptr_A, ptr_B, ptr_C, ptr_D);

      if (!success) {
        success = try_gemm_kernel<KTile32HgemmKernel>(
            M, N, K, ptr_A, ptr_B, ptr_C, ptr_D);
