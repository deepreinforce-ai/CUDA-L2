...
hgemm_bk64_optimized_kernel(const T *__restrict__ Aptr,
  ...
  auto gA = local_tile(A, make_tile(Int<BM>{}, Int<BK>{}), make_coord(by, _));
  auto gB = local_tile(B, make_tile(Int<BN>{}, Int<BK>{}), make_coord(bx, _));
  auto gD =
      local_tile(D, make_tile(Int<BM>{}, Int<BN>{}), make_coord(by, bx));
...
#pragma unroll
  ...
  int ik = 0;
  cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, ik, ismem_read),
             tCrA_view(_, _, ik));
  cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, ik, ismem_read),
             tCrB_view(_, _, ik));
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, ik_next, ismem_read),
                 tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, ik_next, ismem_read),
...
void launch_hgemm_bk64(const T *a, const T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  auto kernel_fn = hgemm_bk64_optimized_kernel<
