...
#pragma unroll
    for (int j = 0; j < step; ++j) {
      cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, (i + 1) * step + j), tCsC_r2s(_, 0, 0, j));
    }
  ...
  if (num_iters > 0) {
    int last = num_iters - 1;
#pragma unroll
    for (int j = 0; j < step; ++j) {
      cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), tCgC_s2gx(_, last * step + j));
    }
  }
}
