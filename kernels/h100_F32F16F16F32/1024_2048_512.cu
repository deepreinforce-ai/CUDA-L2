...
cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  const int tiles_per_block = (total_tiles + 131) / 132;
  const int block_tile_start = blockIdx.x * tiles_per_block;
  const int block_tile_end = min(block_tile_start + tiles_per_block, total_tiles);

  for (int tile_idx = block_tile_start; tile_idx < block_tile_end; ++tile_idx) {
    int iy = tile_idx / total_tiles_x;
    int ix = tile_idx % total_tiles_x;
