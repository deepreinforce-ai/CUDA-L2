...
struct HgemmVerticalTileKernel {
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename HgemmVerticalTileKernel::Gemm;
  using StrideA = typename HgemmVerticalTileKernel::StrideA;
  using StrideB = typename HgemmVerticalTileKernel::StrideB;
  using StrideC = typename HgemmVerticalTileKernel::StrideC;
  using StrideD = typename HgemmVerticalTileKernel::StrideD;
  using ElementA = typename HgemmVerticalTileKernel::ElementA;
  using ElementB = typename HgemmVerticalTileKernel::ElementB;
  using ElementC = typename HgemmVerticalTileKernel::ElementC;
...
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported - requires H100 GPU");
#endif
}