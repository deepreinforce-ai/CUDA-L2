...
void gemm_wgmma_vectorized_epilogue_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                           ...
                                           Alpha alpha, Beta beta)
{
  ...
  bool is_producer_thread = (warp_idx == 0) && lane_predicate;
  ...
  int ultra_sustained_threshold = k_tile_count - 1;
  ...
  for (int iter = 0; iter < k_tile_count; ++iter)
  {
    ...
    if (iter < ultra_sustained_threshold) {
      warpgroup_wait<3>();
    } else {
      int remaining = k_tile_count - iter - 1;
      if (remaining >= 3) {
        warpgroup_wait<2>();
      } else if (remaining >= 2) {
        warpgroup_wait<1>();
    ...
    int M, int N, int K)
{
  ...
  auto kernel_ptr = &gemm_wgmma_vectorized_epilogue_kernel<decltype(prob_shape), decltype(cta_tiler),
