...
struct HgemmAlgorithm2Fallback {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
struct Hgemm3StageConservative {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  try {
    run_gemm_kernel<HgemmChampion1x1x1>(a, b_col_major, c, M, N, K);
    return;
  } catch (const std::exception& e) {
  }
  
  try {
    run_gemm_kernel<HgemmPingpong1x1x1>(a, b_col_major, c, M, N, K);
    return;
  } catch (const std::exception& e) {
  }
  
  try {
    run_gemm_kernel<HgemmNOriented1x2x1>(a, b_col_major, c, M, N, K);
    return;
  } catch (const std::exception& e) {
  }
  
  try {
    run_gemm_kernel<HgemmAutoCarveout1x1x1>(a, b_col_major, c, M, N, K);
    return;
  } catch (const std::exception& e) {
  }
  
  try {
    run_gemm_kernel<HgemmAlgorithm2Fallback>(a, b_col_major, c, M, N, K);
    return;
  } catch (const std::exception& e) {
  }
  
  run_gemm_kernel<Hgemm3StageConservative>(a, b_col_major, c, M, N, K);
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported");
#endif
}