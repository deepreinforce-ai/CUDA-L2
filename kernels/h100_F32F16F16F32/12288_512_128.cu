...
cuda_l2_h100_fp32_kernel(const T * __restrict__ Aptr,
                                   const T * __restrict__ Bptr,
                                   T * __restrict__ Dptr,
                                   int m, int n, int k) {
  using namespace cute;
  
  extern __shared__ __align__(128) T shm_data[];
  T *Ashm = shm_data;
  ...
  cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, 3), tAsA_copy(_, _, _, 3));
  cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, 3), tBsB_copy(_, _, _, 3));
  cp_async_fence();
  ...
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 2), tCrA_view(_, _, 1));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 2), tCrB_view(_, _, 1));
  ...
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, 3), tCrA_view(_, _, 0));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, 3), tCrB_view(_, _, 0));
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 3), tCrA_view(_, _, 1));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 3), tCrB_view(_, _, 1));
  ...
  const register int step = size<3>(tCsC_r2s);

#pragma unroll
  for (register int i = 0; i < size<1>(tCrC_r2sx); i += step) {
#pragma unroll
...
void launch_hgemm_specialized_k128(T *a, T *b, T *c, int M, int N, int K,
                                    int swizzle_stride) {
  using namespace cute;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_specialized_k128<half, 5, true>(
