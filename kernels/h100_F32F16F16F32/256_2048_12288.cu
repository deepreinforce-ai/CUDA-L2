...
void gemm_async_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                       ...
                       Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageAsync<TA, TB, SmemLayoutA, SmemLayoutB>;
    ...
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageAsync<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_async_kernel<decltype(prob_shape), decltype(cta_tiler),
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_async<cute::half_t, cute::half_t, cute::half_t>(
