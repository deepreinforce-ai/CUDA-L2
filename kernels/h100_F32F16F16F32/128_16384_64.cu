...
hgemm_refined_ultra_optimized_kernel(const T *__restrict__ Aptr,
...
#pragma unroll
  ...
  constexpr int ik_last = nk - 1;
  cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik_last), tCrB(_, _, ik_last), tCrD);

  int total_elements = size(tCrD);
...
void launch_refined_ultra_optimized_hgemm(T *a, T *b, T *c, int M, int N, int K,
                                          int swizzle_stride) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_refined_ultra_optimized_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_refined_ultra_optimized_hgemm<half, 3, true>(
