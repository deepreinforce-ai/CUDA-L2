    ...
    torch::Tensor c)
{
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename HgemmFP16NTile::Gemm;
  using StrideA = typename HgemmFP16NTile::StrideA;
  using StrideB = typename HgemmFP16NTile::StrideB;
  using StrideC = typename HgemmFP16NTile::StrideC;
  using StrideD = typename HgemmFP16NTile::StrideD;
  using ElementA = typename HgemmFP16NTile::ElementA;
  using ElementB = typename HgemmFP16NTile::ElementB;
  using ElementC = typename HgemmFP16NTile::ElementC;
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS FP16-N tile-group GEMM cannot implement this problem");
  }

  status = gemm.initialize(arguments, workspace.get());
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS FP16-N tile-group GEMM initialization failed");
  }
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS FP16-N tile-group GEMM execution failed");
  }

  cuda_err = cudaDeviceSynchronize();
  if (cuda_err != cudaSuccess) {
    throw std::runtime_error(std::string("CUDA synchronization error: ") +
                           cudaGetErrorString(cuda_err));
  }
#else
