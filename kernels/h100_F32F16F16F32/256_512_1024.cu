...
cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_buf0 = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_buf0 = thr_mma.partition_fragment_B(gB(_, _, 0));
  auto tCrA_buf1 = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_buf1 = thr_mma.partition_fragment_B(gB(_, _, 0));
  auto tCrA_buf2 = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_buf2 = thr_mma.partition_fragment_B(gB(_, _, 0));
  ...
  auto tCrA_view_buf0 = s2r_thr_copy_a.retile_D(tCrA_buf0);
  auto tCrA_view_buf1 = s2r_thr_copy_a.retile_D(tCrA_buf1);
  auto tCrA_view_buf2 = s2r_thr_copy_a.retile_D(tCrA_buf2);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view_buf0 = s2r_thr_copy_b.retile_D(tCrB_buf0);
  auto tCrB_view_buf1 = s2r_thr_copy_b.retile_D(tCrB_buf1);
  auto tCrB_view_buf2 = s2r_thr_copy_b.retile_D(tCrB_buf2);
...
#pragma unroll
  ...
  int nk = size<2>(tCrA_buf0);
  
  if (ntile > 0) {
    if (nk >= 1) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, ismem_read), tCrA_view_buf0(_, _, 0));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, ismem_read), tCrB_view_buf0(_, _, 0));
    }
    if (nk >= 2) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, ismem_read), tCrA_view_buf1(_, _, 1));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, ismem_read), tCrB_view_buf1(_, _, 1));
    }
    if (nk >= 3) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, 2, ismem_read), tCrA_view_buf2(_, _, 2));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, 2, ismem_read), tCrB_view_buf2(_, _, 2));
    }
  }
...
#pragma unroll
    for (int ik = 0; ik < nk; ik += 4) {
      ...
      if (ik_next3 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next3, ismem_read), 
                   tCrA_view_buf0(_, _, ik_next3));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next3, ismem_read), 
                   tCrB_view_buf0(_, _, ik_next3));
      }
      
      cute::gemm(tiled_mma, tCrD, tCrA_buf0(_, _, ik), tCrB_buf0(_, _, ik), tCrD);
      
      if (ik + 1 < nk) {
        if (ik_next4 < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next4, ismem_read), 
                     tCrA_view_buf1(_, _, ik_next4));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next4, ismem_read), 
                     tCrB_view_buf1(_, _, ik_next4));
        }
        
        cute::gemm(tiled_mma, tCrD, tCrA_buf1(_, _, ik + 1), tCrB_buf1(_, _, ik + 1), tCrD);
      }
      
      if (ik + 2 < nk) {
        if (ik_next5 < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next5, ismem_read), 
                     tCrA_view_buf2(_, _, ik_next5));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next5, ismem_read), 
                     tCrB_view_buf2(_, _, ik_next5));
        }
        
        cute::gemm(tiled_mma, tCrD, tCrA_buf2(_, _, ik + 2), tCrB_buf2(_, _, ik + 2), tCrD);
      }
      
      if (ik + 3 < nk) {
        if (ik_next6 < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next6, ismem_read), 
                     tCrA_view_buf0(_, _, ik_next6));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next6, ismem_read), 
                     tCrB_view_buf0(_, _, ik_next6));
        }
        
        cute::gemm(tiled_mma, tCrD, tCrA_buf0(_, _, ik + 3), tCrB_buf0(_, _, ik + 3), tCrD);
      }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                "Shared memory sizing constraint violated");
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half, 4, true>(
