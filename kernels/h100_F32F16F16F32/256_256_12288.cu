...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void hgemm_split_k_kernel(T *Aptr, T *Bptr, T *Dptr, 
  ...
  Tensor gB = local_tile(B, make_tile(Int<BN>{}, Int<BK>{}),
                         make_coord(ix, _));
  Tensor gD = local_tile(D(iz, _, _), make_tile(Int<BM>{}, Int<BN>{}),
                         make_coord(iy, ix));
  ...
  int tile_start = k_start / BK;
  int ntile = (k_actual + BK - 1) / BK;
  int itile_to_read = 0;
  int ismem_read = 0;
  int ismem_write = 0;
...
#pragma unroll
  for (int istage = 0; istage < kStage - 1; ++istage) {
    if (itile_to_read < ntile) {
      int global_tile = tile_start + itile_to_read;
      cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, global_tile),
                 tAsA_copy(_, _, _, ismem_write));
      cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, global_tile),
...
template <typename T>
__global__ void reduce_split_k_kernel(T *partial, T *output, 
...
#pragma unroll
    for (int k = 0; k < k_split; ++k) {
      sum += __half2float(partial[k * total + idx]);
    }
    output[idx] = __float2half(sum);
  }
}

template <typename T, const int Stages = 6, const int KSplit = 4>
void launch_hgemm_split_k_cute(T *a, T *b, T *c, T *partial,
                               int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_split_k_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
  ...
  hgemm_split_k_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
                      ...
                      S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC,
                      S2GCopyAtomC, S2GCopyC>
      <<<grid, block, kShmSize>>>(a, b, partial, M, N, K, KSplit);

  int threads = 256;
  int blocks = (M * N + threads - 1) / threads;
  reduce_split_k_kernel<<<blocks, threads>>>(partial, c, M, N, KSplit);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  constexpr int K_SPLIT = 4;
  auto partial = torch::empty({K_SPLIT, M, N}, 
                              torch::TensorOptions().dtype(torch::kHalf).device(torch::kCUDA));

  launch_hgemm_split_k_cute<half, 6, K_SPLIT>(
      reinterpret_cast<half *>(a.data_ptr()),
      reinterpret_cast<half *>(b_col_major.data_ptr()),
      reinterpret_cast<half *>(c.data_ptr()),
      reinterpret_cast<half *>(partial.data_ptr()),
      M, N, K);
}