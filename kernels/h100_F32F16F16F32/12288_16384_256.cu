...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename PowerOf2OptimizedHgemm::Gemm;
  using StrideA = typename PowerOf2OptimizedHgemm::StrideA;
  using StrideB = typename PowerOf2OptimizedHgemm::StrideB;
  using StrideC = typename PowerOf2OptimizedHgemm::StrideC;
  using StrideD = typename PowerOf2OptimizedHgemm::StrideD;
  using ElementA = typename PowerOf2OptimizedHgemm::ElementA;
  using ElementB = typename PowerOf2OptimizedHgemm::ElementB;
  using ElementC = typename PowerOf2OptimizedHgemm::ElementC;
  ...
  if (cuda_status != cudaSuccess) {
    std::string err_msg = "CUDA sync error: ";
    err_msg += cudaGetErrorString(cuda_status);
    throw std::runtime_error(err_msg);
  }
#else
