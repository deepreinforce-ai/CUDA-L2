...
hgemm_optimized_904_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  using namespace cute;
  ...
  int total_size = size(tCrD);
  
  #pragma unroll
  for (int i = 0; i < total_size; i += kVecWidth) {
    int remaining = total_size - i;
    if (remaining >= kVecWidth) {
      tCrD_half(i + 0) = static_cast<T>(tCrD(i + 0));
      tCrD_half(i + 1) = static_cast<T>(tCrD(i + 1));
      tCrD_half(i + 2) = static_cast<T>(tCrD(i + 2));
      tCrD_half(i + 3) = static_cast<T>(tCrD(i + 3));
      tCrD_half(i + 4) = static_cast<T>(tCrD(i + 4));
      tCrD_half(i + 5) = static_cast<T>(tCrD(i + 5));
      tCrD_half(i + 6) = static_cast<T>(tCrD(i + 6));
      tCrD_half(i + 7) = static_cast<T>(tCrD(i + 7));
    } else {
      for (int j = 0; j < remaining; ++j) {
        tCrD_half(i + j) = static_cast<T>(tCrD(i + j));
      }
    }
  }
...
void launch_optimized_904_hgemm(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_904_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
                                 SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA,
                                 S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle>,
...
#define LAUNCH_OPTIMIZED_904(stages, stride) \
  launch_optimized_904_hgemm<half, (stages), true>( \
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  int swizzle_stride = 904;
  LAUNCH_OPTIMIZED_904(5, swizzle_stride);
}