...
struct HgemmSafe4StageAuto {
  ...
  static constexpr int AlignmentA = 64 / sizeof(ElementA);
  static constexpr int AlignmentB = 64 / sizeof(ElementB);
  static constexpr int AlignmentC = 64 / sizeof(ElementC);
  static constexpr int AlignmentD = 64 / sizeof(ElementD);
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename HgemmSafe4StageAuto::Gemm;
  using StrideA = typename HgemmSafe4StageAuto::StrideA;
  using StrideB = typename HgemmSafe4StageAuto::StrideB;
  using StrideC = typename HgemmSafe4StageAuto::StrideC;
  using StrideD = typename HgemmSafe4StageAuto::StrideD;
  using ElementA = typename HgemmSafe4StageAuto::ElementA;
  using ElementB = typename HgemmSafe4StageAuto::ElementB;
  using ElementC = typename HgemmSafe4StageAuto::ElementC;
  using ElementCompute = typename HgemmSafe4StageAuto::ElementCompute;
...
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported - requires H100 or newer GPU");
#endif
}