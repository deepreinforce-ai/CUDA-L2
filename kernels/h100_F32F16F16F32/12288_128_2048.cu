...
#pragma unroll
  ...
  {
    float* src_ptr = reinterpret_cast<float*>(tCrD.data());
    __half2* dst_ptr = reinterpret_cast<__half2*>(tCrD_half.data());
    constexpr int num_elements = decltype(size(tCrD))::value;
    constexpr int num_pairs = num_elements / 2;

#pragma unroll
    for (int i = 0; i < num_pairs; ++i) {
      float2 f2 = make_float2(src_ptr[2*i], src_ptr[2*i+1]);
      dst_ptr[i] = __float22half2_rn(f2);
    }

    if constexpr (num_elements % 2 == 1) {
      reinterpret_cast<T*>(tCrD_half.data())[num_elements-1] =
          __float2half_rn(src_ptr[num_elements-1]);
    }
  }
