...
void gemm_pipeline_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                ...
                                Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageWorkspace<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  if (k_tile_count > -K_PIPE_MAX) {
    int read_pipe_0 = read_state.index();
    
    ProducerBarType::wait(&producer_mbar[read_pipe_0], read_state.phase());
    
    warpgroup_arrive();
    gemm(mma, tCrA(_,_,_,read_pipe_0), tCrB(_,_,_,read_pipe_0), tCrC);
    warpgroup_commit_batch();
    
    ConsumerBarType::arrive(&consumer_mbar[read_pipe_0]);
  ...
  if (beta == Beta(0.0f)) {
    if (alpha == Alpha(1.0f)) {
      ...
      for (int i = 0; i < size(tCrC); ++i) {
        tCgC(i) = static_cast<TC>(alpha * tCrC(i));
      }
    }
  } else {
    ...
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageWorkspace<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_pipeline_kernel<decltype(prob_shape), decltype(cta_tiler),
  ...
  if (status != cutlass::Status::kSuccess) {
    printf("Pipeline kernel launch failed\n");
  }
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_pipeline<cute::half_t, cute::half_t, cute::half_t>(
