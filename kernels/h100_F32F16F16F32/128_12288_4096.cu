...
hgemm_pipelined_kernel(T *__restrict__ Aptr, T *__restrict__ Bptr,
  ...
  auto tCrA_curr = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_curr = thr_mma.partition_fragment_B(gB(_, _, 0));
  auto tCrA_next = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_next = thr_mma.partition_fragment_B(gB(_, _, 0));
  ...
  auto tCrA_curr_view = s2r_thr_copy_a.retile_D(tCrA_curr);
  auto tCrA_next_view = s2r_thr_copy_a.retile_D(tCrA_next);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_curr_view = s2r_thr_copy_b.retile_D(tCrB_curr);
  auto tCrB_next_view = s2r_thr_copy_b.retile_D(tCrB_next);
  ...
  for (int ik = 0; ik < 1; ++ik) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, ismem_read), tCrA_curr_view(_, _, 0));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, ismem_read), tCrB_curr_view(_, _, 0));
  }
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if ((ik & 1) == 0) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_next_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_next_view(_, _, ik_next));
      } else {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_curr_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_curr_view(_, _, ik_next));
      }
      ...
      if ((ik & 1) == 0) {
        cute::gemm(tiled_mma, tCrD, tCrA_curr(_, _, ik), tCrB_curr(_, _, ik), tCrD);
      } else {
        cute::gemm(tiled_mma, tCrD, tCrA_next(_, _, ik), tCrB_next(_, _, ik), tCrD);
      }
    }
  }

  auto tCrD_half = make_tensor_like<T>(tCrD);
  const int total_elems = size(tCrD);
  #pragma unroll
  for (int i = 0; i < total_elems; i += 2) {
    tCrD_half(i)     = __float2half(tCrD(i));
    tCrD_half(i + 1) = __float2half(tCrD(i + 1));
  }
...
void launch_hgemm_pipelined(T *a, T *b, T *c, int M, int N, int K,
                           int swizzle_stride) {
  using namespace cute;
  ...
  constexpr auto KStage = Int<Stages>{};
  constexpr auto kSmemLayoutCBatch = Int<4>{};
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C smem reuse: A pipe stage must fit C tile");
  ...
  static constexpr int kShmSize = cute::max(shm_size_AB, shm_size_C);

  cudaFuncSetAttribute(
      hgemm_pipelined_kernel<
...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC, const bool BlockSwizzle>
__global__ void __launch_bounds__(128, 2)
hgemm_standard_kernel(T *__restrict__ Aptr, T *__restrict__ Bptr,
                    T *__restrict__ Dptr, int m, int n, int k) {
  using namespace cute;
  ...
  auto tCrA_alt  = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_alt  = thr_mma.partition_fragment_B(gB(_, _, 0));
  auto tCrD      = thr_mma.partition_fragment_C(gD);
  clear(tCrD);
  ...
  auto tCrA_view_alt     = s2r_thr_copy_a.retile_D(tCrA_alt);

  auto s2r_tiled_copy_b  = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b    = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB              = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view         = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_view_alt     = s2r_thr_copy_b.retile_D(tCrB_alt);
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (use_prime) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_view_alt(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_view_alt(_, _, ik_next));
      } else {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
      ...
      if (use_prime) {
        ...
        cute::gemm(tiled_mma, tCrD, tCrA_alt(_, _, ik), tCrB_alt(_, _, ik), tCrD);
      }
    }
  }

  auto tCrD_half = make_tensor_like<T>(tCrD);
  const int total = size(tCrD);
  #pragma unroll
...
template <typename T, const int Stages = 7, const bool BlockSwizzle = true>
void launch_hgemm_standard(T *a, T *b, T *c, int M, int N, int K,
                          int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C smem reuse: A pipe must be >= C tile");
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  constexpr int swizzle_stride = 4096;

  launch_hgemm_pipelined<half, 4, true>(
      reinterpret_cast<half *>(a.data_ptr()),
      reinterpret_cast<half *>(b_col_major.data_ptr()),
      reinterpret_cast<half *>(c.data_ptr()),
      M, N, K, swizzle_stride);
}