...
hgemm_ultra_optimized_persistent_kernel(T *Aptr, T *Bptr, T *Dptr, 
  ...
  for (int tile_idx = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * gridDim.x * gridDim.y; 
       tile_idx < num_tiles; 
       tile_idx += gridDim.x * gridDim.y * gridDim.z) {
    
    int tiles_per_row = (n + BN - 1) / BN;
    int iy = tile_idx / tiles_per_row;
    int ix = tile_idx % tiles_per_row;
    ...
    for (int i = 0; i < num_pairs; ++i) {
      ...
      __half2 h2 = __floats2half2_rn(tCrD(idx0), tCrD(idx1));
      tCrD_half(idx0) = __low2half(h2);
      tCrD_half(idx1) = __high2half(h2);
    }
    
    if (num_elements % 2 == 1) {
      tCrD_half(num_elements - 1) = __float2half(tCrD(num_elements - 1));
    }
    ...
    int num_iters = size<1>(tCrC_r2sx);
    
    #pragma unroll
    for (int i = 0; i < num_iters; i += step) {
      #pragma unroll
...
void launch_hgemm_ultra_optimized_persistent(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  ...
  cudaFuncSetAttribute(
      hgemm_ultra_optimized_persistent_kernel<
  ...
  hgemm_ultra_optimized_persistent_kernel<
      ...
      S2GCopyC, BlockSwizzle><<<grid, block, kShmSize>>>(a, b, c, M, N, K, num_tiles);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_ultra_optimized_persistent<half, 7, true>(
