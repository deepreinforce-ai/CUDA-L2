...
template <typename T, int BM, int BN, int BK, int kStage, int KTiles,
          typename TiledMMA,
...
#pragma unroll
        for (int ik = 0; ik < nk; ++ik) {
            ...
            if (ik == 0) {
                if (itile_to_read < KTiles) {
                    cute::copy(g2s_copy_a, tAgA_copy(_, _, _, itile_to_read), tAsA_copy(_, _, _, ismem_write));
                    cute::copy(g2s_copy_b, tBgB_copy(_, _, _, itile_to_read), tBsB_copy(_, _, _, ismem_write));
                    ++itile_to_read;
...
template <typename T, int Stages, int KTiles, bool BlockSwizzle>
void launch_hgemm(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride)
{
    using namespace cute;
    ...
    using SmemLayoutA = decltype(tile_to_shape(
        SmemLayoutAtom{}, make_shape(Int<BM>{}, Int<BK>{}, Int<Stages>{})));
    using SmemLayoutB = decltype(tile_to_shape(
        SmemLayoutAtom{}, make_shape(Int<BN>{}, Int<BK>{}, Int<Stages>{})));
    ...
    int bx_base = (N + BN - 1) / BN;
    int by      = (M + BM - 1) / BM;
    int bz      = BlockSwizzle ? (N + swizzle_stride - 1) / swizzle_stride : 1;
    int bx      = BlockSwizzle ? (bx_base + bz - 1) / bz : bx_base;
    ...
    static constexpr int kShmSize = cute::max(shm_AB, shm_C) * sizeof(T);

    cudaFuncSetAttribute(
        cuda_l2_h100_fp32_kernel<
            T, BM, BN, BK, Stages, KTiles, MMA,
            G2SCopyA, G2SCopyB,
    ...
    cuda_l2_h100_fp32_kernel<
        ...
        S2RCopyAtomA, S2RCopyAtomB,
        R2SCopyAtomC, S2GCopyAtomC, S2GCopyC,
        BlockSwizzle><<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}
