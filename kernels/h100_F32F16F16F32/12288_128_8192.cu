...
hgemm_corrected_kernel(const T *__restrict__ Aptr,
...
template <typename T>
void launch_hgemm_corrected(T *a, T *b, T *c, int M, int N, int K) {
  auto BM = Int<128>{};
  auto BN = Int<128>{};
  auto BK = Int<64>{};
  auto KStage = Int<5>{};
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory must fit in A's buffer");
  ...
  cudaFuncSetAttribute(
      hgemm_corrected_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
