...
#pragma unroll
  for (int j = 0; j < step; ++j) {
    if (base_idx + j < size<1>(tCrC_r2sx)) {
      cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, base_idx + j), tCsC_r2s(_, 0, 0, j));
    }
  }
  __syncthreads();

  for (int i = 0; i < num_iterations; ++i) {
    int curr_base = i * step;
    int next_base = (i + 1) * step;

#pragma unroll
    for (int j = 0; j < step; ++j) {
      if (curr_base + j < size<1>(tCgC_s2gx)) {
        cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), tCgC_s2gx(_, curr_base + j));
      }
    }
...
#pragma unroll
      for (int j = 0; j < step; ++j) {
        if (next_base + j < size<1>(tCrC_r2sx)) {
          cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, next_base + j), tCsC_r2s(_, 0, 0, j));
        }
      }
      __syncthreads();
    }
