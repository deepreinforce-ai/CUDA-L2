...
hgemm_optimized_kernel(
    ...
    auto s2r_copy_a = make_tiled_copy_A(
        Copy_Atom<SM75_U32x4_LDSM_N, T>{}, tiled_mma);
...
template<int BM, int BN, int BK, int kStage, int kSmemLayoutCBatch>
void launch_hgemm_optimized(
    ...
    auto kernel = hgemm_optimized_kernel<
    ...
    kernel<<<grid, block, kShmSize>>>(A, B_col, C, M, N, K);
}
...
void cuda_l2_h100_fp32(
    ...
    const __half* B_col_ptr = reinterpret_cast<const __half*>(b_col_major.data_ptr());
    __half*       C_ptr     = reinterpret_cast<__half*>(c.data_ptr());

    launch_hgemm_optimized<64, 128, 32, 3, 2>(A_ptr, B_col_ptr, C_ptr, M, N, K);
}