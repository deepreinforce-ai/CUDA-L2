...
hgemm_opt_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
void launch_config_64x64x32(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_opt_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
  ...
  hgemm_opt_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
                   SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA,
                   S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC,
                   S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}

template <typename T, const int Stages>
void launch_config_64x128x32(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
template <typename T, const int Stages>
void launch_config_64x64x64(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void launch_128x128x64(T *a, T *b, T *c, int M, int N, int K,
                       int swizzle_stride) {
  using namespace cute;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_config_64x64x64<half, 5>(a_ptr, b_col_ptr, c_ptr, M, N, K);
}