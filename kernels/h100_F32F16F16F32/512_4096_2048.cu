...
hgemm_optimized_streamk_kernel(const T * __restrict__ Aptr, 
                                const T * __restrict__ Bptr, 
                                T * __restrict__ Dptr, 
                                const int m, const int n, const int k,
                                const int num_tiles_m, const int num_tiles_n,
  ...
  const int tiles_per_block = (total_tiles + gridDim.x * gridDim.y - 1) / (gridDim.x * gridDim.y);
  const int block_linear_id = blockIdx.y * gridDim.x + blockIdx.x;
  const int tile_start = block_linear_id * tiles_per_block;
  ...
  for (int tile_idx = tile_start; tile_idx < tile_end; ++tile_idx) {
    const int iy = tile_idx % num_tiles_m;
    const int ix = tile_idx / num_tiles_m;
...
void launch_hgemm_optimized_streamk(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory exceeds A buffer");
  ...
  int num_blocks = min(264, total_tiles);
  int BX = min(32, num_tiles_n);
  int BY = (num_blocks + BX - 1) / BX;
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_streamk_kernel<
  ...
  hgemm_optimized_streamk_kernel<
      ...
      S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K, num_tiles_m, num_tiles_n, total_tiles);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_streamk<half, 6>(
