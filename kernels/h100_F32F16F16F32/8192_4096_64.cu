...
cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  int ntile = k / BK;
  constexpr int kPipelineStages = kStage;
...
#pragma unroll
  for (int itile = 0; itile < ntile; ++itile) {
    int smem_pipe_read = itile % kPipelineStages;
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      int ik_next = ik + 1;
      if (ik_next < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, smem_pipe_read),
                   tCrA_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, smem_pipe_read),
                   tCrB_view(_, _, ik_next));
      }

      if (ik == 0) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, smem_pipe_read),
                   tCrA_view(_, _, 0));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, smem_pipe_read),
                   tCrB_view(_, _, 0));
      }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  cudaFuncSetAttribute(
      cuda_l2_h100_fp32_kernel<
          T, BM, BN, BK, 2, MMA, G2SCopyA, G2SCopyB, SmemLayoutA,
  ...
  cuda_l2_h100_fp32_kernel<
      T, BM, BN, BK, 2, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB,
