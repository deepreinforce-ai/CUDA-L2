...
__global__ void __launch_bounds__(256, 1)
    hgemm_deep_pipeline_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik + 2 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 2, ismem_read), tCrA_view(_, _, ik + 2));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 2, ismem_read), tCrB_view(_, _, ik + 2));
      }
...
void launch_hgemm_deep_pipeline(T *a, T *b, T *c, int M, int N, int K) {
  ...
  cudaFuncSetAttribute(
      hgemm_deep_pipeline_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_deep_pipeline<half, 7>(
