...
hgemm_k_split_optimized_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k, int k_split) {
  ...
  int ix = blockIdx.x;
  int iy = blockIdx.y;
  int k_block_idx = blockIdx.z;
  
  int k_per_block = (k + k_split - 1) / k_split;
  int k_start = k_block_idx * k_per_block;
  ...
  Tensor D = make_tensor(make_gmem_ptr(Dptr), 
                         make_shape(k_split, m, n),
                         make_stride(m * n, n, Int<1>{}));

  Tensor gA = local_tile(A, make_tile(Int<BM>{}, Int<BK>{}),
                         make_coord(iy, _));
  Tensor gB = local_tile(B, make_tile(Int<BN>{}, Int<BK>{}),
                         make_coord(ix, _));
  Tensor gD = local_tile(D(k_block_idx, _, _), make_tile(Int<BM>{}, Int<BN>{}),
                         make_coord(iy, ix));
  ...
  int tile_k_start = k_start / BK;
  int tile_k_end = (k_end + BK - 1) / BK;
  int ntile = tile_k_end - tile_k_start;
...
#pragma unroll
  for (int istage = 0; istage < kStage - 1 && itile_to_read < ntile; ++istage) {
    int global_k_tile = tile_k_start + itile_to_read;
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, global_k_tile),
               tAsA_copy(_, _, _, ismem_write));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, global_k_tile),
...
template <typename T>
__global__ void reduce_k_split_warp_optimized(T *partial, T *output, int k_split, int m, int n) {
...
#pragma unroll 8
    for (int k = 0; k < k_split; ++k) {
      sum += __half2float(partial[k * total + base_offset]);
    }
...
template <typename T, const int Stages = 4, const int KSplit = 24>
void launch_hgemm_k_split_optimized(T *a, T *b, T *c, int M, int N, int K, T *intermediate_buffer) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_k_split_optimized_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
  ...
  hgemm_k_split_optimized_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
      ...
      <<<grid, block, kShmSize>>>(a, b, intermediate_buffer, M, N, K, KSplit);

  int total_elements = M * N;
  int reduce_threads = 256;
  int reduce_blocks = (total_elements + reduce_threads - 1) / reduce_threads;
  
  reduce_k_split_warp_optimized<<<reduce_blocks, reduce_threads>>>(
      intermediate_buffer, c, KSplit, M, N);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  constexpr int KSplit = 24;
  
  struct BufferManager {
    half *buffer;
    int size;
    
    BufferManager() : buffer(nullptr), size(0) {}
    ...
    half* get_buffer(int required_size) {
      if (size < required_size) {
        if (buffer) {
          cudaFree(buffer);
        }
        cudaMalloc(&buffer, required_size * sizeof(half));
        size = required_size;
      }
      return buffer;
    }
  ...
  int required_size = KSplit * M * N;
  half *intermediate_buffer = buffer_manager.get_buffer(required_size);

  launch_hgemm_k_split_optimized<half, 4, KSplit>(
