...
cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_dbuf = make_fragment_like(tCrA);
  auto tCrB_dbuf = make_fragment_like(tCrB);
  ...
  auto tCrA_dbuf_view = s2r_thr_copy_a.retile_D(tCrA_dbuf);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_dbuf_view = s2r_thr_copy_b.retile_D(tCrB_dbuf);
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if ((ik & 1) == 0) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_dbuf_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_dbuf_view(_, _, ik_next));
      } else {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
      ...
      if ((ik & 1) == 0) {
        ...
        cute::gemm(tiled_mma, tCrD, tCrA_dbuf(_, _, ik), tCrB_dbuf(_, _, ik), tCrD);
      }
    }
  }
