...
template <class ElementA,
          ...
          class SmemLayoutA,
          class SmemLayoutB>
struct SharedStorageSMSaturated
{
...
void gemm_sm_saturated_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                              ...
                              Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageSMSaturated<TA, TB, SmemLayoutA, SmemLayoutB>;
...
void launch_hgemm_sm_saturated(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageSMSaturated<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_sm_saturated_kernel<decltype(prob_shape), decltype(cta_tiler),
  ...
  if (status != cutlass::Status::kSuccess) {
    printf("Error: SM-saturated kernel launch failed\n");
  }
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_sm_saturated<cute::half_t, cute::half_t, cute::half_t>(
