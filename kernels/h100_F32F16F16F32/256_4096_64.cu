...
__global__ void cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr,
                                                  T *Dptr, int m,
                                                  int n, int k) {
...
template <typename T, const int BM_val, const int BN_val, const int BK_val,
          const int Stages = 3, const bool BlockSwizzle = false>
void launch_hgemm_kernel(T *a, T *b, T *c, int M, int N, int K,
  ...
  using SmemLayoutAtom = decltype(composition(
      Swizzle<3, 3, 3>{}, make_layout(make_shape(Int<8>{}, Int<BK_val>{}),
                                      make_stride(Int<BK_val>{}, Int<1>{}))));
  using SmemLayoutA = decltype(tile_to_shape(
      SmemLayoutAtom{}, make_shape(Int<BM_val>{}, Int<BK_val>{}, Int<Stages>{})));
  using SmemLayoutB = decltype(tile_to_shape(
      SmemLayoutAtom{}, make_shape(Int<BN_val>{}, Int<BK_val>{}, Int<Stages>{})));
  ...
  cudaFuncSetAttribute(
      cuda_l2_h100_fp32_kernel<
          T, BM_val, BN_val, BK_val, Stages, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_kernel<half, 128, 128, 32, 3, true>(
