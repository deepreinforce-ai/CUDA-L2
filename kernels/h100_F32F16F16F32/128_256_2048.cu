...
__global__ void __launch_bounds__(128, 2)
    safe_enhanced_wavefront_kernel(const T *__restrict__ Aptr,
...
template <typename T>
void launch_safe_enhanced_wavefront(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      safe_enhanced_wavefront_kernel<T, 32, 32, 128, 8, MMA,
                                      G2SCopyA, G2SCopyB, SmemLayoutA,
                                      SmemLayoutB, SmemLayoutC,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_safe_enhanced_wavefront<half>(
