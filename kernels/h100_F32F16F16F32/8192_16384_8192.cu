...
struct OptimizedFP16HgemmConfig {
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Config = OptimizedFP16HgemmConfig;
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS GEMM kernel cannot implement the given configuration");
  }

  status = gemm.initialize(arguments, workspace.get());
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("Failed to initialize CUTLASS GEMM kernel");
  }
...
#else
  throw std::runtime_error("This kernel requires H100 GPU (SM90 architecture)");
#endif
}