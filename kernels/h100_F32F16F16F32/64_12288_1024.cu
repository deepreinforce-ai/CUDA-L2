...
__device__ __forceinline__ int swizzle_block_idx(int bx, int by, int grid_x, int swizzle_width) {
    int group = bx / swizzle_width;
    int local = bx % swizzle_width;
    return group * swizzle_width + local;
}
...
hgemm_swizzle_8stage_kernel(T* Aptr, T* Bptr, T* Dptr, int m, int n, int k) {
    using namespace cute;
    ...
    cp_async_wait(cute::Int<kStage - 2>{});
    __syncthreads();
...
template<typename T>
void launch_hgemm_swizzle_8stage(T* a, T* b_col_major, T* c, int M, int N, int K) {
    using namespace cute;
    ...
    static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                  size(SmemLayoutC{}),
                  "C smem must fit in A's one pipe stage");
...
void cuda_l2_h100_fp32(
    ...
    launch_hgemm_swizzle_8stage<half>(a_ptr, b_col_ptr, c_ptr, M, N, K);
}