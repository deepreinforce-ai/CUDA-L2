...
hgemm_persistent_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k, 
                       int total_tiles_n) {
  ...
  int tiles_per_block = (total_tiles_n + gridDim.x - 1) / gridDim.x;
  int block_tile_start = blockIdx.x * tiles_per_block;
  int block_tile_end = min(block_tile_start + tiles_per_block, total_tiles_n);
  ...
  for (int tile_n_idx = block_tile_start; tile_n_idx < block_tile_end; ++tile_n_idx) {
    int ix = tile_n_idx;
...
#pragma unroll
    ...
    if (tile_n_idx < block_tile_end - 1) {
      __syncthreads();
    }
  }
}
...
void launch_hgemm_persistent_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_persistent_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
  ...
  hgemm_persistent_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
      ...
      <<<grid, block, shm_size>>>(a, b, c, M, N, K, total_tiles_n);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_persistent_cute<half, 6>(
