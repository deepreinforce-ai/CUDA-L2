...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  using Gemm = typename HgemmSingleCTAKernel::Gemm;
  using StrideA = typename HgemmSingleCTAKernel::StrideA;
  using StrideB = typename HgemmSingleCTAKernel::StrideB;
  using StrideC = typename HgemmSingleCTAKernel::StrideC;
  using StrideD = typename HgemmSingleCTAKernel::StrideD;
  using ElementA = typename HgemmSingleCTAKernel::ElementA;
  using ElementB = typename HgemmSingleCTAKernel::ElementB;
  using ElementC = typename HgemmSingleCTAKernel::ElementC;
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS GEMM cannot implement this problem");
  }
