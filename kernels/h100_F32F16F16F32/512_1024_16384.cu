...
__device__ __forceinline__ void barrier_arrive_tx_inline(uint64_t* barrier, uint32_t tx_bytes)
{
  uint32_t smem_addr;
  asm volatile(
    "{\n\t"
...
__device__ __forceinline__ void barrier_wait_parity_inline(uint64_t* barrier, uint32_t phase)
{
  uint32_t smem_addr;
  asm volatile(
    "{\n\t"
...
struct SharedStorageExtreme16x
{
...
void gemm_wgmma_16x_unroll_kernel(
    ...
    Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageExtreme16x<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  constexpr int K_PIPE_MAX = decltype(size<2>(SmemLayoutA{}))::value;
  ...
  for (int pipe = 0; pipe < K_PIPE_MAX; ++pipe)
  {
    if ((warp_idx == 0) && lane_predicate)
    {
      barrier_arrive_tx_inline(&producer_mbar[pipe], tma_transaction_bytes);
  ...
  Tensor tCrC_ping = thr_mma.make_fragment_C(tCgC);
  Tensor tCrC_pong = thr_mma.make_fragment_C(tCgC);
  clear(tCrC_ping);
  clear(tCrC_pong);
  ...
  const int total_iters = k_tile_count + K_PIPE_MAX;
  const int main_loop_iters = (total_iters / UNROLL_FACTOR) * UNROLL_FACTOR;
  
  int iter = 0;
  
  CUTE_NO_UNROLL
  while (iter < main_loop_iters)
  {
    #pragma unroll
    for (int u = 0; u < UNROLL_FACTOR; ++u)
    {
      const int read_pipe = read_state.index();
      
      barrier_wait_parity_inline(&producer_mbar[read_pipe], read_state.phase());

      const bool use_ping = (u % 2 == 0);
      
      warpgroup_arrive();
      if (use_ping) {
        gemm(mma, tCrA(_,_,_,read_pipe), tCrB(_,_,_,read_pipe), tCrC_ping);
      } else {
        gemm(mma, tCrA(_,_,_,read_pipe), tCrB(_,_,_,read_pipe), tCrC_pong);
      }
  ...
  #pragma unroll
  for (int i = 0; i < size(tCrC_ping); ++i) {
    tCrC_ping(i) = tCrC_ping(i) + tCrC_pong(i);
  }

  axpby(alpha, tCrC_ping, beta, tCgC);
}

template <class TA, class TB, class TC>
void launch_hgemm_16x_unroll(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  TiledMMA tiled_mma = make_tiled_mma(SM90_64x128x16_F32F16F16_SS<GMMA::Major::K,GMMA::Major::K>{});
  ...
  using SharedStorage = SharedStorageExtreme16x<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_wgmma_16x_unroll_kernel<
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_16x_unroll<cute::half_t, cute::half_t, cute::half_t>(
