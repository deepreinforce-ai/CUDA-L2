...
struct SharedStorageAsyncEpilogue
{
  ...
  alignas(128) uint64_t epilogue_barrier;
};

template<typename T>
__device__ __forceinline__ void async_store_128bit(half* dst, const T& src_val) {
  uint4 data;
  const half* src = reinterpret_cast<const half*>(&src_val);
  data.x = *reinterpret_cast<const uint32_t*>(&src[0]);
  data.y = *reinterpret_cast<const uint32_t*>(&src[2]);
  data.z = *reinterpret_cast<const uint32_t*>(&src[4]);
  data.w = *reinterpret_cast<const uint32_t*>(&src[6]);
    ...
    Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageAsyncEpilogue<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  if ((warp_idx == 0) && lane_predicate) {
    cutlass::arch::ClusterBarrier::init(&smem.epilogue_barrier, 128);
  }
    ...
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageAsyncEpilogue<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_async_epilogue_kernel<
  ...
  if (result != cudaSuccess || status != cutlass::Status::kSuccess) {
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_async_epilogue_hgemm<cute::half_t, cute::half_t, cute::half_t>(
