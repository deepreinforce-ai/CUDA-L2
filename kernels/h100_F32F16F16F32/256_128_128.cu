...
cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  constexpr int total_elems = size(tCrD);
  float* src_f = reinterpret_cast<float*>(tCrD.data());
  __half2* dst_h2 = reinterpret_cast<__half2*>(tCrD_half.data());
  half* dst_h = reinterpret_cast<half*>(tCrD_half.data());
  
  constexpr int vec8_iters = total_elems / 8;
  constexpr int vec4_iters = (total_elems % 8) / 4;
  constexpr int vec2_iters = (total_elems % 4) / 2;
  constexpr int scalar_rem = total_elems % 2;
  
  #pragma unroll
  for (int i = 0; i < vec8_iters; ++i) {
    int base = i * 8;
    float f0 = src_f[base + 0];
    float f1 = src_f[base + 1];
    float f2 = src_f[base + 2];
    float f3 = src_f[base + 3];
    float f4 = src_f[base + 4];
    float f5 = src_f[base + 5];
    float f6 = src_f[base + 6];
    float f7 = src_f[base + 7];
    
    __half2 h0 = __float22half2_rn(make_float2(f0, f1));
    __half2 h1 = __float22half2_rn(make_float2(f2, f3));
    __half2 h2 = __float22half2_rn(make_float2(f4, f5));
    __half2 h3 = __float22half2_rn(make_float2(f6, f7));
  ...
  int base4 = vec8_iters * 8;
  #pragma unroll
  for (int i = 0; i < vec4_iters; ++i) {
    int offset = base4 + i * 4;
    __half2 h0 = __float22half2_rn(make_float2(src_f[offset + 0], src_f[offset + 1]));
    __half2 h1 = __float22half2_rn(make_float2(src_f[offset + 2], src_f[offset + 3]));
    dst_h2[(offset) / 2 + 0] = h0;
    dst_h2[(offset) / 2 + 1] = h1;
  }
  
  int base2 = base4 + vec4_iters * 4;
  #pragma unroll
  for (int i = 0; i < vec2_iters; ++i) {
    int offset = base2 + i * 2;
    dst_h2[offset / 2] = __float22half2_rn(make_float2(src_f[offset], src_f[offset + 1]));
  }
  
  if (scalar_rem > 0) {
    dst_h[total_elems - 1] = __float2half_rn(src_f[total_elems - 1]);
  }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                "C shared memory exceeds A's allocation");
