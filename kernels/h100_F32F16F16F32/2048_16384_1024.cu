...
using HgemmBreakthrough = HgemmKernelBreakthrough<
...
void run_gemm_breakthrough(torch::Tensor a, torch::Tensor b_col_major, torch::Tensor c, 
                           int M, int N, int K) {
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  run_gemm_breakthrough<HgemmBreakthrough>(a, b_col_major, c, M, N, K);
#else
