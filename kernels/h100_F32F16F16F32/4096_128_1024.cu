...
template <typename T, int Stages, bool BlockSwizzle>
void launch_hgemm_v4_64x64x64(T *a, T *b, T *c, int M, int N, int K,
                               int swizzle_stride)
{
    auto BM     = Int<64>{};
    auto BN     = Int<64>{};
    ...
    static constexpr int kShmSize =
        cute::max(cute::cosize(SmemLayoutA{}) + cute::cosize(SmemLayoutB{}),
                  cute::cosize(SmemLayoutC{})) * sizeof(T);

    cudaFuncSetAttribute(
        hgemm_optimized_v4_kernel<T, BM, BN, BK, Stages, MMA,
                                G2SCopyA, G2SCopyB,
                                ...
                                torch::Tensor b_col_major, torch::Tensor c)
{
    ...
    half *bcm_ptr = reinterpret_cast<half *>(b_col_major.data_ptr());
    half *c_ptr   = reinterpret_cast<half *>(c.data_ptr());

    launch_hgemm_v4_64x64x64<half, 5, true>(
        a_ptr, bcm_ptr, c_ptr, M, N, K, 768
    );
}