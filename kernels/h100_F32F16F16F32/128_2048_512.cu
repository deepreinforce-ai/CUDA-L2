...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 2) hgemm_hybrid_bk64_5stage_kernel(
  ...
  auto tCrA_ping = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_ping = thr_mma.partition_fragment_B(gB(_, _, 0));
  auto tCrA_pong = make_fragment_like(tCrA_ping);
  auto tCrB_pong = make_fragment_like(tCrB_ping);
...
void launch_hgemm_hybrid_bk64(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory exceeds A pipeline stage");
  ...
  cudaFuncSetAttribute(
      hgemm_hybrid_bk64_5stage_kernel<
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_hybrid_bk64<half, 5>(
