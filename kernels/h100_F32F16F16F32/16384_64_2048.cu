...
hgemm_3level_pipelined_persistent(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k, int num_m_tiles) {
  ...
  int block_idx_base = blockIdx.y + blockIdx.z * gridDim.y;
  
  for (int block_m_idx = block_idx_base; block_m_idx < num_m_tiles; block_m_idx += gridDim.y * gridDim.z) {
    int iy = block_m_idx;
...
template <typename T>
void launch_hgemm_3level_pipeline(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_3level_pipelined_persistent<
  ...
  hgemm_3level_pipelined_persistent<
      ...
      S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K, num_m_tiles);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_3level_pipeline<half>(
