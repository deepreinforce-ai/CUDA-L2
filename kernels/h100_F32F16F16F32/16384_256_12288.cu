...
void gemm_dual_wavefront_kernel(ProblemShape shape_MNK, CtaTiler cta_tiler,
                                ...
                                Alpha alpha, Beta beta)
{
  ...
  int prefetched = min(k_tile_count, K_PIPE_MAX);
  k_tile_count -= prefetched;
  k_tile = prefetched;
  ...
  for (int i = 0; i < prefetched; ++i) ++write_state;

  int main_loop_iters = k_tile_count / 4;
  
  for (int iter = 0; iter < main_loop_iters; ++iter)
  {
    {
  ...
  k_tile_count -= main_loop_iters * 4;
  ...
  CUTE_UNROLL
  for (int drain = 0; drain < prefetched; ++drain)
  {
    int read_pipe = read_state.index();
    ProducerBarType::wait(&producer_mbar[read_pipe], read_state.phase());
...
void launch_hgemm_dual_wavefront(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  auto kernel_ptr = &gemm_dual_wavefront_kernel<decltype(prob_shape), decltype(cta_tiler),
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_dual_wavefront<cute::half_t, cute::half_t, cute::half_t>(
