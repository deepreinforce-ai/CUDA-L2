...
#pragma unroll
  ...
  constexpr int kNumElems = decltype(size(tCrD))::value;
  float* src_ptr = reinterpret_cast<float*>(&tCrD(0));
  half* dst_ptr = reinterpret_cast<half*>(&tCrD_half(0));
  
#pragma unroll
  for (int i = 0; i < kNumElems / 2; ++i) {
    float2 f2 = make_float2(src_ptr[i*2], src_ptr[i*2 + 1]);
    half2 h2 = __float22half2_rn(f2);
    reinterpret_cast<half2*>(dst_ptr)[i] = h2;
  }
  
  if (kNumElems % 2 != 0) {
    dst_ptr[kNumElems - 1] = __float2half_rn(src_ptr[kNumElems - 1]);
  }
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  LAUNCH_HGEMM_MMA_STAGES_CUTE_SWIZZLE_TN(5, swizzle_stride);
}