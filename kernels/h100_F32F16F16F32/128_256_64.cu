...
__global__ __launch_bounds__(128, 2) void cuda_l2_h100_fp32_kernel(
...
#pragma unroll
  ...
  constexpr int kEpilogueBatches = 2;
  
#pragma unroll
  for (int batch_pair = 0; batch_pair < (num_batches + 1) / 2; ++batch_pair) {
    int batch_idx = batch_pair * 2;
...
#pragma unroll
      for (int j = 0; j < step; ++j) {
        if (batch_idx * step + j < size<1>(tCrC_r2sx)) {
          auto src_frag = tCrC_r2sx(_, batch_idx * step + j);
          auto dst_smem = tCsC_r2s(_, 0, 0, j);
          
          constexpr int kFragSize = decltype(size(src_frag))::value;
          
#pragma unroll
          for (int i = 0; i < kFragSize; i += 2) {
            float val0 = src_frag(i);
            float val1 = (i + 1 < kFragSize) ? src_frag(i + 1) : 0.0f;
            
            __half2 h2 = __floats2half2_rn(val0, val1);
            
            if (i + 1 < kFragSize) {
              *reinterpret_cast<__half2*>(&dst_smem(i)) = h2;
            } else {
              dst_smem(i) = __low2half(h2);
            }
          }
        }
      }
...
#pragma unroll
      for (int j = 0; j < step; ++j) {
        if ((batch_idx + 1) * step + j < size<1>(tCrC_r2sx)) {
          auto src_frag = tCrC_r2sx(_, (batch_idx + 1) * step + j);
          auto dst_smem = tCsC_r2s(_, 0, 0, j);
...
#pragma unroll
      for (int j = 0; j < step; ++j) {
        if ((batch_idx + 1) * step + j < size<1>(tCrC_r2sx)) {
          cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), 
                     tCgC_s2gx(_, (batch_idx + 1) * step + j));
        }
      }
    }
  }
