...
cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr,
                                  int m, int n, int k) {
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  using g2s_copy_atom_a = Copy_Atom<Copy_Traits<g2s_copy_op_a>, T>;
  using g2s_copy_atom_b = Copy_Atom<Copy_Traits<g2s_copy_op_b>, T>;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C smem exceeds A single-stage buffer");
