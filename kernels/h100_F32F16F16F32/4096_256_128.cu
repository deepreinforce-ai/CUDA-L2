...
__global__ void __launch_bounds__(128, 2) hgemm_optimized_asymmetric_fp32_kernel(
...
void launch_hgemm_optimized_asymmetric_fp32_tn_cute(T *a, T *b, T *c, int M, int N, int K,
                                                    int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "SmemLayoutC size exceeds SmemLayoutA pipe size");
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_asymmetric_fp32_kernel<
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_asymmetric_fp32_tn_cute<half, stages, swizzle>(
