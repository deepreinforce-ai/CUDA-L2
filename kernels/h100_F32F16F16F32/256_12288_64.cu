...
__device__ __forceinline__ void cvt_f32x2_to_f16x2(
    const float f0, const float f1, __half& h0, __half& h1) {
  uint32_t packed;
  asm volatile("cvt.rn.f16x2.f32 %0, %1, %2;" 
               : "=r"(packed) : "f"(f1), "f"(f0));
  h0 = reinterpret_cast<__half*>(&packed)[0];
  h1 = reinterpret_cast<__half*>(&packed)[1];
}
...
hgemm_optimized_ultimate_v2_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
#pragma unroll
  for (int stage = 0; stage < kStage && stage < ntile; ++stage) {
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, stage),
               tAsA_copy(_, _, _, stage));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, stage),
               tBsB_copy(_, _, _, stage));
    cp_async_fence();
  }
...
#pragma unroll
  ...
  const int num_elements = size(tCrD);
  
#pragma unroll
  for (int i = 0; i < num_elements; i += 2) {
    if (i + 1 < num_elements) {
      cvt_f32x2_to_f16x2(
          static_cast<float>(tCrD(i)),
          static_cast<float>(tCrD(i + 1)),
          tCrD_half(i),
          tCrD_half(i + 1)
      );
    } else {
      tCrD_half(i) = __float2half(static_cast<float>(tCrD(i)));
    }
  }
...
template <typename T>
void launch_hgemm_optimized_ultimate_v2(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_ultimate_v2_kernel<T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_ultimate_v2<half>(
