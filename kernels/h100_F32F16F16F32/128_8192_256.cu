...
__global__ void __launch_bounds__(256)
    hgemm_safe_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto A = cute::make_tensor(cute::make_gmem_ptr(Aptr),
                             cute::make_shape(m, k),
                             cute::make_stride(k, Int<1>{}));
  auto B = cute::make_tensor(cute::make_gmem_ptr(Bptr),
                             cute::make_shape(n, k),
                             cute::make_stride(k, Int<1>{}));
  auto D = cute::make_tensor(cute::make_gmem_ptr(Dptr),
                             cute::make_shape(m, n),
                             cute::make_stride(n, Int<1>{}));

  auto gA = cute::local_tile(A, cute::make_tile(Int<BM>{}, Int<BK>{}),
                             cute::make_coord(iy, _));
  auto gB = cute::local_tile(B, cute::make_tile(Int<BN>{}, Int<BK>{}),
                             cute::make_coord(ix, _));
  auto gD = cute::local_tile(D, cute::make_tile(Int<BM>{}, Int<BN>{}),
                             cute::make_coord(iy, ix));
  ...
  auto s2r_tiled_copy_b = cute::make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  ...
  auto tCrD_half = cute::make_tensor_like<T>(tCrD);
  cute::copy(tCrD, tCrD_half);

  auto sC = cute::make_tensor(sA(_, _, ismem_read).data(), SmemLayoutC{});

  auto r2s_tiled_copy_c = cute::make_tiled_copy_C(R2SCopyAtomC{}, tiled_mma);
  ...
  auto tCgC_s2gx = cute::group_modes<1, 3>(tCgC_s2g);
  auto tCrC_r2sx = cute::group_modes<1, 3>(tCrC_r2s);

  int step = cute::size<3>(tCsC_r2s);
  #pragma unroll
  for (int i = 0; i < cute::size<1>(tCrC_r2sx); i += step) {
    #pragma unroll
...
template <typename T>
void launch_hgemm_safe(T *a, T *b, T *c, int M, int N, int K) {
  auto BM = Int<64>{};
  auto BN = Int<256>{};
  auto BK = Int<64>{};
  auto KStage = Int<4>{};
  ...
  static constexpr int kMmaPM =
      1 * kMmaEURepeatM * cute::get<0>(mma_atom_shape{});
  static constexpr int kMmaPN =
      2 * kMmaEURepeatN * cute::get<1>(mma_atom_shape{});
  static constexpr int kMmaPK =
      1 * kMmaEURepeatK * cute::get<2>(mma_atom_shape{});
  ...
  using g2s_copy_op_a     = SM80_CP_ASYNC_CACHEALWAYS<cute::uint128_t>;
  using g2s_copy_traits_a = Copy_Traits<g2s_copy_op_a>;
  using g2s_copy_atom_a   = Copy_Atom<g2s_copy_traits_a, T>;
  ...
  using g2s_copy_op_b     = SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>;
  using g2s_copy_traits_b = Copy_Traits<g2s_copy_op_b>;
  using g2s_copy_atom_b   = Copy_Atom<g2s_copy_traits_b, T>;
  ...
  cudaFuncSetAttribute(
      hgemm_safe_kernel<T, BM, BN, BK, 4, MMA,
                       G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  optimized_gemm_safe::launch_hgemm_safe<half>(
