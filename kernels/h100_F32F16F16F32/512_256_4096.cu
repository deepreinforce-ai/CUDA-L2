    ...
    Alpha alpha, Beta beta)
{
  ...
  using SharedStorageType = SharedStorage<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  constexpr int K_PIPE_MAX = cute::size<2>(SmemLayoutA{});
  constexpr int tma_transaction_bytes = 
      (cute::cosize_v<SmemLayoutA> * sizeof(TA) + 
       cute::cosize_v<SmemLayoutB> * sizeof(TB)) / K_PIPE_MAX;
  ...
  int tiles_remaining = k_tile_count;
    ...
    int M, int N, int K)
{
  ...
  using SharedStorageType = SharedStorage<TA, TB, decltype(sA), decltype(sB)>;
  ...
  dim3 dimGrid(
    round_up((N + bN - 1) / bN, dimCluster.x),
    round_up((M + bM - 1) / bM, dimCluster.y)
  );
  
  cutlass::ClusterLaunchParams params = {dimGrid, dimBlock, dimCluster, smem_size};
  
  auto kernel_ptr = &gemm_wgmma_kernel<
                                 ...
                                 torch::Tensor b_col_major, torch::Tensor c)
{
  ...
  launch_hgemm<cute::half_t, cute::half_t, cute::half_t>(
