...
#pragma unroll
  ...
  constexpr int TILES_PER_GROUP = 4;
  int num_groups = ntile / TILES_PER_GROUP;

#pragma unroll 1
  for (int igroup = 0; igroup < num_groups; ++igroup) {
#pragma unroll
    for (int itile_in_group = 0; itile_in_group < TILES_PER_GROUP; ++itile_in_group) {
      int itile = igroup * TILES_PER_GROUP + itile_in_group;
