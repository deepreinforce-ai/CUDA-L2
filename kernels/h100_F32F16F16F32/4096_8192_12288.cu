...
using ElementEpilogueAccumulator = float;
...
using StageCountType = cutlass::gemm::collective::StageCountAutoCarveout<
    static_cast<int>(sizeof(typename cutlass::half_t))
>;
...
using CollectiveEpilogue = typename cutlass::epilogue::collective::CollectiveBuilder<
    ...
    ElementEpilogueAccumulator, ElementEpilogueAccumulator,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  ElementEpilogueAccumulator alpha = ElementEpilogueAccumulator(1.0f);
  ElementEpilogueAccumulator beta = ElementEpilogueAccumulator(0.0f);
  ...
  if (cuda_status != cudaSuccess) {
    ...
    err_msg += " (M=" + std::to_string(M) + ", N=" + std::to_string(N) + ", K=" + std::to_string(K) + ")";
    throw std::runtime_error(err_msg);
  }
