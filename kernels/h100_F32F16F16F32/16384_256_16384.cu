...
cutlass::Status try_run_gemm(torch::Tensor a, torch::Tensor b_col_major, torch::Tensor c,
                              int M, int N, int K) {
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  status = try_run_gemm<HgemmVariantA>(a, b_col_major, c, M, N, K);
  if (status == cutlass::Status::kSuccess) return;

  status = try_run_gemm<HgemmVariantB>(a, b_col_major, c, M, N, K);
  if (status == cutlass::Status::kSuccess) return;

  status = try_run_gemm<HgemmVariantC>(a, b_col_major, c, M, N, K);
  if (status == cutlass::Status::kSuccess) return;

  status = try_run_gemm<HgemmVariantD>(a, b_col_major, c, M, N, K);
  if (status == cutlass::Status::kSuccess) return;

  status = try_run_gemm<HgemmVariantE>(a, b_col_major, c, M, N, K);
  if (status == cutlass::Status::kSuccess) return;
