...
struct EnhancedCooperativeEpilogueHgemm {
...
struct Standard4StageBaselineHgemm {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
struct ConservativePingpongAutoHgemm {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  try {
    run_gemm<EnhancedCooperativeEpilogueHgemm>(a, b_col_major, c, M, N, K);
  } catch (const std::exception& e1) {
    try {
      run_gemm<Standard4StageBaselineHgemm>(a, b_col_major, c, M, N, K);
    } catch (const std::exception& e2) {
      run_gemm<ConservativePingpongAutoHgemm>(a, b_col_major, c, M, N, K);
    }
  }
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported");
