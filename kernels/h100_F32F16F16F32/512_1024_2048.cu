...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 2) cuda_l2_h100_fp32_kernel(
...
#pragma unroll
    ...
    if (itile < ntile - 1) {
      int next_ismem_read = ismem_read;
      
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, next_ismem_read), tCrA_view(_, _, 0));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, next_ismem_read), tCrB_view(_, _, 0));
      
      if (nk > 1) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, next_ismem_read), tCrA_view(_, _, 1));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, next_ismem_read), tCrB_view(_, _, 1));
      }
      
      if (nk > 2) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, 2, next_ismem_read), tCrA_view(_, _, 2));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, 2, next_ismem_read), tCrB_view(_, _, 2));
      }
    }
  }
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half, 4>(
