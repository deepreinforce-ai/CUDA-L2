...
__global__ void __launch_bounds__(128)
    hgemm_dual_warp_prefetch_kernel(T *Aptr, T *Bptr, T *Dptr,
                                    int m, int n, int k) {
  ...
  auto tCrA_set0 = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_set0 = thr_mma.partition_fragment_B(gB(_, _, 0));
  auto tCrA_set1 = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_set1 = thr_mma.partition_fragment_B(gB(_, _, 0));
  ...
  auto tCrA_view_set0 = s2r_thr_copy_a.retile_D(tCrA_set0);
  auto tCrA_view_set1 = s2r_thr_copy_a.retile_D(tCrA_set1);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view_set0 = s2r_thr_copy_b.retile_D(tCrB_set0);
  auto tCrB_view_set1 = s2r_thr_copy_b.retile_D(tCrB_set1);

  constexpr int nk = decltype(size<2>(tCrA_set0))::value;
  ...
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, 0), tCrA_view_set0(_, _, 0));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, 0), tCrB_view_set0(_, _, 0));
  
  cp_async_wait<0>();
  __syncthreads();
  
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, 1), tCrA_view_set1(_, _, 0));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, 1), tCrB_view_set1(_, _, 0));
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 1), tCrA_view_set1(_, _, 1));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 1), tCrB_view_set1(_, _, 1));
  
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 0), tCrA_view_set0(_, _, 1));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 0), tCrB_view_set0(_, _, 1));
  
  cute::gemm(tiled_mma, tCrD, tCrA_set0(_, _, 0), tCrB_set0(_, _, 0), tCrD);

  cute::gemm(tiled_mma, tCrD, tCrA_set0(_, _, 1), tCrB_set0(_, _, 1), tCrD);

#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    cute::gemm(tiled_mma, tCrD, tCrA_set1(_, _, ik), tCrB_set1(_, _, ik), tCrD);
  }
...
void launch_hgemm_dual_warp_prefetch(T *a, T *b, T *c, int M, int N, int K,
                                      int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory exceeds A's one pipe");
  ...
  cudaFuncSetAttribute(
      hgemm_dual_warp_prefetch_kernel<
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_dual_warp_prefetch<half, 3, true>(
