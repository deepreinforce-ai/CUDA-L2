...
struct MaxIndependenceHgemmKernel {
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename MaxIndependenceHgemmKernel::Gemm;
  using StrideA = typename MaxIndependenceHgemmKernel::StrideA;
  using StrideB = typename MaxIndependenceHgemmKernel::StrideB;
  using StrideC = typename MaxIndependenceHgemmKernel::StrideC;
  using StrideD = typename MaxIndependenceHgemmKernel::StrideD;
  using ElementA = typename MaxIndependenceHgemmKernel::ElementA;
  using ElementB = typename MaxIndependenceHgemmKernel::ElementB;
  using ElementC = typename MaxIndependenceHgemmKernel::ElementC;
...
#else
  throw std::runtime_error("This kernel requires CUDA architecture SM90 (H100) support");
#endif
}