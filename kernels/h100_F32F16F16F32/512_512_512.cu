...
cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  extern __shared__ char shm_data_raw[];
  
  T *Ashm = reinterpret_cast<T*>(shm_data_raw);
  T *Bshm = Ashm + cute::cosize(SmemLayoutA{});
  
  constexpr int output_buffer_size = BM * BN;
  T *output_buffer = Bshm + cute::cosize(SmemLayoutB{});
...
#pragma unroll
  ...
  constexpr int num_elem = decltype(size(tCrD))::value;
#pragma unroll
