...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik + 2 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next2, ismem_read), tCrA_view(_, _, ik_next2));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next2, ismem_read), tCrB_view(_, _, ik_next2));
      } else if (ik + 1 < nk) {
  ...
  int num_batches = (size<1>(tCrC_r2sx) + step - 1) / step;

  int base_idx = 0;
  int current_step = (size<1>(tCrC_r2sx) < step) ? size<1>(tCrC_r2sx) : step;
  
#pragma unroll
  for (int j = 0; j < current_step; ++j) {
    auto t = make_tensor_like<T>(tCrC_r2sx(_, base_idx + j));
    cute::copy(tCrC_r2sx(_, base_idx + j), t);
...
#pragma unroll
  for (int i = 0; i < num_batches; ++i) {
    base_idx = i * step;
    int remaining = size<1>(tCrC_r2sx) - base_idx;
    current_step = (remaining < step) ? remaining : step;

#pragma unroll
    for (int j = 0; j < current_step; ++j) {
      cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), tCgC_s2gx(_, base_idx + j));
    }

    if (i + 1 < num_batches) {
      __syncthreads();
      
      int next_base = (i + 1) * step;
      int next_remaining = size<1>(tCrC_r2sx) - next_base;
      int next_step = (next_remaining < step) ? next_remaining : step;
      
#pragma unroll
      for (int j = 0; j < next_step; ++j) {
        auto t = make_tensor_like<T>(tCrC_r2sx(_, next_base + j));
        cute::copy(tCrC_r2sx(_, next_base + j), t);
        cute::copy(r2s_tiled_copy_c, t, tCsC_r2s(_, 0, 0, j));
      }
      __syncthreads();
    }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  ...
  cudaFuncSetAttribute(
      cuda_l2_h100_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, 
