...
template <typename T, int BM, int BN, int BK,
          ...
          typename R2SCopyAtomC, typename S2GCopyAtomC,
          typename S2GCopyC, const bool BlockSwizzle>
__global__ __launch_bounds__(128, 4)
void hgemm_bk64_bn128_optimized_kernel(T *__restrict__ Aptr, T *__restrict__ Bptr,
...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC, const bool BlockSwizzle>
__global__ void hgemm_pipeline_kernel(T *Aptr, T *Bptr,
...
template <typename T, const bool BlockSwizzle = true>
void launch_hgemm_bk64_bn128(T *a, T *b, T *c, int M, int N, int K,
                               int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory request exceeds A's one pipe");
  ...
  cudaFuncSetAttribute(
      hgemm_bk64_bn128_optimized_kernel<T, BM, BN, BK, MMA,
...
void launch_hgemm_pipeline_bn160(T *a, T *b, T *c, int M, int N, int K,
                                  int swizzle_stride) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_pipeline_kernel<T, BM, BN, BK, KStage, MMA,
          G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC,
          S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC,
          BlockSwizzle>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  if (K == 64 && N % 128 == 0 && M % 128 == 0) {
    launch_hgemm_bk64_bn128<half, true>(
  ...
  launch_hgemm_pipeline_bn160<half, 3, true>(
      reinterpret_cast<half *>(a.data_ptr()),
      reinterpret_cast<half *>(b_col_major.data_ptr()),
      reinterpret_cast<half *>(c.data_ptr()),
      M, N, K, 3200);
