...
struct HgemmVerticalGroup3Stage {
...
cutlass::Status try_launch(int M, int N, int K,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  cutlass::Status status = try_launch<HgemmVerticalGroup3Stage>(M, N, K, ptr_A, ptr_B, ptr_C);

  if (status != cutlass::Status::kSuccess) {
    status = try_launch<HgemmVerticalGroupAuto>(M, N, K, ptr_A, ptr_B, ptr_C);
  }

  if (status != cutlass::Status::kSuccess) {
    status = try_launch<HgemmFallback256x192>(M, N, K, ptr_A, ptr_B, ptr_C);
  }
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS GEMM failed on all variants");
  }
