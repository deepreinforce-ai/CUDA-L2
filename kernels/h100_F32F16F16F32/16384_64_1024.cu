...
hgemm_optimized_correct_kernel(
...
void launch_hgemm_optimized_final(T *a, T *b, T *c, int M, int N, int K)
{
    constexpr int BM_val = 128;
    constexpr int BN_val = 64;
    ...
    using g2s_copy_op_a = SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>;
    ...
    using g2s_copy_op_b = SM80_CP_ASYNC_CACHEALWAYS<cute::uint128_t>;
    ...
    static constexpr int kShmSize = 
        (shm_size_AB > shm_size_C) ? shm_size_AB : shm_size_C;

    auto kernel_fn = hgemm_optimized_correct_kernel<
        T, BM_val, BN_val, BK_val, Stages,
                                ...
                                torch::Tensor b_col_major, torch::Tensor c)
{
    ...
    launch_hgemm_optimized_final<half, 5>(
