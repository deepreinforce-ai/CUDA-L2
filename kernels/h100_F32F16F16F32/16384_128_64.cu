...
hgemm_warpgroup_k64_kernel(T *__restrict__ Aptr, T *__restrict__ Bptr,
...
template <typename T>
void launch_hgemm_warpgroup_k64(T *a, T *b_col_major, T *c, int M, int N, int K) {
  using namespace cute;

  static constexpr int BM = 128;
  static constexpr int BN = 128;
  static constexpr int BK = 64;
  static constexpr int kStage = 1;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C smem exceeds A's single stage");
  ...
  cudaFuncSetAttribute(
      hgemm_warpgroup_k64_kernel<T, BM, BN, BK, MMA,
                                 G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_warpgroup_k64<half>(
