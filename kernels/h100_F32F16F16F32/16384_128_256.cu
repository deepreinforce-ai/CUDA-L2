...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, 
                                                  int m, int n, int k) {
  using namespace cute;
...
#pragma unroll
  ...
  constexpr int reg_count = decltype(size(tCrD))::value;
  
#pragma unroll
  for (int i = 0; i < reg_count / 2; ++i) {
    float2 f2 = make_float2(tCrD(2*i), tCrD(2*i + 1));
    half2 h2 = __float22half2_rn(f2);
    tCrD_half(2*i) = __low2half(h2);
    tCrD_half(2*i + 1) = __high2half(h2);
  }
  
  if (reg_count % 2 == 1) {
    tCrD_half(reg_count - 1) = __float2half_rn(tCrD(reg_count - 1));
  }
