...
struct SharedStorageOptimized
{
  ...
  alignas(16) uint64_t tma_barrier[7];
  alignas(16) uint64_t mma_barrier[7];
};
...
void gemm_split_epilogue_kernel(
    ...
    Alpha alpha, Beta beta)
{
  ...
  constexpr int tma_transaction_bytes = 
      sizeof(cute::make_tensor_like(cute::tensor<0>(tAsA))) +
      sizeof(cute::make_tensor_like(cute::tensor<0>(tBsB)));
  ...
  auto write_state = cutlass::PipelineState<7>();
  auto read_state  = cutlass::PipelineState<7>();
  
  int k_tiles_remaining = k_tile_count;
  ...
  while (k_tiles_remaining > 0)
  {
    ...
    if (k_tiles_remaining > 5) {
      warpgroup_wait<4>();
    } else if (k_tiles_remaining > 3) {
      warpgroup_wait<3>();
    } else if (k_tiles_remaining > 1) {
      warpgroup_wait<1>();
    } else {
      warpgroup_wait<0>();
    }
...
void launch_hgemm_split_epilogue_sm90(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  auto kernel_ptr = &gemm_split_epilogue_kernel<
    ...
    torch::Tensor c)
{
  ...
  launch_hgemm_split_epilogue_sm90<cute::half_t, cute::half_t, cute::half_t>(
