...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  if (status != cutlass::Status::kSuccess) {
    std::string err_msg = "CUTLASS cannot implement problem: M=" + std::to_string(M) + 
  ...
  if (cuda_status != cudaSuccess) {
    std::string err_msg = "CUDA kernel error: " + std::string(cudaGetErrorString(cuda_status));
  ...
  if (cuda_status != cudaSuccess) {
    std::string err_msg = "CUDA synchronization failed: " + std::string(cudaGetErrorString(cuda_status));
    err_msg += " (M=" + std::to_string(M) + ", N=" + std::to_string(N) + ", K=" + std::to_string(K) + ")";
    throw std::runtime_error(err_msg);
  }
}