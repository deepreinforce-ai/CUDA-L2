...
hgemm_kernel_v3(T *__restrict__ Aptr, T *__restrict__ Bptr,
...
hgemm_kernel_bn128(T *__restrict__ Aptr, T *__restrict__ Bptr,
                   T *__restrict__ Dptr, int m, int n, int k) {
  using namespace cute;
  extern __shared__ T shm_data[];
...
template <typename T, int Stages, bool DoBlockSwizzle>
void launch_hgemm_v3(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;
  ...
  constexpr bool BlockSwizzle = DoBlockSwizzle;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                "C smem too large for A's pipeline stage");
  ...
  cudaFuncSetAttribute(
      hgemm_kernel_v3<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void launch_hgemm_bn128(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_kernel_bn128<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
                         SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA,
                         S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_v3<half, 6, true>(a_ptr, b_ptr, c_ptr, M, N, K, 128);
}