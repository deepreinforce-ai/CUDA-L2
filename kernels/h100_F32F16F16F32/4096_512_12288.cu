...
void gemm_optimized_barrier_kernel(
    ...
    Alpha alpha, Beta beta)
{
  ...
  if (warp_idx == 0 && lane_predicate) {
    ...
    int initial_tiles = min(K_PIPE_MAX, size<1>(tAgA));
    k_tile += initial_tiles;
    k_tile_count -= initial_tiles;
  }
...
void launch_optimized_barrier_hgemm_sm90(
    TA const* A, int ldA,
    TB const* B, int ldB,
    TC* C, int ldC,
    int M, int N, int K)
{
  ...
  auto kernel_ptr = &gemm_optimized_barrier_kernel<
                                 ...
                                 torch::Tensor b_col_major, torch::Tensor c)
{
  ...
  launch_optimized_barrier_hgemm_sm90<cute::half_t, cute::half_t, cute::half_t>(
