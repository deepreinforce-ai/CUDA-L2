...
hgemm_kernel_bm32_optimized(T * __restrict__ Aptr, T * __restrict__ Bptr, 
                             T * __restrict__ Dptr, const int m, const int n, const int k) {
...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 2)
hgemm_kernel_bm64(T * __restrict__ Aptr, T * __restrict__ Bptr, 
                   T * __restrict__ Dptr, const int m, const int n, const int k) {
  using namespace cute;
...
template <typename T>
void launch_bm32_optimized(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_kernel_bm32_optimized<T, BM, BN, BK, KStage, MMA,
...
template <typename T>
void launch_bm64(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_kernel_bm64<T, BM, BN, BK, KStage, MMA,
                        G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB,
                        SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB,
                        R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);
...
template <typename T>
inline void launch_optimal_hgemm(T *a, T *b, T *c, int M, int N, int K) {
  constexpr int H100_SM_COUNT = 108;
  
  const int blocks_bm32 = ((M + 31) / 32) * ((N + 63) / 64);
  const int blocks_bm64 = ((M + 63) / 64) * ((N + 63) / 64);
  
  const float occupancy_bm32 = static_cast<float>(blocks_bm32) / H100_SM_COUNT;
  
  if (occupancy_bm32 >= 1.0f && occupancy_bm32 <= 1.5f) {
    launch_bm32_optimized<T>(a, b, c, M, N, K);
  } else {
    launch_bm64<T>(a, b, c, M, N, K);
  }
}
