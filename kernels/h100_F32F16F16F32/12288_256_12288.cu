...
template <
  class MainloopScheduleType = cutlass::gemm::KernelTmaWarpSpecializedPingpong,
  class EpilogueScheduleType = cutlass::epilogue::TmaWarpSpecialized,
  class TileSchedulerType = cutlass::gemm::PersistentScheduler
>
struct HgemmPingpong128 {
  using LayoutA = cutlass::layout::RowMajor;
  using LayoutB = cutlass::layout::ColumnMajor;
  using LayoutC = cutlass::layout::RowMajor;
  using LayoutD = cutlass::layout::RowMajor;
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  run_gemm<HgemmProvenCooperative<>>(a, b_col_major, c, M, N, K);
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported");
#endif
}