...
hgemm_kernel_optimized_v2(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    ...
    if (ik + 2 < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, ik_next), tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, ik_next), tCrB_view(_, _, ik_next));
    }
...
template <typename T>
void launch_hgemm_optimized_v2(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_kernel_optimized_v2<T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_v2<half>(
