...
hgemm_gen7_kernel(const T *__restrict__ Aptr, const T *__restrict__ Bptr,
  ...
  const int k_start  = k_split_idx * k_chunk;
  const int k_len    = (k_split_idx == SPLIT_K - 1) ? (k - k_start) : k_chunk;
  const int k_tiles  = k_len / BK;

  const T *Aptr_off = Aptr + k_start;
  const T *Bptr_off = Bptr + k_start;

  Tensor A = make_tensor(make_gmem_ptr(Aptr_off),
                         make_shape(m, k_len),
                         make_stride(k, Int<1>{}));
  Tensor B = make_tensor(make_gmem_ptr(Bptr_off),
                         make_shape(n, k_len),
                         make_stride(k, Int<1>{}));
...
#pragma unroll
    ...
    auto tCgD_mma = thr_mma.partition_C(gD);

    const int total_c = size(tCrD_half);
    const int total_g = size(tCgD_mma);
...
template <typename T, const int Stages, const int SplitK = 1>
void launch_hgemm_gen7(const T *a, const T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C smem exceeds A single-stage size");
  ...
  using KernelType = decltype(&hgemm_gen7_kernel<
      T, BM, BN, BK, Stages, SplitK, MMA,
      G2SCopyA, G2SCopyB,
      SmemLayoutA, SmemLayoutB, SmemLayoutC,
      S2RCopyAtomA, S2RCopyAtomB,
      R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>);

  KernelType kernel_ptr = hgemm_gen7_kernel<
      T, BM, BN, BK, Stages, SplitK, MMA,
      G2SCopyA, G2SCopyB,
      SmemLayoutA, SmemLayoutB, SmemLayoutC,
      S2RCopyAtomA, S2RCopyAtomB,
  ...
  kernel_ptr<<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}
