...
__global__ void cuda_l2_h100_fp32_kernel(
...
#pragma unroll
  ...
  const int total_iters = size<1>(tCrC_r2sx);

#pragma unroll
  for (int i = 0; i < total_iters; i += step) {
#pragma unroll
...
#pragma unroll
    ...
    if (i + step < total_iters) {
      __syncthreads();
    }
  }
}

template <typename T, const int Stages = 5, const bool BlockSwizzle = true>
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized<half, 5, true>(
