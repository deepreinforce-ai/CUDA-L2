    ...
    int swizzle_stride)
{
    ...
    T * const Ashm = shm_data;
    T * const Bshm = shm_data + cute::cosize(SmemLayoutA{});

    const int idx = threadIdx.x;
    const int BZ  = (n + swizzle_stride - 1) / swizzle_stride;
    ...
    const int total = size<1>(tCrC_r2sx);
...
template <typename T>
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride)
{
    using namespace cute;
    ...
    static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                  "Epilogue C smem exceeds A pipeline capacity");
    ...
    const int BX_base = (N + BN - 1) / BN;
    const int BY      = (M + BM - 1) / BM;
    const int BZ      = (N + swizzle_stride - 1) / swizzle_stride;
    const int BX      = (BX_base + BZ - 1) / BZ;

    const dim3 block(size(MMA{}));
    ...
    cuda_l2_h100_fp32_kernel<
        ...
        G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC,
        S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>
        <<<grid, block, kShmSize>>>(a, b, c, M, N, K, swizzle_stride);
}
