...
struct PrimaryAggressiveHgemmKernel {
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename PrimaryAggressiveHgemmKernel::Gemm;
  using StrideA = typename PrimaryAggressiveHgemmKernel::StrideA;
  using StrideB = typename PrimaryAggressiveHgemmKernel::StrideB;
  using StrideC = typename PrimaryAggressiveHgemmKernel::StrideC;
  using StrideD = typename PrimaryAggressiveHgemmKernel::StrideD;
  using ElementA = typename PrimaryAggressiveHgemmKernel::ElementA;
  using ElementB = typename PrimaryAggressiveHgemmKernel::ElementB;
  using ElementC = typename PrimaryAggressiveHgemmKernel::ElementC;
...
#else
  throw std::runtime_error("CUTLASS SM90 MMA operations not supported on this GPU");
#endif
}