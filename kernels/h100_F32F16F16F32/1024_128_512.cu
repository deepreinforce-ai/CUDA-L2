...
hgemm_kernel_64x64x64_fixed(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 3)
hgemm_kernel_64x128x32_fallback(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  using namespace cute;
  extern __shared__ T shm_data[];
...
template <typename T>
void launch_64x64x64_6stage(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  auto kernel_ptr = hgemm_kernel_64x64x64_fixed<
...
template <typename T>
void launch_64x128x32_6stage(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  auto kernel_ptr = hgemm_kernel_64x128x32_fallback<
      T, BM, BN, BK, Stages, MMA,
      G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC,
      S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  if (g_best_config == -1) {
    ...
    for (int i = 0; i < warmup; i++)
      launch_64x64x64_6stage<half>(a_ptr, b_col_ptr, c_ptr, M, N, K);
    ...
    for (int i = 0; i < warmup; i++)
      launch_64x128x32_6stage<half>(a_ptr, b_col_ptr, c_ptr, M, N, K);
    cudaDeviceSynchronize();
    cudaEventRecord(start);
    for (int i = 0; i < bench; i++)
      launch_64x128x32_6stage<half>(a_ptr, b_col_ptr, c_ptr, M, N, K);
    ...
    g_best_config = (times[0] <= times[1]) ? 0 : 1;
  }
