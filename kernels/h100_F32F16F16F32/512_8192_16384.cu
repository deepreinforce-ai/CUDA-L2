...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename HgemmMinimalTiling::Gemm;
  using StrideA = typename HgemmMinimalTiling::StrideA;
  using StrideB = typename HgemmMinimalTiling::StrideB;
  using StrideC = typename HgemmMinimalTiling::StrideC;
  using StrideD = typename HgemmMinimalTiling::StrideD;
  using ElementA = typename HgemmMinimalTiling::ElementA;
  using ElementB = typename HgemmMinimalTiling::ElementB;
  using ElementC = typename HgemmMinimalTiling::ElementC;
  ...
  if (cuda_status != cudaSuccess) {
    throw std::runtime_error(std::string("Sync failed: ") + cudaGetErrorString(cuda_status));
  }
#else
