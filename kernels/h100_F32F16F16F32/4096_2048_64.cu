...
__global__ void __launch_bounds__(128, 2) hgemm_ultimate_bk32_kernel(
...
#pragma unroll
  for (int i = 0; i < size(tCrD); i += 8) {
#pragma unroll
    for (int j = 0; j < 8 && (i + j) < size(tCrD); j += 2) {
      float2 f2 = make_float2(tCrD(i + j), 
                              (i + j + 1 < size(tCrD)) ? tCrD(i + j + 1) : 0.0f);
      __half2 h2 = __float22half2_rn(f2);
      tCrD_half(i + j) = reinterpret_cast<T*>(&h2)[0];
      if (i + j + 1 < size(tCrD)) {
        tCrD_half(i + j + 1) = reinterpret_cast<T*>(&h2)[1];
      }
    }
  }
...
void launch_hgemm_ultimate(T *a, T *b, T *c, int M, int N, int K,
                           int swizzle_stride) {
  using namespace cute;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_ultimate<half, 2, true>(
