...
struct SwizzledSingleCTAHgemmConfig {
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename SwizzledSingleCTAHgemmConfig::Gemm;
  using StrideA = typename SwizzledSingleCTAHgemmConfig::StrideA;
  using StrideB = typename SwizzledSingleCTAHgemmConfig::StrideB;
  using StrideC = typename SwizzledSingleCTAHgemmConfig::StrideC;
  using StrideD = typename SwizzledSingleCTAHgemmConfig::StrideD;
  using ElementA = typename SwizzledSingleCTAHgemmConfig::ElementA;
  using ElementB = typename SwizzledSingleCTAHgemmConfig::ElementB;
  using ElementC = typename SwizzledSingleCTAHgemmConfig::ElementC;
...
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported - requires H100");
#endif
}