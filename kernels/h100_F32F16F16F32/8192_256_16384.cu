...
void gemm_wgmma_warp_specialized_kernel(
    ...
    Alpha alpha, Beta beta)
{
  ...
  using SharedStorage = SharedStorageWarpSpec<TA, TB, SmemLayoutA, SmemLayoutB>;
  ...
  int total_iterations = k_tile_count;
  int main_loop_count = total_iterations / kUnrollFactor;
  int remainder = total_iterations % kUnrollFactor;
  
  int consumed_iterations = 0;
  
  CUTE_NO_UNROLL
  for (int unroll_idx = 0; unroll_idx < main_loop_count; ++unroll_idx)
  {
    CUTE_UNROLL
    for (int local_idx = 0; local_idx < kUnrollFactor; ++local_idx)
    {
      ...
      int remaining = total_iterations - consumed_iterations - 1;
      if (remaining > 3) {
        warpgroup_wait<2>();
      } else if (remaining > 1) {
        warpgroup_wait<1>();
  ...
  if constexpr (!is_constant<1, Alpha>::value) {
    CUTE_UNROLL
  ...
  if constexpr (is_constant<0, Beta>::value) {
    copy(tCrC, tCgC);
  } else {
    ...
    int M, int N, int K)
{
  ...
  using SharedStorage = SharedStorageWarpSpec<TA, TB, decltype(sA), decltype(sB)>;
  ...
  auto kernel_ptr = &gemm_wgmma_warp_specialized_kernel<
