...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, 1), tCrA_view(_, _, ik));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, 1), tCrB_view(_, _, ik));
  }
...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  ...
  constexpr int kMmaPM = 2 * get<0>(mma_atom_shape{});
  constexpr int kMmaPN = 4 * get<1>(mma_atom_shape{});
  constexpr int kMmaPK = 1 * get<2>(mma_atom_shape{});
