...
hgemm_kernel_optimized(T *__restrict__ Aptr, T *__restrict__ Bptr,
...
#pragma unroll
  for (int i = 0; i < size(tCrD); i += 8) {
    int chunk_end = min(i + 8, (int)size(tCrD));
#pragma unroll
...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  ...
  cudaFuncSetAttribute(
      hgemm_kernel_optimized<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
