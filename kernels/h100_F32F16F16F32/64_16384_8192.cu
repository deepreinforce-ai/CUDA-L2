...
hgemm_double_buffer_fixed_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_ping = make_fragment_like(tCrA);
  auto tCrB_ping = make_fragment_like(tCrB);
  auto tCrA_pong = make_fragment_like(tCrA);
  auto tCrB_pong = make_fragment_like(tCrB);
  ...
  auto tCrA_ping_view = s2r_thr_copy_a.retile_D(tCrA_ping);
  auto tCrA_pong_view = s2r_thr_copy_a.retile_D(tCrA_pong);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_ping_view = s2r_thr_copy_b.retile_D(tCrB_ping);
  auto tCrB_pong_view = s2r_thr_copy_b.retile_D(tCrB_pong);
...
#pragma unroll
  ...
  int ik = 0;
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, ismem_read), tCrA_ping_view(_, _, ik));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, ismem_read), tCrB_ping_view(_, _, ik));

  int ntile = k / BK;

#pragma unroll 1
  for (int itile = 0; itile < ntile; ++itile) {
    int nk = size<2>(tCrA_ping);
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik & 1) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_ping_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_ping_view(_, _, ik_next));
      } else {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_pong_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_pong_view(_, _, ik_next));
      }
      ...
      if (ik & 1) {
        cute::gemm(tiled_mma, tCrD, tCrA_pong(_, _, ik), tCrB_pong(_, _, ik), tCrD);
      } else {
        cute::gemm(tiled_mma, tCrD, tCrA_ping(_, _, ik), tCrB_ping(_, _, ik), tCrD);
      }
    }
  }
...
void launch_hgemm_double_buffer_fixed(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_double_buffer_fixed_kernel<
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_double_buffer_fixed<half, 5>(
