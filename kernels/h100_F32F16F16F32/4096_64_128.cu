    ...
    int m, int n, int k)
{
    ...
    cute::copy(g2s_copy_a, tAgA(_, _, _, 0), tAsA(_, _, _, 0));
    cute::copy(g2s_copy_b, tBgB(_, _, _, 0), tBsB(_, _, _, 0));
    cp_async_fence();
    cp_async_wait<0>();
    __syncthreads();
    ...
    cute::copy(s2r_copy_a, tAsA_s2r(_, _, 0, 0), tCrA_view(_, _, 0));
    cute::copy(s2r_copy_b, tBsB_s2r(_, _, 0, 0), tCrB_view(_, _, 0));

#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
        if (ik < nk - 1) {
            cute::copy(s2r_copy_a, tAsA_s2r(_, _, ik + 1, 0), tCrA_view(_, _, ik + 1));
            cute::copy(s2r_copy_b, tBsB_s2r(_, _, ik + 1, 0), tCrB_view(_, _, ik + 1));
        }
...
template <typename T>
void launch_bm32_max_coverage(T *a, T *b, T *c, int M, int N, int K)
{
    static constexpr int BM = 32, BN = 64, BK = 128;
    ...
    static constexpr int kNThreads = size(MMA{});
    ...
    static constexpr int shm_size_C = cute::cosize(SmemLayoutC{}) * (int)sizeof(T);
    static constexpr int kShmSize = shm_size_AB > shm_size_C ? shm_size_AB : shm_size_C;

    auto kernel = hgemm_max_sm_coverage_kernel<
    ...
    cudaFuncSetAttribute(kernel, cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);
    kernel<<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}
    ...
    torch::Tensor c)
{
    ...
    launch_bm32_max_coverage<half>(a_ptr, b_col_ptr, c_ptr, M, N, K);
}