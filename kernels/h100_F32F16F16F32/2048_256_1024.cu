...
cuda_l2_h100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  int num_full_vecs = size(tCrD) / vec_width;
  int remainder = size(tCrD) % vec_width;
  
  #pragma unroll
  for (int i = 0; i < num_full_vecs; ++i) {
    int base = i * vec_width;
    
    float f0 = tCrD(base);
    float f1 = tCrD(base + 1);
    float f2 = tCrD(base + 2);
    float f3 = tCrD(base + 3);
    
    half2 h01 = __float22half2_rn(make_float2(f0, f1));
    half2 h23 = __float22half2_rn(make_float2(f2, f3));
    
    tCrD_half(base) = h01.x;
    tCrD_half(base + 1) = h01.y;
    tCrD_half(base + 2) = h23.x;
    tCrD_half(base + 3) = h23.y;
  }
  
  int base = num_full_vecs * vec_width;
  if (remainder >= 2) {
    half2 h = __float22half2_rn(make_float2(tCrD(base), tCrD(base + 1)));
    tCrD_half(base) = h.x;
    tCrD_half(base + 1) = h.y;
    base += 2;
    remainder -= 2;
  }
  if (remainder == 1) {
    tCrD_half(base) = __float2half_rn(tCrD(base));
  }
  ...
  int num_iterations = size<1>(tCrC_r2sx);
  ...
  for (int i = 0; i < num_iterations; i += step) {
    ...
    if (i + step < num_iterations) {
      __syncthreads();
    }
  }
}

template <typename T, const int Stages = 7, const bool BlockSwizzle = false>
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  LAUNCH_HGEMM_MMA_STAGES_CUTE_SWIZZLE_TN(7, swizzle_stride);
}