...
hgemm_optimized_v21_kernel(T *__restrict__ Aptr, T *__restrict__ Bptr,
  ...
  constexpr int nk = size<2>(decltype(tCrA){});
  ...
  auto tCrC_r2s = r2s_thr_copy_c.retile_S(tCrD_fp16);
...
void launch_hgemm_optimized_v21(T *a, T *b, T *c, int M, int N, int K,
                               int swizzle_stride) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_v21_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_v21<half, 2, true>(
