...
#pragma unroll
  ...
  if constexpr (num_elements % 2 == 0) {
    float* src = reinterpret_cast<float*>(&tCrD(0));
    __half* dst = reinterpret_cast<__half*>(&tCrD_half(0));
    
#pragma unroll
    for (int i = 0; i < num_elements; i += 2) {
      __half2 h2 = __float22half2_rn(make_float2(src[i], src[i+1]));
      *reinterpret_cast<__half2*>(&dst[i]) = h2;
    }
  } else {
