...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, 0), 
               tCrA_view(_, _, ik));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, 0), 
               tCrB_view(_, _, ik));
  }
...
template <typename T>
void launch_hgemm_mma_bk128_tn_cute(T *a, T *b, T *c, int M,
                                    int N, int K,
                                    int swizzle_stride) {
  using namespace cute;
  ...
  using SmemLayoutAtom = decltype(composition(
      Swizzle<3, 3, 3>{},
      make_layout(make_shape(Int<16>{}, Int<64>{}),
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_bk128_tn_cute<half>(
