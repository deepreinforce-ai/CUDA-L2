...
void run_proven_stable_gemm(torch::Tensor a, torch::Tensor b_col_major, torch::Tensor c, 
                             int M, int N, int K) {
  using Gemm = typename HgemmVerifiedStable::Gemm;
  using StrideA = typename HgemmVerifiedStable::StrideA;
  using StrideB = typename HgemmVerifiedStable::StrideB;
  using StrideC = typename HgemmVerifiedStable::StrideC;
  using StrideD = typename HgemmVerifiedStable::StrideD;
  using ElementA = typename HgemmVerifiedStable::ElementA;
  using ElementB = typename HgemmVerifiedStable::ElementB;
  using ElementC = typename HgemmVerifiedStable::ElementC;
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  run_proven_stable_gemm(a, b_col_major, c, M, N, K);
#else
  throw std::runtime_error("This kernel requires CUDA compute capability 9.0+ (H100)");
#endif
}