...
hgemm_branchless_dual_buffer_kernel(T * __restrict__ Aptr, 
                                     T * __restrict__ Bptr, 
                                     T * __restrict__ Dptr,
                                     int m, int n, int k) {
  ...
  if (ix * BN >= n || m_tile_idx * BM >= m) return;
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      bool should_prefetch = (ik < nk - 1) || !is_last_tile;
      
      if (should_prefetch) {
        if (buf_select_next == 0) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                     tCrA_view_buf0(_, _, ik_next));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                     tCrB_view_buf0(_, _, ik_next));
        } else {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                     tCrA_view_buf1(_, _, ik_next));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                     tCrB_view_buf1(_, _, ik_next));
        }
        __syncwarp();
      }
      ...
      if (buf_select == 0) {
        cute::gemm(tiled_mma, tCrD, tCrA_buf0(_, _, ik), 
                   tCrB_buf0(_, _, ik), tCrD);
      } else {
        cute::gemm(tiled_mma, tCrD, tCrA_buf1(_, _, ik), 
                   tCrB_buf1(_, _, ik), tCrD);
      }
    }
  }
  ...
  for (int i = 0; i < size<1>(tCrC_r2sx); i += step) {
    ...
    for (int j = 0; j < step; ++j) {
      cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, i + j), 
                 tCsC_r2s(_, 0, 0, j));
    }
    __syncthreads();
    ...
    #pragma unroll
    for (int j = 0; j < step; ++j) {
      cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), 
                 tCgC_s2gx(_, i + j));
    }
...
template <typename T, const int Stages = 6>
void launch_hgemm_branchless_dual_buffer(T *a, T *b, T *c, 
                                          int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_branchless_dual_buffer_kernel<
  ...
  hgemm_branchless_dual_buffer_kernel<
      ...
      S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_branchless_dual_buffer<half, 6>(
