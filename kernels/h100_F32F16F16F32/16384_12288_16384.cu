...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  if (status != cutlass::Status::kSuccess) {
    std::string err_msg = "CUTLASS Deep Pipeline Warp-Group GEMM cannot implement: M=" +
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS Deep Pipeline Warp-Group GEMM initialization failed");
  }
  ...
  if (status != cutlass::Status::kSuccess) {
    throw std::runtime_error("CUTLASS Deep Pipeline Warp-Group GEMM execution failed");
  }
