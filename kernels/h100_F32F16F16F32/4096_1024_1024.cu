...
#pragma unroll
  ...
  int vec_size = size(tCrD);
  
#pragma unroll
  for (int i = 0; i < vec_size; i += 4) {
    if (i + 3 < vec_size) {
      float4 tmp = make_float4(tCrD(i), tCrD(i+1), tCrD(i+2), tCrD(i+3));
      half2 r01 = __float22half2_rn(make_float2(tmp.x, tmp.y));
      half2 r23 = __float22half2_rn(make_float2(tmp.z, tmp.w));
      tCrD_half(i) = r01.x;
      tCrD_half(i+1) = r01.y;
      tCrD_half(i+2) = r23.x;
      tCrD_half(i+3) = r23.y;
    } else if (i + 1 < vec_size) {
      float2 tmp = make_float2(tCrD(i), tCrD(i+1));
      half2 result = __float22half2_rn(tmp);
      tCrD_half(i) = result.x;
      tCrD_half(i+1) = result.y;
      if (i + 2 < vec_size) {
        tCrD_half(i+2) = __float2half_rn(tCrD(i+2));
      }
      break;
    } else {
