...
struct ExtendedKTileHgemmKernel {
...
#if defined(CUTLASS_ARCH_MMA_SM90_SUPPORTED)
  using Gemm = typename ExtendedKTileHgemmKernel::Gemm;
  using StrideA = typename ExtendedKTileHgemmKernel::StrideA;
  using StrideB = typename ExtendedKTileHgemmKernel::StrideB;
  using StrideC = typename ExtendedKTileHgemmKernel::StrideC;
  using StrideD = typename ExtendedKTileHgemmKernel::StrideD;
  using ElementA = typename ExtendedKTileHgemmKernel::ElementA;
  using ElementB = typename ExtendedKTileHgemmKernel::ElementB;
  using ElementC = typename ExtendedKTileHgemmKernel::ElementC;
...
#else
  throw std::runtime_error("CUTLASS SM90 MMA not supported on this GPU architecture");
#endif
}