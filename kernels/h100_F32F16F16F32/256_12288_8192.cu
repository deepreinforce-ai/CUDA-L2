...
struct HgemmConfig_2x4x1 {
  ...
  using GridShape  = cute::Shape<cute::_2,   cute::_4,   cute::_1>;
...
void cuda_l2_h100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  status = try_run_gemm<HgemmConfig_2x4x1>(M, N, K, ptr_A, ptr_B, ptr_C);
  if (status == cutlass::Status::kSuccess) {
    if (cudaGetLastError() == cudaSuccess) return;
  }

  status = try_run_gemm<HgemmConfig_1x4x1>(M, N, K, ptr_A, ptr_B, ptr_C);
  if (status == cutlass::Status::kSuccess) {
    if (cudaGetLastError() == cudaSuccess) return;
  }

  status = try_run_gemm<HgemmConfig_1x2x1>(M, N, K, ptr_A, ptr_B, ptr_C);
  if (status == cutlass::Status::kSuccess) {
    if (cudaGetLastError() == cudaSuccess) return;
  }

  status = try_run_gemm<HgemmConfig_2x2x1>(M, N, K, ptr_A, ptr_B, ptr_C);
  if (status == cutlass::Status::kSuccess) {
    if (cudaGetLastError() == cudaSuccess) return;
  }

  throw std::runtime_error("CUTLASS GEMM failed on all configurations");
