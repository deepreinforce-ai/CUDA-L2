...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(64, 8) cuda_l2_a100_fp32_kernel(
...
void launch_hgemm_asymmetric_tile(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_asymmetric_tile<half, 3>(
