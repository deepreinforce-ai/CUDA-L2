...
#pragma unroll
  ...
  int itile = 0;
  for (; itile + 15 < ntile; itile += 16) {
#pragma unroll
    for (int unroll_idx = 0; unroll_idx < 16; ++unroll_idx) {
#pragma unroll
...
#pragma unroll
  ...
  for (; itile + 3 < ntile; itile += 4) {
#pragma unroll
    for (int unroll_idx = 0; unroll_idx < 4; ++unroll_idx) {
#pragma unroll
      for (int ik = 0; ik < nk; ++ik) {
        int ik_next = (ik + 1) % nk;
