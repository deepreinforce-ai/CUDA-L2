...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, 
                                   int m, int n, int k) {
...
#pragma unroll
  for (int i = 0; i < size(tCrD); ++i) {
    tCrD_fp16(i) = static_cast<T>(tCrD(i));
  }
...
template <typename T>
void launch_hgemm_optimized_k64_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(size(SmemLayoutA{}) >= size(SmemLayoutC{}),
                "C shared memory request is larger than A's");
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_optimized_k64_tn_cute<half>(
