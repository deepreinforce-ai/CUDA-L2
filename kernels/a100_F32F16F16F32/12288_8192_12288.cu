...
#pragma unroll
  ...
  constexpr int kNumVecs = decltype(size(tCrD))::value / kVecSize;
  constexpr int kRemaining = decltype(size(tCrD))::value % kVecSize;
  
#pragma unroll
  for (int vec_idx = 0; vec_idx < kNumVecs; ++vec_idx) {
    int base = vec_idx * kVecSize;
    
#pragma unroll
    for (int i = 0; i < kVecSize; ++i) {
      float val = tCrD(base + i);
      tCrD_half(base + i) = __float2half(val);
    }
  }
  
#pragma unroll
  for (int i = kNumVecs * kVecSize; i < kNumVecs * kVecSize + kRemaining; ++i) {
