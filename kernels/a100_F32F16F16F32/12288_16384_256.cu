...
void launch_hgemm_warp_specialized(T *a, T *b, T *c, int M, int N, int K,
                                   int swizzle_stride) {
  using namespace cute;
...
#define LAUNCH_WARP_SPEC_SWIZZLE(stages, stride)                               \
  launch_hgemm_warp_specialized<half, (stages), true>(                         \
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  LAUNCH_WARP_SPEC_SWIZZLE(4, swizzle_stride);
}