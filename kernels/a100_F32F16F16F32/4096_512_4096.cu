...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik < nk - 1 || itile < ntile - 1) {
        int read_stage = (ik == nk - 1) ? ismem_read : ismem_read;
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, read_stage),
                   tCrA_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, read_stage),
