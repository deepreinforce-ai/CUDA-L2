...
template<typename T>
__device__ __forceinline__ void warp_cooperative_load_and_distribute(
    const T* __restrict__ smem_ptr, 
    T* __restrict__ reg_buffer,
    int thread_idx_in_warp,
    int elements_per_thread) {
  
  constexpr int WARP_SIZE = 32;
  const unsigned FULL_MASK = 0xffffffff;
  
  const int base_offset = thread_idx_in_warp * 8;
  
  #pragma unroll
  for (int i = 0; i < 8; ++i) {
    reg_buffer[i] = smem_ptr[base_offset + i];
  }
  
  #pragma unroll
  for (int i = 0; i < elements_per_thread; ++i) {
    int src_lane = (thread_idx_in_warp ^ (i / 2)) % WARP_SIZE;
    int src_elem = (i % 2) * 4 + (i / 8);
    
    if (src_elem < 8) {
      T shuffled = __shfl_sync(FULL_MASK, reg_buffer[src_elem], src_lane);
      if (i < elements_per_thread) {
        reg_buffer[i] = shuffled;
      }
    }
  }
}
...
__global__ void __launch_bounds__(128, 2) cuda_l2_a100_fp32_kernel(
