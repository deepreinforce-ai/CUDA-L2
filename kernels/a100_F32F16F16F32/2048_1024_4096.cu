...
#pragma unroll
    for (int i = 0; i < num_elements; i += 2) {
      if (i + 1 < num_elements) {
        float f0 = static_cast<float>(tCrD(i));
        float f1 = static_cast<float>(tCrD(i + 1));
        __half2 h2 = __float22half2_rn(make_float2(f0, f1));
        tCrD_half(i) = h2.x;
        tCrD_half(i + 1) = h2.y;
      } else {
        tCrD_half(i) = __float2half_rn(static_cast<float>(tCrD(i)));
      }
    }
  } else {
    int base = (num_elements / 2) * 2;
#pragma unroll
    ...
    if (base < num_elements) {
      tCrD_half(base) = __float2half_rn(static_cast<float>(tCrD(base)));
    }
  }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory exceeds A buffer size");
  ...
  cudaFuncSetAttribute(
      cuda_l2_a100_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA,
                                        ...
                                        SmemLayoutC, S2RCopyAtomA,
                                        S2RCopyAtomB, R2SCopyAtomC,
