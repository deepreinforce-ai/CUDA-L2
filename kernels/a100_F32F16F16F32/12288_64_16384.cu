...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  const int num_tiles_m = (m + BM - 1) / BM;
  const int num_tiles_n = (n + BN - 1) / BN;
  const int total_tiles = num_tiles_m * num_tiles_n;
  
  const int tiles_per_block = (total_tiles + gridDim.x - 1) / gridDim.x;
  const int block_start = blockIdx.x * tiles_per_block;
  const int block_end = min(block_start + tiles_per_block, total_tiles);

  for (int tile_id = block_start; tile_id < block_end; ++tile_id) {
    const int iy = tile_id / num_tiles_n;
    const int ix = tile_id % num_tiles_n;
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory exceeds A pipeline capacity");
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half, 3>(
