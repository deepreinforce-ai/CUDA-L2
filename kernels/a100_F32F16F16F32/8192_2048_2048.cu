...
#pragma unroll
  ...
  constexpr int kWarpOffset = 8;
  auto sC_base = make_tensor(make_smem_ptr(Ashm), SmemLayoutC{});
  
  T* sC_warp_ptr = Ashm + (warp_id * kWarpOffset);
  auto sC = make_tensor(make_smem_ptr(sC_warp_ptr), SmemLayoutC{});

  auto r2s_tiled_copy_c = make_tiled_copy_C(R2SCopyAtomC{}, tiled_mma);
  auto r2s_thr_copy_c = r2s_tiled_copy_c.get_slice(idx);
  auto tCrC_r2s = r2s_thr_copy_c.retile_S(tCrD_half);
  auto tCsC_r2s = r2s_thr_copy_c.partition_D(sC_base);

  S2GCopyC s2g_tiled_copy_c;
  auto s2g_thr_copy_c = s2g_tiled_copy_c.get_thread_slice(idx);
  auto tCsC_s2g = s2g_thr_copy_c.partition_S(sC_base);
...
#pragma unroll
    for (int j = 0; j < step; ++j) {
      auto offset_idx = (warp_id * kWarpOffset + lane_id) % 32;
