...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  int total_blocks_x = (n + BN - 1) / BN;
  int total_blocks_y = (m + BM - 1) / BM;
  int total_blocks = total_blocks_x * total_blocks_y;

  for (int tile_idx = block_id; tile_idx < total_blocks; tile_idx += gridDim.x) {
    int ix = tile_idx % total_blocks_x;
    int iy = tile_idx / total_blocks_x;
