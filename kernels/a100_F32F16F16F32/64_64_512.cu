...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 4) cuda_l2_a100_fp32_kernel(
  ...
  int ntile = k / BK;
  int nprefetch = cute::min(kStage - 1, ntile);

#pragma unroll
  for (int istage = 0; istage < nprefetch; ++istage) {
...
template <typename T, const int Stages = 12>
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_tn_cute<half, 12>(
