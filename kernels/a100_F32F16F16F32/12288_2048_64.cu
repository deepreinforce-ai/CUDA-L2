...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    int ik_prefetch = ik + 4;
    if (ik_prefetch < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch, 0), 
                 tCrA_view(_, _, ik_prefetch));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch, 0), 
                 tCrB_view(_, _, ik_prefetch));
    }
...
#pragma unroll
    for (int j = 0; j < step; ++j) {
      auto tCrC_half = make_tensor_like<T>(tCrC_r2sx(_, i + j));
      
#pragma unroll
      for (int e = 0; e < size(tCrC_half); ++e) {
        tCrC_half(e) = static_cast<T>(tCrC_r2sx(e, i + j));
      }
      
      cute::copy(r2s_tiled_copy_c, tCrC_half, tCsC_r2s(_, 0, 0, j));
    }
    __syncthreads();
