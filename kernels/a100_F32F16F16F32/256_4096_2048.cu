...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k, int total_tiles) {
  ...
  int tiles_x = (n + BN - 1) / BN;
  
  for (int tile_id = blockIdx.x; tile_id < total_tiles; tile_id += gridDim.x) {
    
    int ix = tile_id % tiles_x;
    int iy = tile_id / tiles_x;
    
    if (iy * BM >= m || ix * BN >= n) continue;
    ...
    constexpr int kMemoryWarps = 2;
    bool is_memory_warp = (warp_id < kMemoryWarps);
...
void launch_hgemm_mma_persistent_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cuda_l2_a100_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
      SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB,
      R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>
      <<<grid, block, kShmSize>>>(a, b, c, M, N, K, total_tiles);
}
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_persistent_cute<half, 6>(
