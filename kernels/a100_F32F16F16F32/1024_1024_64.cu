...
__global__ void __launch_bounds__(128)
  ...
  auto tCrD = make_fragment_like<float>(thr_mma.partition_fragment_C(gD));
  clear(tCrD);
...
#pragma unroll
  for (int prefetch_k = 0; prefetch_k < min(6, nk); ++prefetch_k) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, prefetch_k, 0), tCrA_view(_, _, prefetch_k));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, prefetch_k, 0), tCrB_view(_, _, prefetch_k));
  }

#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    int ik_prefetch = ik + 6;
    if (ik_prefetch < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch, 0), tCrA_view(_, _, ik_prefetch));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch, 0), tCrB_view(_, _, ik_prefetch));
    }
...
void launch_hgemm_optimized_cute(T *a, T *b, T *c, int M, int N, int K) {
  ...
  cudaFuncSetAttribute(
      cuda_l2_a100_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_cute<half, 2>(
