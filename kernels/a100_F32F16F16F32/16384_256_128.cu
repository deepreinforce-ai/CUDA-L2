...
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
  ...
  int iy = blockIdx.y;
  int k_block = blockIdx.z;

  if (iy * BM >= m || ix * BN >= n)
    return;

  int k_start = k_block * k_split;
  ...
  int prefetch_stages = min(kStage - 1, k_tiles);
#pragma unroll
  for (int istage = 0; istage < prefetch_stages; ++istage) {
    if (itile_to_read < k_end / BK) {
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  cuda_l2_a100_fp32_kernel<
      ...
      S2GCopyC, BlockSwizzle><<<grid, block, shm_size>>>(a, b, c, M, N, K, k_split);
}
