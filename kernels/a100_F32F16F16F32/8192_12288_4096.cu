...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  constexpr int vec_size = size(tCrD);
  
  #pragma unroll
  for (int i = 0; i < vec_size / 2; ++i) {
    float2 f2 = make_float2(
      static_cast<float>(tCrD(i * 2)),
      static_cast<float>(tCrD(i * 2 + 1))
    );
    half2 h2 = __float22half2_rn(f2);
    tCrD_half(i * 2) = h2.x;
    tCrD_half(i * 2 + 1) = h2.y;
  }
  
  if (vec_size % 2 == 1) {
    tCrD_half(vec_size - 1) = __float2half(static_cast<float>(tCrD(vec_size - 1)));
  }
...
template <typename T>
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
