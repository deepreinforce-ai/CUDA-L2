...
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
  ...
  if (ntile == 1) {
    ...
    #pragma unroll
    for (int i = 0; i < 8 && i < nk; ++i) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, i, 0), tCrA_view(_, _, i));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, i, 0), tCrB_view(_, _, i));
    }

    #pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      if (ik + 8 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 8, 0), tCrA_view(_, _, ik + 8));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 8, 0), tCrB_view(_, _, ik + 8));
      }
...
void launch_hgemm_aggressive_prefetch_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_aggressive_prefetch_cute<half, 2>(
