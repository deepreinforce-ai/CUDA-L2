...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2RCopyAtomB, const bool BlockSwizzle>
...
#pragma unroll
  ...
  auto sC_layout = make_layout(
      make_shape(Int<BM>{}, Int<BN + kPadding>{}),
      make_stride(Int<BN + kPadding>{}, Int<1>{}));
  auto sC = make_tensor(make_smem_ptr(Ashm), sC_layout);

  auto tCsC = thr_mma.partition_C(sC);
  cute::copy(tCrD_half, tCsC);
  ...
  T *gmem_ptr = Dptr + (iy * BM) * n + (ix * BN);
  T *smem_ptr = raw_pointer_cast(sC.data());
  
  constexpr int kVecsPerThread = (BM * BN) / (kThreads * kVecSize);
  
#pragma unroll
  for (int vec_idx = 0; vec_idx < kVecsPerThread; ++vec_idx) {
    int linear_idx = (vec_idx * kThreads + tid) * kVecSize;
    int row = linear_idx / BN;
    int col = linear_idx % BN;
    
    if (row < BM && col + kVecSize <= BN) {
      int smem_offset = row * (BN + kPadding) + col;
      int gmem_offset = row * n + col;
      
      uint4 data = *reinterpret_cast<const uint4*>(smem_ptr + smem_offset);
      *reinterpret_cast<uint4*>(gmem_ptr + gmem_offset) = data;
    }
  }
}
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static constexpr int shm_size_C = BM * (BN + 32);
  ...
  cudaFuncSetAttribute(
      cuda_l2_a100_fp32_kernel<
          ...
          SmemLayoutB, S2RCopyAtomA, S2RCopyAtomB, BlockSwizzle>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);

  cuda_l2_a100_fp32_kernel<
      T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB,
      S2RCopyAtomA, S2RCopyAtomB, BlockSwizzle><<<grid, block, shm_size>>>(a, b, c, M, N, K);
}
