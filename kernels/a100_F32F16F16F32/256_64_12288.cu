...
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
  ...
  int k_start = k_idx * (k / k_split);
  int k_end = (k_idx == k_split - 1) ? k : (k_idx + 1) * (k / k_split);
  int k_per_split = k_end - k_start;
  ...
  T *output_ptr = (k_split > 1) ? 
    (workspace + k_idx * m * n) : Dptr;
  
  Tensor D = make_tensor(make_gmem_ptr(output_ptr), make_shape(m, n),
                         make_stride(n, Int<1>{}));

  Tensor A_shifted = make_tensor(make_gmem_ptr(Aptr + k_start), 
                                 make_shape(m, k_per_split),
                                 make_stride(k, Int<1>{}));
  Tensor B_shifted = make_tensor(make_gmem_ptr(Bptr + k_start),
                                 make_shape(n, k_per_split),
                                 make_stride(k, Int<1>{}));

  Tensor gA = local_tile(A_shifted, make_tile(Int<BM>{}, Int<BK>{}),
                         make_coord(iy, _));
  Tensor gB = local_tile(B_shifted, make_tile(Int<BN>{}, Int<BK>{}),
                         make_coord(ix, _));
...
template <typename T>
__global__ void reduce_splitk_vectorized_kernel(T *workspace, T *output, 
  ...
  int k = 0;
  for (; k + 3 < k_split; k += 4) {
    acc[0] += __half2float(workspace[(k + 0) * total + idx]);
    acc[1] += __half2float(workspace[(k + 1) * total + idx]);
    acc[2] += __half2float(workspace[(k + 2) * total + idx]);
    acc[3] += __half2float(workspace[(k + 3) * total + idx]);
  }
  
  for (; k < k_split; ++k) {
    acc[0] += __half2float(workspace[k * total + idx]);
  }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, T *workspace,
                                                   int M, int N, int K,
                                                   int k_split) {
  using namespace cute;
  ...
  cuda_l2_a100_fp32_kernel<
      ...
      S2GCopyC, BlockSwizzle><<<grid, block, shm_size>>>(a, b, c, workspace, M, N, K, k_split);

  if (k_split > 1) {
    int total = M * N;
    int threads = 256;
    int blocks = (total + threads - 1) / threads;
    reduce_splitk_vectorized_kernel<<<blocks, threads>>>(workspace, c, M, N, k_split);
  }
}
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  half *workspace_ptr = nullptr;
  if (k_split > 1) {
    workspace = torch::empty({k_split, M, N}, torch::dtype(torch::kHalf).device(a.device()));
    workspace_ptr = reinterpret_cast<half *>(workspace.data_ptr());
  }
  
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half, 3, false>(
