...
template <typename T, int BM, int BN, int BK, int kStage, int SplitK, typename TiledMMA,
...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  int k_per_split = (k + SplitK - 1) / SplitK;
  ...
  Tensor B = make_tensor(make_gmem_ptr(Bptr), make_shape(n, k),
                         make_stride(k, Int<1>{}));
  Tensor D = make_tensor(make_gmem_ptr(Dptr), make_shape(m, n, SplitK),
...
reduce_split_k_simple_kernel(T *partial, T *output, int m, int n, int split_k) {
  ...
  for (int k = 0; k < split_k; ++k) {
    sum += __half2float(partial[idx + k * total]);
  }
  
  output[idx] = __float2half(sum);
}

template <typename T, const int Stages = 3, const int SplitK = 8>
void launch_hgemm_optimized(T *a, T *b, T *c, T *workspace, int M, int N, int K) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      cuda_l2_a100_fp32_kernel<
          T, BM, BN, BK, KStage, SplitK, MMA, G2SCopyA, G2SCopyB, SmemLayoutA,
          SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC,
          S2GCopyAtomC, S2GCopyC>,
      cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);

  cuda_l2_a100_fp32_kernel<
      T, BM, BN, BK, KStage, SplitK, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB,
      SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC,
      S2GCopyC><<<grid, block, shm_size>>>(a, b, workspace, M, N, K);

  int threads = 256;
  int blocks = (M * N + threads - 1) / threads;
  reduce_split_k_simple_kernel<<<blocks, threads>>>(workspace, c, M, N, SplitK);
}
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  auto workspace = torch::empty({8, M, N}, torch::dtype(torch::kHalf).device(a.device()));
  
  launch_hgemm_optimized<half, 3, 8>(
      reinterpret_cast<half *>(a.data_ptr()),
      reinterpret_cast<half *>(b_col_major.data_ptr()),
      reinterpret_cast<half *>(c.data_ptr()),
      reinterpret_cast<half *>(workspace.data_ptr()),
      M, N, K);
}