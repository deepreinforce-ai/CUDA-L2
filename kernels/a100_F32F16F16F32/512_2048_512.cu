...
cuda_l2_a100_fp32_kernel(T * __restrict__ Aptr, 
                                  T * __restrict__ Bptr, 
                                  T * __restrict__ Dptr, 
  ...
  if (__builtin_expect(iy * BM >= m || ix * BN >= n, 0))
    return;
  ...
  __builtin_assume(ntile > 0);
  __builtin_assume(ntile <= 64);
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      const bool should_prefetch = __builtin_expect((ik < nk - 1) || (itile < ntile - 1), 1);
      if (should_prefetch) {
      ...
      if (ik == 0) {
        if (__builtin_expect(itile_to_read < ntile, 1)) {
  ...
  const int num_steps = size<1>(tCrC_r2sx);
  ...
  for (int i = 0; i < num_steps; i += step) {
    ...
    if (__builtin_expect(i + step < num_steps, 1)) {
      __syncthreads();
    }
  }
}
