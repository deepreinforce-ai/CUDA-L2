...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 2) cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
                                                  T *Dptr, const int m,
  ...
  const int total_tiles_n = (n + BN - 1) / BN;
  const int total_tiles_m = (m + BM - 1) / BM;
  const int total_tiles = total_tiles_m * total_tiles_n;
  
  for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
    const int ix = tile_idx % total_tiles_n;
    const int iy = tile_idx / total_tiles_n;
...
void launch_hgemm_persistent_cute(T *a, T *b, T *c, int M, int N, int K) {
  ...
  constexpr auto BN = Int<96>{};
  constexpr auto BK = Int<32>{};
  constexpr auto KStage = Int<6>{};
  ...
  constexpr int shm_size = kShmSize;
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_persistent_cute<half>(
