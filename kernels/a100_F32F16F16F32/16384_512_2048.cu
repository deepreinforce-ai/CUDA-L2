...
template<typename T>
__device__ __forceinline__ void direct_reg_to_global_epilogue(
    T* gmem_ptr, 
    const float* reg_acc,
    int num_elements,
    int m_offset,
  ...
  #pragma unroll
  for (int i = 0; i < 8; ++i) {
    temp[i] = __float2half(reg_acc[i]);
  }
  
  int group_id = lane_id / 4;
  int in_group_id = lane_id % 4;
  
  int warp_m = warp_id / 2;
  int warp_n = warp_id % 2;
  
  int base_m = m_offset + warp_m * 32 + (group_id / 2) * 16 + (lane_id / 8) * 8;
  int base_n = n_offset + warp_n * 64 + (group_id % 2) * 32 + in_group_id * 8;
  
  if (base_m + 8 <= m_offset + 128 && base_n + 8 <= n_offset + 128) {
    uint4* vec_ptr = reinterpret_cast<uint4*>(&temp[0]);
    uint4* gmem_vec = reinterpret_cast<uint4*>(gmem_ptr + base_m * N + base_n);
    *gmem_vec = *vec_ptr;
  }
}
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  int shm_size = shm_size_AB * sizeof(T);
