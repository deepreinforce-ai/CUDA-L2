...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
...
#pragma unroll
  for (int i = 0; i < num_elements; i += 16) {
    if (i + 15 < num_elements) {
      float f0 = tCrD(i+0), f1 = tCrD(i+1), f2 = tCrD(i+2), f3 = tCrD(i+3);
      float f4 = tCrD(i+4), f5 = tCrD(i+5), f6 = tCrD(i+6), f7 = tCrD(i+7);
      float f8 = tCrD(i+8), f9 = tCrD(i+9), f10 = tCrD(i+10), f11 = tCrD(i+11);
      float f12 = tCrD(i+12), f13 = tCrD(i+13), f14 = tCrD(i+14), f15 = tCrD(i+15);
      
      half2 h2_0 = __float22half2_rn(make_float2(f0, f1));
      half2 h2_1 = __float22half2_rn(make_float2(f2, f3));
      half2 h2_2 = __float22half2_rn(make_float2(f4, f5));
      half2 h2_3 = __float22half2_rn(make_float2(f6, f7));
      half2 h2_4 = __float22half2_rn(make_float2(f8, f9));
      half2 h2_5 = __float22half2_rn(make_float2(f10, f11));
      half2 h2_6 = __float22half2_rn(make_float2(f12, f13));
      half2 h2_7 = __float22half2_rn(make_float2(f14, f15));
      
      *reinterpret_cast<half2*>(&tCrD_half(i+0)) = h2_0;
      *reinterpret_cast<half2*>(&tCrD_half(i+2)) = h2_1;
      *reinterpret_cast<half2*>(&tCrD_half(i+4)) = h2_2;
      *reinterpret_cast<half2*>(&tCrD_half(i+6)) = h2_3;
      *reinterpret_cast<half2*>(&tCrD_half(i+8)) = h2_4;
      *reinterpret_cast<half2*>(&tCrD_half(i+10)) = h2_5;
      *reinterpret_cast<half2*>(&tCrD_half(i+12)) = h2_6;
      *reinterpret_cast<half2*>(&tCrD_half(i+14)) = h2_7;
    } else {
#pragma unroll
      for (int j = i; j < num_elements; ++j) {
        tCrD_half(j) = __float2half_rn(tCrD(j));
      }
      break;
    }
  }
...
template <typename T, const int Stages = 4, const bool BlockSwizzle = false>
