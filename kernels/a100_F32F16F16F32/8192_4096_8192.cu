...
__device__ __forceinline__ void convert_float4_to_half4(const float* src, half* dst) {
  ...
  *reinterpret_cast<half2*>(dst) = h2_0;
  *reinterpret_cast<half2*>(dst + 2) = h2_1;
}
...
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
  ...
  int num_vec = size(tCrD) / VecSize;
  
  #pragma unroll
  for (int i = 0; i < num_vec; ++i) {
    if (i * VecSize + 3 < size(tCrD)) {
      convert_float4_to_half4(&tCrD(i * VecSize), &tCrD_half(i * VecSize));
    }
    if (i * VecSize + 7 < size(tCrD)) {
      convert_float4_to_half4(&tCrD(i * VecSize + 4), &tCrD_half(i * VecSize + 4));
    }
  }
  
  #pragma unroll
  for (int i = num_vec * VecSize; i < size(tCrD); ++i) {
  ...
  for (int batch = 0; batch < num_batches; ++batch) {
    int offset = batch * step;
    int actual_step = min(step, (int)size<1>(tCrC_r2sx) - offset);
    
    #pragma unroll
    for (int j = 0; j < actual_step; ++j) {
      if (offset + j < size<1>(tCrC_r2sx)) {
        cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, offset + j), tCsC_r2s(_, 0, 0, j));
      }
    }
    __syncthreads();
    ...
    for (int j = 0; j < actual_step; ++j) {
      if (offset + j < size<1>(tCrC_r2sx)) {
        cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), tCgC_s2gx(_, offset + j));
      }
    }
