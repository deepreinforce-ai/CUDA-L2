...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik + 2 < nk) {
        int ik_prefetch = (ik + 2) % nk;
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch, ismem_read), tCrA_view(_, _, ik_prefetch));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch, ismem_read), tCrB_view(_, _, ik_prefetch));
      } else {
