...
#pragma unroll
  for (int i = 0; i < num_elems / 2; ++i) {
    float2 fp32_pair = make_float2(tCrD(2*i), tCrD(2*i + 1));
    __half2 fp16_pair = __float22half2_rn(fp32_pair);
    *reinterpret_cast<__half2*>(&tCrD_half(2*i)) = fp16_pair;
  }
  if (num_elems % 2 == 1) {
    tCrD_half(num_elems - 1) = __float2half_rn(tCrD(num_elems - 1));
  }
