...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  TORCH_CHECK(a.dtype() == torch::kHalf, "Tensor a must be FP16");
  TORCH_CHECK(b.dtype() == torch::kHalf, "Tensor b must be FP16");
  TORCH_CHECK(c.dtype() == torch::kHalf, "Tensor c must be FP16");
  
  const int M = a.size(0);
  const int K = a.size(1);
  const int N = b.size(1);
  
  TORCH_CHECK(a.size(1) == K, "Dimension mismatch");
  TORCH_CHECK(b.size(0) == K, "Dimension mismatch");
  TORCH_CHECK(c.size(0) == M && c.size(1) == N, "Output dimension mismatch");
