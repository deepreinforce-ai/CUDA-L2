...
#pragma unroll
  ...
  constexpr int kEpilogueSteps = decltype(size<1>(tCrC_r2sx))::value;

  int warp_id = idx / 32;
  int lane_id = idx % 32;
  
  int write_offset = (lane_id * 8 + warp_id * 2) % 32;

#pragma unroll
  for (int i = 0; i < kEpilogueSteps; ++i) {
    cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, i), tCsC_r2s(_, 0, 0, 0));
    __syncthreads();
    
    cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, 0), tCgC_s2gx(_, i));
    __syncthreads();
  }
}
