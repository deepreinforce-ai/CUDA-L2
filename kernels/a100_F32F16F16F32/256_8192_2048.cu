...
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
  ...
  auto tCrA_prefetch = make_fragment_like(tCrA);
  auto tCrB_prefetch = make_fragment_like(tCrB);
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik < nk - 2) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next2, ismem_read),
                   tCrA_prefetch_view(_, _, ik_next2));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next2, ismem_read),
                   tCrB_prefetch_view(_, _, ik_next2));
      } else if (ik == nk - 2 && itile < ntile - 1) {
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory exceeds A's single pipe");
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half, 5, true>(
