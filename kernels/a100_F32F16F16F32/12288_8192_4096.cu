...
__device__ __forceinline__ void fast_f32_to_f16x2(const float* src, half* dst) {
  uint32_t result;
  asm volatile(
  ...
  *reinterpret_cast<uint32_t*>(dst) = result;
}
...
#pragma unroll
  ...
  const int num_elem = size(tCrD);
  const int vec16_iters = num_elem / 16;
  const int remainder = num_elem % 16;
  
  float* tCrD_ptr = reinterpret_cast<float*>(&tCrD(0));
  half* tCrD_half_ptr = reinterpret_cast<half*>(&tCrD_half(0));
  
#pragma unroll
  for (int i = 0; i < vec16_iters; ++i) {
    int base = i * 16;
    
    fast_f32_to_f16x2(&tCrD_ptr[base + 0], &tCrD_half_ptr[base + 0]);
    fast_f32_to_f16x2(&tCrD_ptr[base + 2], &tCrD_half_ptr[base + 2]);
    fast_f32_to_f16x2(&tCrD_ptr[base + 4], &tCrD_half_ptr[base + 4]);
    fast_f32_to_f16x2(&tCrD_ptr[base + 6], &tCrD_half_ptr[base + 6]);
    fast_f32_to_f16x2(&tCrD_ptr[base + 8], &tCrD_half_ptr[base + 8]);
    fast_f32_to_f16x2(&tCrD_ptr[base + 10], &tCrD_half_ptr[base + 10]);
    fast_f32_to_f16x2(&tCrD_ptr[base + 12], &tCrD_half_ptr[base + 12]);
    fast_f32_to_f16x2(&tCrD_ptr[base + 14], &tCrD_half_ptr[base + 14]);
  }
  
  int base = vec16_iters * 16;
  for (int i = 0; i < remainder / 2; ++i) {
    fast_f32_to_f16x2(&tCrD_ptr[base + i * 2], &tCrD_half_ptr[base + i * 2]);
  }
  
  if (remainder & 1) {
    tCrD_half(num_elem - 1) = static_cast<T>(tCrD(num_elem - 1));
  }
