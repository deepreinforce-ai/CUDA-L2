...
template <typename T>
void launch_hgemm_ultrafine_optimized_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(BM * BK >= size(SmemLayoutC{}),
                "C shared memory request is larger than A");
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_ultrafine_optimized_cute<half>(
