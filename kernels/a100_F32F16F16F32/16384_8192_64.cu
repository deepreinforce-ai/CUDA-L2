...
template <typename T, int BM, int BN, int BK, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, 
                                                  int m, int n, int k) {
...
#pragma unroll
  ...
  int num_iterations = (size<1>(tCrC_r2sx) + step - 1) / step;

#pragma unroll
  for (int i = 0; i < num_iterations; ++i) {
    int start_idx = i * step;
    int end_idx = min(start_idx + step, (int)size<1>(tCrC_r2sx));
    int actual_step = end_idx - start_idx;

#pragma unroll
    for (int j = 0; j < actual_step; ++j) {
      cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, start_idx + j), tCsC_r2s(_, 0, 0, j));
    }
...
#pragma unroll
    for (int j = 0; j < actual_step; ++j) {
      cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), tCgC_s2gx(_, start_idx + j));
    }
...
template <typename T>
void launch_hgemm_k64_optimized(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  shm_size = max(shm_size, (int)(cute::cosize(SmemLayoutC{}) * sizeof(T)));

  cudaFuncSetAttribute(
      cuda_l2_a100_fp32_kernel<T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB, 
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_k64_optimized<half>(
