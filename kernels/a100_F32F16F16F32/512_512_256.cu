...
cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  constexpr int A_smem_stride = BK + 8;
  constexpr int B_smem_stride = BK + 8;
  
  T *Ashm = shm_data;
  T *Bshm = shm_data + BM * A_smem_stride * kStage;
  ...
  auto sA = make_tensor(make_smem_ptr(Ashm), 
                        ...
                        make_stride(Int<A_smem_stride>{}, Int<1>{}, Int<BM * A_smem_stride>{}));
  auto sB = make_tensor(make_smem_ptr(Bshm),
                        make_shape(Int<BN>{}, Int<BK>{}, Int<kStage>{}),
                        make_stride(Int<B_smem_stride>{}, Int<1>{}, Int<BN * B_smem_stride>{}));
  ...
  auto tAsA_read = s2r_thr_copy_a.partition_S(sA);
  auto tCrA_view = s2r_thr_copy_a.retile_D(tCrA);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB_read = s2r_thr_copy_b.partition_S(sB);
...
#pragma unroll
  ...
  int ik = 0;
  cute::copy(s2r_tiled_copy_a, tAsA_read(_, _, ik, ismem_read), tCrA_view(_, _, ik));
  cute::copy(s2r_tiled_copy_b, tBsB_read(_, _, ik, ismem_read), tCrB_view(_, _, ik));
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      cute::copy(s2r_tiled_copy_a, tAsA_read(_, _, ik_next, ismem_read),
                 tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB_read(_, _, ik_next, ismem_read),
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static constexpr int shm_size_AB = 
      (BM * A_smem_stride * KStage + BN * B_smem_stride * KStage) * sizeof(T);
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  LAUNCH_HGEMM_MMA_STAGES_CUTE_SWIZZLE_TN(4, 1024);
}