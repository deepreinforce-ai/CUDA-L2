...
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
  ...
  if (block_id >= total_tiles) return;
  
  int iy = block_id / num_tiles_n;
  int ix = block_id % num_tiles_n;
...
#pragma unroll
  ...
  if (nk > 3) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 3, ismem_read), 
               tCrA_view(_, _, 3));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 3, ismem_read), 
               tCrB_view(_, _, 3));
  }
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik + 2 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 2, ismem_read),
                   tCrA_view(_, _, ik + 2));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 2, ismem_read),
                   tCrB_view(_, _, ik + 2));
      } else if (ik == nk - 2 && itile + 1 < ntile) {
        int next_stage = (ismem_read + 1) % kStage;
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, next_stage),
                   tCrA_view(_, _, 0));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, next_stage),
                   tCrB_view(_, _, 0));
      }

      if (ik + 3 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 3, ismem_read),
                   tCrA_view(_, _, ik + 3));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 3, ismem_read),
                   tCrB_view(_, _, ik + 3));
      } else if (ik == nk - 3 && itile + 1 < ntile && nk > 1) {
        int next_stage = (ismem_read + 1) % kStage;
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, next_stage),
                   tCrA_view(_, _, 0));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, next_stage),
...
void launch_hgemm_mma_stages_persistent_tn_cute(T *a, T *b, T *c, int M,
                                                int N, int K) {
  using namespace cute;
  ...
  cuda_l2_a100_fp32_kernel<
      ...
      S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K, num_tiles_m, num_tiles_n);
}
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_mma_stages_persistent_tn_cute<half, 7>(
