...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    ...
    if (ik < nk - 1) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next), tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next), tCrB_view(_, _, ik_next));
    }
...
template <typename T>
void launch_hgemm_single_tile_k(T *a, T *b, T *c, int M, int N, int K,
                                int swizzle_stride) {
  using namespace cute;
...
void cuda_l2_a100_fp32(torch::Tensor a, torch::Tensor b,
  ...
  constexpr int swizzle_stride = 8192;

  launch_hgemm_single_tile_k<half>(
