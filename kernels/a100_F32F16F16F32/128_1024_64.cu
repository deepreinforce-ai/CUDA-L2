...
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
  ...
  int prefetch_tiles = cute::min(kStage - 1, ntile);
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik_next != 0 || itile + 1 < ntile) {
        int next_smem_read = (ik_next == 0) ? (ismem_read + 1) % kStage : ismem_read;
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, next_smem_read),
                   tCrA_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, next_smem_read),
