...
__global__ void cuda_l2_a100_fp32_kernel(T *Aptr, T *Bptr,
  ...
  T *Cshm = shm_data + cute::cosize(SmemLayoutA{}) + cute::cosize(SmemLayoutB{});
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static constexpr int shm_size_ABC =
      cute::cosize(SmemLayoutA{}) + cute::cosize(SmemLayoutB{}) + cute::cosize(SmemLayoutC{});
  static constexpr int kShmSize = shm_size_ABC * sizeof(T);
