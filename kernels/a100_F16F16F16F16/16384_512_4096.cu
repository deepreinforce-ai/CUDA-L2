...
__global__ void cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr,
  ...
  const int total_blocks = 216;
  int block_id = blockIdx.x;
  
  for (int tile_id = block_id; tile_id < total_tiles; tile_id += total_blocks) {
    int ix = tile_id % total_tiles_x;
    int iy = tile_id / total_tiles_x;
...
#pragma unroll
    ...
    if (tile_id + total_blocks < total_tiles) {
      __syncthreads();
    }
  }
}
