...
__global__ void __launch_bounds__(128, 2) cuda_l2_a100_fp16_kernel(
  ...
  for (int ik = 0; ik < nk; ik += 2) {
    const int ik_next0 = ik + 4;
    const int ik_next1 = ik + 5;
    
    if (ik_next0 < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next0, 0), tCrA_view(_, _, ik_next0));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next0, 0), tCrB_view(_, _, ik_next0));
    }
    
    cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik), tCrB(_, _, ik), tCrD);
    
    if (ik_next1 < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next1, 0), tCrA_view(_, _, ik_next1));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next1, 0), tCrB_view(_, _, ik_next1));
    }
  ...
  for (int ik = 0; ik < nk; ik += 2) {
    ...
    if (ik_next0 < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next0, 1), tCrA_view(_, _, ik_next0));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next0, 1), tCrB_view(_, _, ik_next0));
    }
    
    cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik), tCrB(_, _, ik), tCrD);
    
    if (ik_next1 < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next1, 1), tCrA_view(_, _, ik_next1));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next1, 1), tCrB_view(_, _, ik_next1));
    }
