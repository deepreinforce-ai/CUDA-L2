    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik < nk - 1) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_prefetch_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_prefetch_view(_, _, ik_next));
      }
      ...
      if (ik == nk - 1) {
        ...
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, ismem_read),
                   tCrA_prefetch_view(_, _, 0));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, ismem_read),
                   tCrB_prefetch_view(_, _, 0));
      }
      ...
      if (ik < nk - 1) {
        ...
        cute::copy(tCrA_prefetch(_, _, 0), tCrA(_, _, 0));
        cute::copy(tCrB_prefetch(_, _, 0), tCrB(_, _, 0));
      }
    }
  }
