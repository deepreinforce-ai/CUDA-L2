...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(64, 3) cuda_l2_a100_fp16_kernel(
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      int ismem_prefetch = ismem_read;
      
      if (ik_prefetch >= nk) {
        ik_prefetch -= nk;
        ismem_prefetch = (ismem_read + 1) % kStage;
      }
      ...
      if (itile < ntile - 1 || ik_prefetch < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch, ismem_prefetch),
                   tCrA_view(_, _, ik_prefetch));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch, ismem_prefetch),
