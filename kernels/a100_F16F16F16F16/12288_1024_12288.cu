...
__global__ void __launch_bounds__(128, 2) cuda_l2_a100_fp16_kernel(
  ...
  if (nk >= 1) cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, ismem_read), tCrA_view(_, _, 0));
  if (nk >= 2) cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, ismem_read), tCrA_view(_, _, 1));
  if (nk >= 3) cute::copy(s2r_tiled_copy_a, tAsA(_, _, 2, ismem_read), tCrA_view(_, _, 2));
  if (nk >= 4) cute::copy(s2r_tiled_copy_a, tAsA(_, _, 3, ismem_read), tCrA_view(_, _, 3));
  
  if (nk >= 1) cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, ismem_read), tCrB_view(_, _, 0));
  if (nk >= 2) cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, ismem_read), tCrB_view(_, _, 1));
  if (nk >= 3) cute::copy(s2r_tiled_copy_b, tBsB(_, _, 2, ismem_read), tCrB_view(_, _, 2));
  if (nk >= 4) cute::copy(s2r_tiled_copy_b, tBsB(_, _, 3, ismem_read), tCrB_view(_, _, 3));
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ik += 4) {
      ...
      if (ik + 4 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 4, ismem_read),
                   tCrA_view(_, _, ik + 4));
      }
      ...
      if (ik + 4 < nk) {
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 4, ismem_read),
                   tCrB_view(_, _, ik + 4));
      }
      
      if (ik + 1 < nk) {
        if (ik + 5 < nk) {
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 5, ismem_read),
                     tCrB_view(_, _, ik + 5));
        }
        
        cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik + 1), tCrB(_, _, ik + 1), tCrD);
        
        if (ik + 5 < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 5, ismem_read),
                     tCrA_view(_, _, ik + 5));
        }
      }
      ...
      if (ik + 2 < nk) {
        if (ik + 6 < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 6, ismem_read),
                     tCrA_view(_, _, ik + 6));
        }
        
        cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik + 2), tCrB(_, _, ik + 2), tCrD);
        
        if (ik + 6 < nk) {
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 6, ismem_read),
                     tCrB_view(_, _, ik + 6));
        }
      }
      ...
      if (ik + 3 < nk) {
        if (ik + 7 < nk) {
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 7, ismem_read),
                     tCrB_view(_, _, ik + 7));
        }
        
        cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik + 3), tCrB(_, _, ik + 3), tCrD);
        
        if (ik + 7 < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 7, ismem_read),
                     tCrA_view(_, _, ik + 7));
        }
      }
      
      if (ik >= nk - 4 && itile < ntile - 1) {
        cp_async_wait<kStage - 2>();
        __syncthreads();
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half>(
      ...
      reinterpret_cast<half *>(c.data_ptr()), M, N, K, swizzle_stride);
}