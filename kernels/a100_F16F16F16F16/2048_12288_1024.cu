...
__global__ void __launch_bounds__(128, 2) cuda_l2_a100_fp16_kernel(
  ...
  auto tCrA_buf = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_buf = thr_mma.partition_fragment_B(gB(_, _, 0));
  ...
  auto tCrA_view_buf = s2r_thr_copy_a.retile_D(tCrA_buf);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_view_buf = s2r_thr_copy_b.retile_D(tCrB_buf);
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik % 2 == 0) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_view_buf(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_view_buf(_, _, ik_next));
        ...
        cute::gemm(tiled_mma, tCrD, tCrA_buf(_, _, ik), tCrB_buf(_, _, ik), tCrD);
      }
    }
  }
