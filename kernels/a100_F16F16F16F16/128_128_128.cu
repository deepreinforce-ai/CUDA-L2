...
void launch_hgemm_ultra_fine(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;
...
#define LAUNCH_HGEMM_ULTRA_FINE_SWIZZLE(stages, stride) \
  launch_hgemm_ultra_fine<half, (stages), true>( \
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, 
  ...
  LAUNCH_HGEMM_ULTRA_FINE_SWIZZLE(3, swizzle_stride);
}