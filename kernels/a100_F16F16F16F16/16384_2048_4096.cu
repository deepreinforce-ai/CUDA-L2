...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m,
                                   int n, int k) {
...
#pragma unroll
  ...
  int prefetch_depth = min(6, (int)nk);
  
#pragma unroll
  for (int i = 0; i < prefetch_depth; ++i) {
    if (i < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, i, ismem_read), tCrA_view(_, _, i));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, i, ismem_read), tCrB_view(_, _, i));
    }
  }
