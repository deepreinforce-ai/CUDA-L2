...
#pragma unroll
  ...
  constexpr int kEpilogueBatch = 2;
  int num_batches = (size<1>(tCrC_r2sx) + kEpilogueBatch - 1) / kEpilogueBatch;
  
#pragma unroll
  for (int batch = 0; batch < num_batches; ++batch) {
    int i_start = batch * kEpilogueBatch;
    int i_end = min(i_start + kEpilogueBatch, (int)size<1>(tCrC_r2sx));
