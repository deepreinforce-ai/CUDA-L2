...
__global__ void __launch_bounds__(128, 2)
  ...
  for (int tile_idx = blockIdx.y * gridDim.x + blockIdx.x; 
       tile_idx < total_tiles; 
       tile_idx += gridDim.x * gridDim.y) {
    
    int ix = tile_idx % total_tiles_x;
    int iy = tile_idx / total_tiles_x;
    
    if (BlockSwizzle) {
      int swizzle_group = ix / 16;
      int swizzle_offset = ix % 16;
      ix = swizzle_group * 16 + swizzle_offset;
    }
