...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    ...
    if (ik < nk - 1) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, 0), tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, 0), tCrB_view(_, _, ik_next));
    }
...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    ...
    if (ik < nk - 1) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, 1), tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, 1), tCrB_view(_, _, ik_next));
    }
...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    ...
    if (ik < nk - 1) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, 2), tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, 2), tCrB_view(_, _, ik_next));
    }
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half>(
      ...
      reinterpret_cast<half *>(c.data_ptr()), M, N, K, 3072);
}