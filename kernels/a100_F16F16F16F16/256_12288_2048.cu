...
template <typename T, const int Stages = 5, const bool BlockSwizzle = false>
void launch_hgemm_optimized_cute(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;
...
#define LAUNCH_HGEMM_OPTIMIZED_CUTE_SWIZZLE(stages, stride)                    \
  launch_hgemm_optimized_cute<half, (stages), true>(                           \
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  LAUNCH_HGEMM_OPTIMIZED_CUTE_SWIZZLE(5, swizzle_stride);
}