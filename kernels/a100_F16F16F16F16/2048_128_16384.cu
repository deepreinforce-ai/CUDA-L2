...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k,
                                   int total_tiles_m, int total_tiles_n) {
  ...
  const int tiles_per_block = (total_tiles_m * total_tiles_n + gridDim.x - 1) / gridDim.x;
  const int block_start_tile = blockIdx.x * tiles_per_block;
  const int block_end_tile = min(block_start_tile + tiles_per_block, total_tiles_m * total_tiles_n);

  for (int tile_id = block_start_tile; tile_id < block_end_tile; tile_id++) {
    int iy = tile_id / total_tiles_n;
    int ix = tile_id % total_tiles_n;
...
template <typename T, const int Stages = 9>
void launch_hgemm_persistent_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  int total_tiles_m = (M + BM - 1) / BM;
  int total_tiles_n = (N + BN - 1) / BN;
  ...
  cuda_l2_a100_fp16_kernel<
      ...
      S2GCopyC><<<grid, block, shm_size>>>(a, b, c, M, N, K, total_tiles_m, total_tiles_n);
}
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_persistent_tn_cute<half, 9>(
