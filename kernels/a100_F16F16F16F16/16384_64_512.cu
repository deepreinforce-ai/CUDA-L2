...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_double = make_fragment_like(tCrA);
  auto tCrB_double = make_fragment_like(tCrB);
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (use_double) {
        ...
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_view_double(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_view_double(_, _, ik_next));
      }
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_optimized_cute<half, 3>(
