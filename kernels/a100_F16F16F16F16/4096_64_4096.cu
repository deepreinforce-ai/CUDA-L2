...
#pragma unroll
  ...
  if (ik + 1 < nk) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 1, ismem_read), tCrA_view(_, _, ik + 1));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 1, ismem_read), tCrB_view(_, _, ik + 1));
  }
...
template <typename T, const int Stages = 8, const bool BlockSwizzle = true>
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  int swizzle_stride = 320;
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half, 8, true>(
