...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k, int total_tiles) {
  ...
  int tiles_per_block = (total_tiles + gridDim.x * gridDim.y * gridDim.z - 1) / (gridDim.x * gridDim.y * gridDim.z);
  int block_id = ((int)BlockSwizzle) * blockIdx.z * gridDim.x * gridDim.y + blockIdx.y * gridDim.x + blockIdx.x;
  int start_tile = block_id * tiles_per_block;
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  int BX = (N + BN - 1) / BN;
  int BY = (M + BM - 1) / BM;
  int total_tiles = BX * BY;
  
  int BZ = BlockSwizzle ? (N + (swizzle_stride)-1) / (swizzle_stride) : 1;
  BX = BlockSwizzle ? (BX + BZ - 1) / BZ : BX;
  
  int max_blocks = 192;
  int actual_blocks = min(BX * BY * BZ, max_blocks);
  
  dim3 block(size(MMA{}));
  dim3 grid(actual_blocks, 1, 1);
  ...
  cuda_l2_a100_fp16_kernel<
      ...
      S2GCopyC, BlockSwizzle><<<grid, block, shm_size>>>(a, b, c, M, N, K, total_tiles);
}
