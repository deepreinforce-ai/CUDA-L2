...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    ...
    if (ik_prefetch < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch), tCrA_view(_, _, ik_prefetch));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch), tCrB_view(_, _, ik_prefetch));
    }
...
template <typename T>
void launch_hgemm_optimized_bk64(T *a, T *b, T *c, int M, int N, int K,
                                  int swizzle_stride) {
  using namespace cute;
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  int swizzle_stride = 6144;

  launch_hgemm_optimized_bk64<half>(
