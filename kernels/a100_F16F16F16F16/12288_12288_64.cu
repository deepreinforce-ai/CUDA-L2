...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto A = make_tensor(make_gmem_ptr(Aptr), make_shape(m, k),
                       make_stride(k, Int<1>{}));
  auto B = make_tensor(make_gmem_ptr(Bptr), make_shape(n, k),
                       make_stride(k, Int<1>{}));
  auto D = make_tensor(make_gmem_ptr(Dptr), make_shape(m, n),
                       make_stride(n, Int<1>{}));

  auto gA = local_tile(A, make_tile(Int<BM>{}, Int<BK>{}),
                       make_coord(iy, _));
  auto gB = local_tile(B, make_tile(Int<BN>{}, Int<BK>{}),
                       make_coord(ix, _));
  auto gD = local_tile(D, make_tile(Int<BM>{}, Int<BN>{}),
                       make_coord(iy, ix));
