...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  if (nk > 1) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, ismem_read), tCrA_prefetch_view(_, _, 0));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, ismem_read), tCrB_prefetch_view(_, _, 0));
  }
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik + 2 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next2, ismem_read),
                   tCrA_prefetch_view(_, _, ik_next2 % size<2>(tCrA_prefetch)));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next2, ismem_read),
                   tCrB_prefetch_view(_, _, ik_next2 % size<2>(tCrB_prefetch)));
      }
