...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  cute::copy(g2s_tiled_copy_a, tAgA, tAsA);
  cute::copy(g2s_tiled_copy_b, tBgB, tBsB);
  cp_async_fence();
...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    int ik_next = ik + 1;
    if (ik_next < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA_load(_, _, ik_next), tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB_load(_, _, ik_next), tCrB_view(_, _, ik_next));
    }
...
template <typename T>
void launch_hgemm_optimized_single_pass(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_single_pass<half>(
