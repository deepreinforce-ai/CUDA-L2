...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  #pragma unroll
  for (int ik_pre = 0; ik_pre < cute::min(10, nk); ++ik_pre) {
    cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, ik_pre), tCrA_view(_, _, ik_pre));
    cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, ik_pre), tCrB_view(_, _, ik_pre));
  }

  #pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    int ik_prefetch = ik + 10;
    if (ik_prefetch < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA_s2r(_, _, ik_prefetch), tCrA_view(_, _, ik_prefetch));
      cute::copy(s2r_tiled_copy_b, tBsB_s2r(_, _, ik_prefetch), tCrB_view(_, _, ik_prefetch));
    }
...
template <typename T>
void launch_hgemm_single_tile_cute(T *a, T *b, T *c, int M, int N, int K,
                                   int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory request is larger than A's buffer");
  ...
  cudaFuncSetAttribute(
      cuda_l2_a100_fp16_kernel<
          T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB, SmemLayoutA,
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_single_tile_cute<half>(
