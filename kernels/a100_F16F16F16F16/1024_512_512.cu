...
#pragma unroll
    for (int ik = 0; ik < nk; ik += 3) {
      int ik_next0 = (ik + 3) % nk;
      int ik_next1 = (ik + 4) % nk;
      int ik_next2 = (ik + 5) % nk;
      ...
      if (ik + 3 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next0, ismem_read),
                   tCrA_view(_, _, ik_next0));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next0, ismem_read),
      ...
      if (ik + 4 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next1, ismem_read),
                   tCrA_view(_, _, ik_next1));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next1, ismem_read),
                   tCrB_view(_, _, ik_next1));
      }
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  LAUNCH_HGEMM_MMA_STAGES_CUTE_SWIZZLE_TN(6, swizzle_stride);
}