...
__global__ void __launch_bounds__(128, 2)
  ...
  int tiles_per_block = (num_n_tiles + gridDim.x - 1) / gridDim.x;
  int n_tile_start = block_id * tiles_per_block;
  int n_tile_end = min(n_tile_start + tiles_per_block, num_n_tiles);
  ...
  for (int n_tile_idx = n_tile_start; n_tile_idx < n_tile_end; ++n_tile_idx) {
    if (n_tile_idx * BN >= n)
      continue;
    ...
    Tensor gB = local_tile(B, make_tile(Int<BN>{}, Int<BK>{}),
                          make_coord(n_tile_idx, _));
    Tensor gD = local_tile(D, make_tile(Int<BM>{}, Int<BN>{}),
                          make_coord(0, n_tile_idx));
...
void launch_hgemm_persistent_cute(T *a, T *b, T *c, int M, int N, int K) {
  ...
  dim3 block(size(MMA{}));
  dim3 grid(min(216, max(108, num_n_tiles)));
  ...
  cudaFuncSetAttribute(
      cuda_l2_a100_fp16_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA,
  ...
  cuda_l2_a100_fp16_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA,
                                    ...
                                    SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB,
                                    R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>
      <<<grid, block, kShmSize>>>(a, b, c, M, N, K, num_n_tiles);
}
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_persistent_cute<half, 7>(
