...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr,
  ...
  for (int i = 0; i < num_iters; i += step) {
    ...
    for (int j = 0; j < step; ++j) {
      int idx_r2s = i + j;
      if (idx_r2s < num_iters) {
        cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, idx_r2s), tCsC_r2s(_, 0, 0, j));
      }
    }
    ...
    for (int j = 0; j < step; ++j) {
      int idx_s2g = i + j;
      if (idx_s2g < num_iters) {
        cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, 0, 0, j), tCgC_s2gx(_, idx_s2g));
      }
    }
