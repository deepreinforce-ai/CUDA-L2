...
cuda_l2_a100_fp16_kernel(const T * __restrict__ Aptr,
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    const int ismem = itile & 1;
    
    if (itile + 1 < ntile) {
      const int next_stage = (itile + 1) & 1;
      cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, itile + 1),
                 tAsA_copy(_, _, _, next_stage));
      cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, itile + 1),
                 tBsB_copy(_, _, _, next_stage));
      cp_async_fence();
    }

    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, ismem), tCrA_view(_, _, 0));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, ismem), tCrB_view(_, _, 0));
    
    cute::gemm(tiled_mma, tCrD, tCrA(_, _, 0), tCrB(_, _, 0), tCrD);
    
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, ismem), tCrA_view(_, _, 1));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, ismem), tCrB_view(_, _, 1));
    
    if (itile + 1 < ntile) {
      cp_async_wait<0>();
      __syncthreads();
      
      const int next_ismem = (itile + 1) & 1;
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, next_ismem), tCrA_view(_, _, 0));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, next_ismem), tCrB_view(_, _, 0));
    }
  ...
  const int total_steps = size<1>(tCrC_r2sx);
  
  #pragma unroll
  for (int i = 0; i < total_steps; i += step) {
    #pragma unroll
