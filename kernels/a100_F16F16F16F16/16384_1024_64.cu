...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_tile0 = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_tile0 = thr_mma.partition_fragment_B(gB(_, _, 0));
  auto tCrA_tile1 = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_tile1 = thr_mma.partition_fragment_B(gB(_, _, 0));
  ...
  auto tCrA_tile0_view = s2r_thr_copy_a.retile_D(tCrA_tile0);
  auto tCrA_tile1_view = s2r_thr_copy_a.retile_D(tCrA_tile1);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_tile0_view = s2r_thr_copy_b.retile_D(tCrB_tile0);
  auto tCrB_tile1_view = s2r_thr_copy_b.retile_D(tCrB_tile1);
  ...
  int nk = size<2>(tCrA_tile0);

#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, 0), tCrA_tile0_view(_, _, ik));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, 0), tCrB_tile0_view(_, _, ik));
  }
...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, 1), tCrA_tile1_view(_, _, ik));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, 1), tCrB_tile1_view(_, _, ik));
  }

#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    cute::gemm(tiled_mma, tCrD, tCrA_tile0(_, _, ik), tCrB_tile0(_, _, ik), tCrD);
  }

#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    cute::gemm(tiled_mma, tCrD, tCrA_tile1(_, _, ik), tCrB_tile1(_, _, ik), tCrD);
  }
