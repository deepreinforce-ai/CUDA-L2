...
template <typename T>
void launch_hgemm_reinforcement_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  auto BK = Int<32>{};
  auto KStage = Int<3>{};
  constexpr bool BlockSwizzle = true;
  int swizzle_stride = N;
  ...
  static constexpr int shm_size_AB =
      cute::cosize_v<SmemLayoutA> + cute::cosize_v<SmemLayoutB>;
  static constexpr int shm_size_C = cute::cosize_v<SmemLayoutC>;
  static constexpr int kShmSize =
      cute::max(shm_size_AB, shm_size_C) * sizeof(T);
  
  static_assert(kShmSize == 49152, "Shared memory size calculation is incorrect for 3-wave occupancy config");
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_reinforcement_cute<half>(
