...
__global__ void cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr,
  ...
  T *Cshm = Bshm + cute::cosize(SmemLayoutB{});
...
void launch_hgemm_optimized_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  ...
  using SmemLayoutAtom = decltype(composition(
      Swizzle<4, 3, 4>{}, make_layout(make_shape(Int<8>{}, Int<BK>{}),
  ...
  static constexpr int kShmSize = (shm_size_AB + shm_size_C) * sizeof(T);
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_optimized_tn_cute<half, 4>(
