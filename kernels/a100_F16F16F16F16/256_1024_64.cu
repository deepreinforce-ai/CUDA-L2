...
#pragma unroll
  ...
  auto sC = make_tensor(sA(_, _, ismem).data(), SmemLayoutC{});
...
template <typename T, const int Stages = 1>
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_tn_cute<half, 1>(
