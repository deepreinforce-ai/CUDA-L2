...
__global__ void __launch_bounds__(128)
  ...
  auto tCrA_view0 = s2r_thr_copy_a.retile_D(tCrA_buf0);
  auto tCrA_view1 = s2r_thr_copy_a.retile_D(tCrA_buf1);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view0 = s2r_thr_copy_b.retile_D(tCrB_buf0);
  auto tCrB_view1 = s2r_thr_copy_b.retile_D(tCrB_buf1);
...
#pragma unroll
  ...
  if (nk > 0) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, ismem_read), tCrA_view0(_, _, 0));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, ismem_read), tCrB_view0(_, _, 0));
  }
  
  if (nk > 1) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, ismem_read), tCrA_view1(_, _, 1));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, ismem_read), tCrB_view1(_, _, 1));
  }
      ...
      if (use_buf0) {
        if (ik_next < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                     tCrA_view1(_, _, ik_next));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                     tCrB_view1(_, _, ik_next));
        }
      } else {
        if (ik_next < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                     tCrA_view0(_, _, ik_next));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                     tCrB_view0(_, _, ik_next));
        }
      }
...
template <typename T, const int Stages = 6, const bool BlockSwizzle = false>
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  LAUNCH_HGEMM_MMA_STAGES_CUTE_NO_SWIZZLE_TN(6)
}