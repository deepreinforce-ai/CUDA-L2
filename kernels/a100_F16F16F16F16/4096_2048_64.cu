...
template <typename T, int BM, int BN, int BK, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(256, 4) cuda_l2_a100_fp16_kernel(
  ...
  if (nk > 0) {
    cute::copy(s2r_tiled_copy_a, tAsA_load(_, _, 0), tCrA_view(_, _, 0));
    cute::copy(s2r_tiled_copy_b, tBsB_load(_, _, 0), tCrB_view(_, _, 0));
  }
  
  #pragma unroll
  for (int ik = 0; ik < nk - 1; ++ik) {
    cute::copy(s2r_tiled_copy_a, tAsA_load(_, _, ik + 1), tCrA_view(_, _, ik + 1));
    cute::copy(s2r_tiled_copy_b, tBsB_load(_, _, ik + 1), tCrB_view(_, _, ik + 1));
...
template <typename T>
void launch_hgemm_higher_occupancy_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >= size(SmemLayoutC{}),
                "C shared memory exceeds A allocation");
  ...
  cuda_l2_a100_fp16_kernel<T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB,
      SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB,
      R2SCopyAtomC, S2GCopyAtomC, S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_higher_occupancy_tn_cute<half>(
