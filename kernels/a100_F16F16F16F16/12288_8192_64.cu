...
__global__ void __launch_bounds__(256, 2) cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr,
                                                              T *Dptr, int m,
                                                              int n, int k) {
  ...
  constexpr int nk = decltype(size<2>(tCrA))::value;
...
void launch_hgemm_prefetch_all_k64(T *a, T *b, T *c, int M, int N, int K,
                                    int swizzle_stride) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory request is larger than A's allocation");
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_prefetch_all_k64<half, true>(
