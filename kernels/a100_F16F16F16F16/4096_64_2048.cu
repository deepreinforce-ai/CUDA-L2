...
void launch_hgemm_7stage_optimized_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
#define LAUNCH_HGEMM_7STAGE_OPTIMIZED_CUTE_TN(stages)                          \
  launch_hgemm_7stage_optimized_tn_cute<half, (stages)>(                       \
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b,
  ...
  LAUNCH_HGEMM_7STAGE_OPTIMIZED_CUTE_TN(7);
}