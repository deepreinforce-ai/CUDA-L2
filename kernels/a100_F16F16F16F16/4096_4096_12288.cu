...
__global__ void __launch_bounds__(128, 2) cuda_l2_a100_fp16_kernel(
    T *Aptr, T *Bptr, T *Dptr, int m, int n, int k, int swizzle_stride) {
...
template <typename T>
void launch_hgemm_optimized_v5_cute(T *a, T *b, T *c, int M, int N, int K,
                                     int swizzle_stride) {
  using namespace cute;
  ...
  cuda_l2_a100_fp16_kernel<
      ...
      S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K, swizzle_stride);
}
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_optimized_v5_cute<half>(
