...
template <typename T>
void launch_hgemm_direct(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  using MMA_EU_RepeatT = Layout<Shape<Int<2>, Int<1>, Int<1>>>;
  using mma_atom_shape = mma_traits::Shape_MNK;
  constexpr int kMmaPM = 2 * get<0>(mma_atom_shape{});
  constexpr int kMmaPN = 2 * get<1>(mma_atom_shape{});
  ...
  using G2SCopyA = decltype(make_tiled_copy(
      g2s_copy_atom{},
      Layout<Shape<Int<8>, Int<8>>, Stride<Int<8>, Int<1>>>{},
  ...
  cuda_l2_a100_fp16_kernel<
      ...
      S2RCopyAtomA, S2RCopyAtomB><<<grid, block, shm_size>>>(a, b, c, M, N, K);
}
