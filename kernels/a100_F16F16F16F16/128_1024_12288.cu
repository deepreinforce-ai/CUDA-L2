...
#pragma unroll
    for (int ik = 0; ik < nk; ik += 2) {
      ...
      if (ik_next1 < nk) {
        cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik_next1), tCrB(_, _, ik_next1), tCrD);
      }
...
template <typename T, const int Stages = 8, const bool BlockSwizzle = false>
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  LAUNCH_HGEMM_MMA_STAGES_CUTE_SWIZZLE_TN(8, swizzle_stride);
}