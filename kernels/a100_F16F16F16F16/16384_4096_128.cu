...
__global__ void __launch_bounds__(128, 2) cuda_l2_a100_fp16_kernel(
  ...
  const int total_tiles_x = (n + BN - 1) / BN;
  const int total_tiles_y = (m + BM - 1) / BM;
  ...
  for (int tile_id = block_id; tile_id < total_tiles; tile_id += num_blocks) {
    const int iy = tile_id / total_tiles_x;
    const int ix = tile_id % total_tiles_x;
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      if (ik < nk - 1) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 1, 1), tCrA_view(_, _, ik + 1));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 1, 1), tCrB_view(_, _, ik + 1));
      }
      cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik), tCrB(_, _, ik), tCrD);
    }
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      if (ik < nk - 1) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 1, 2), tCrA_view(_, _, ik + 1));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 1, 2), tCrB_view(_, _, ik + 1));
      }
      cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik), tCrB(_, _, ik), tCrD);
    }
