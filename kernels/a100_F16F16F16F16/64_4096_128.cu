...
#pragma unroll
    ...
    TiledMMA tiled_mma;
    auto thr_mma = tiled_mma.get_slice(idx - 128);
    ...
    auto s2r_thr_copy_a = s2r_tiled_copy_a.get_slice(idx - 128);
    auto tAsA = s2r_thr_copy_a.partition_S(sA);
    auto tCrA_view = s2r_thr_copy_a.retile_D(tCrA);

    auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
    auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx - 128);
...
#pragma unroll
    ...
    auto r2s_thr_copy_c = r2s_tiled_copy_c.get_slice(idx - 128);
    auto tCrC_r2s = r2s_thr_copy_c.retile_S(tCrD);
    auto tCsC_r2s = r2s_thr_copy_c.partition_D(sC);

    S2GCopyC s2g_tiled_copy_c;
    auto s2g_thr_copy_c = s2g_tiled_copy_c.get_thread_slice(idx - 128);
...
template <typename T>
void launch_hgemm_warp_specialized_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory exceeds A pipe");
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_warp_specialized_cute<half>(
