...
void launch_hgemm_optimized_v2(T *a, T *b, T *c, int M, int N, int K) {
  ...
  using SmemLayoutC = decltype(make_layout(
      ...
      make_stride(Int<kMmaPN>{}, Int<1>{}, Int<kMmaPM * kMmaPN>{})));
  ...
  static constexpr int kShmSize = 
      (BM * BK + BN * BK + cute::max(0, int(kMmaPM * kMmaPN) - BM * BK)) * sizeof(T);
  ...
  cuda_l2_a100_fp16_kernel<
      ...
      S2GCopyAtomC, S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}
