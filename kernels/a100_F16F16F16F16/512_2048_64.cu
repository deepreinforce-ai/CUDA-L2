...
template <typename T, int BM, int BN, int BK, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(64, 14) cuda_l2_a100_fp16_kernel(
  ...
  {
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, 0), tAsA_copy(_, _, _));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, 0), tBsB_copy(_, _, _));
    __syncthreads();
...
#pragma unroll
    for (int ik = 0; ik < nk - 1; ++ik) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 1), tCrA_view(_, _, ik + 1));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 1), tCrB_view(_, _, ik + 1));
  ...
  {
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, 1), tAsA_copy(_, _, _));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, 1), tBsB_copy(_, _, _));
    __syncthreads();
...
template <typename T>
void launch_hgemm_optimized_prefetch(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_prefetch<half>(
