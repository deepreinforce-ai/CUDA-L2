...
__global__ void __launch_bounds__(128, 2)
    hgemm_optimized_persistent_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n,
                                      int k) {
...
void launch_hgemm_optimized_persistent(T *a, T *b, T *c, int M, int N, int K,
                                       int swizzle_stride) {
  using namespace cute;
  ...
  cudaFuncSetAttribute(
      hgemm_optimized_persistent_kernel<
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_persistent<half, 4, true>(
