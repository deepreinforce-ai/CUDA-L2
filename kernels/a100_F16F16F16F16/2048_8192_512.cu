...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2RCopyAtomB, typename R2SCopyAtomC, typename S2GCopyC,
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  using SmemLayoutAtomC = decltype(
    make_layout(make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}),
                make_stride(Int<kMmaPN>{} + Int<8>{}, Int<1>{}))
  );
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  TORCH_CHECK(a.is_cuda() && b_col_major.is_cuda() && c.is_cuda(), "All tensors must be on CUDA device");
  TORCH_CHECK(a.dtype() == torch::kHalf && b_col_major.dtype() == torch::kHalf && c.dtype() == torch::kHalf, "All tensors must be FP16");

  const int M = a.size(0);
  const int K = a.size(1);
  const int N = c.size(1);

  TORCH_CHECK(a.sizes() == torch::IntArrayRef({M, K}));
  TORCH_CHECK(b_col_major.sizes() == torch::IntArrayRef({K, N}));
  TORCH_CHECK(c.sizes() == torch::IntArrayRef({M, N}));
