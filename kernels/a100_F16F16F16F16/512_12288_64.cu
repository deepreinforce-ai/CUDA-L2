...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 2) cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr,
                                                              T *Dptr, int m,
                                                              int n, int k) {
  ...
  auto tCrA_buf0_view = s2r_thr_copy_a.retile_D(tCrA_buf0);
  auto tCrB_buf0_view = s2r_thr_copy_b.retile_D(tCrB_buf0);
  auto tCrA_buf1_view = s2r_thr_copy_a.retile_D(tCrA_buf1);
  auto tCrB_buf1_view = s2r_thr_copy_b.retile_D(tCrB_buf1);
  ...
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, 0), tCrA_buf0_view(_, _, 0));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, 0), tCrB_buf0_view(_, _, 0));
  
  if (nk > 1) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 0), tCrA_buf1_view(_, _, 1));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 0), tCrB_buf1_view(_, _, 1));
  }
  
  cute::gemm(tiled_mma, tCrD, tCrA_buf0(_, _, 0), tCrB_buf0(_, _, 0), tCrD);
  
  if (nk > 1) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, 1), tCrA_buf0_view(_, _, 0));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, 1), tCrB_buf0_view(_, _, 0));
    
    cute::gemm(tiled_mma, tCrD, tCrA_buf1(_, _, 1), tCrB_buf1(_, _, 1), tCrD);
  }

  if (nk > 1) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 1), tCrA_buf1_view(_, _, 1));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 1), tCrB_buf1_view(_, _, 1));
  }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  using SmemLayoutA = decltype(tile_to_shape(
      SmemLayoutAtom{}, make_shape(Int<BM>{}, Int<BK>{}, Int<2>{})));
  using SmemLayoutB = decltype(tile_to_shape(
      SmemLayoutAtom{},
      make_shape(Int<BN>{}, Int<BK>{}, Int<2>{})));
