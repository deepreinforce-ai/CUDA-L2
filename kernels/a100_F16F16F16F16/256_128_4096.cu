...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrD0 = thr_mma.partition_fragment_C(gD);
  auto tCrD1 = thr_mma.partition_fragment_C(gD);
  clear(tCrD0);
  clear(tCrD1);
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if ((ik & 1) == 0) {
        cute::gemm(tiled_mma, tCrD0, tCrA(_, _, ik), tCrB(_, _, ik), tCrD0);
      } else {
        cute::gemm(tiled_mma, tCrD1, tCrA(_, _, ik), tCrB(_, _, ik), tCrD1);
      }
    }
  }
