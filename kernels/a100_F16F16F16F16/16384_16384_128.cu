...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 0), tCrA_next_view(_, _, 1));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 0), tCrB_next_view(_, _, 1));
  ...
  cute::gemm(tiled_mma, tCrD, tCrA_next(_, _, 1), tCrB_next(_, _, 1), tCrD);

  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 1), tCrA_next_view(_, _, 1));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 1), tCrB_next_view(_, _, 1));
  ...
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, 2), tCrA_view(_, _, 0));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, 2), tCrB_view(_, _, 0));
  
  cute::gemm(tiled_mma, tCrD, tCrA_next(_, _, 1), tCrB_next(_, _, 1), tCrD);

  cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, 2), tCrA_next_view(_, _, 1));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, 2), tCrB_next_view(_, _, 1));
  ...
  auto sC = make_tensor(sA(_, _, 1).data(), SmemLayoutC{});
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half>(
      ...
      reinterpret_cast<half *>(c.data_ptr()), M, N, K, 8192);
}