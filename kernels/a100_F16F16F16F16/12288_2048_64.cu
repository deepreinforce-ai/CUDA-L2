...
#pragma unroll
  ...
  int num_steps = size<1>(tCrC_r2sx);
...
template <typename T>
void launch_hgemm_double_buffer_epilogue(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  using SmemLayoutA =
      decltype(tile_to_shape(SmemLayoutAtom{}, make_shape(Int<BM>{}, Int<BK>{})));
  using SmemLayoutB =
      decltype(tile_to_shape(SmemLayoutAtom{}, make_shape(Int<BN>{}, Int<BK>{})));
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_double_buffer_epilogue<half>(
