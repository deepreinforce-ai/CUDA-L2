...
__global__ void cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr,
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, itile),
               tAsA_copy(_, _, _, read_stage));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, itile),
               tBsB_copy(_, _, _, read_stage));
    cp_async_fence();
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, read_stage),
                 tCrA_view(_, _, ik));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, read_stage),
                 tCrB_view(_, _, ik));
