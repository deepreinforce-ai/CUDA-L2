...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    ...
    if (ik_next < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, 0),
                 tCrA_view(_, _, ik_next));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, 0),
...
template <typename T, const bool BlockSwizzle = false>
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half, true>(
