...
__global__ void cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr,
  ...
  auto tAgA_copy =
      g2s_thr_copy_a.partition_S(gA);
  auto tAsA_copy =
      g2s_thr_copy_a.partition_D(sA);

  G2SCopyB g2s_tiled_copy_b;
  auto g2s_thr_copy_b = g2s_tiled_copy_b.get_slice(idx);
  auto tBgB_copy =
      g2s_thr_copy_b.partition_S(gB);
  auto tBsB_copy =
      g2s_thr_copy_b.partition_D(sB);
...
template <typename T, const int Stages, const bool BlockSwizzle>
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static constexpr int kMmaEURepeatM = 2, kMmaEURepeatN = 2, kMmaEURepeatK = 1;
  ...
  using SmemLayoutAtomC = decltype(composition(
      ...
      make_layout(make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}),
                  make_stride(Int<kMmaPN + 32>{}, Int<1>{}))));
  using SmemLayoutC = decltype(tile_to_shape(
      SmemLayoutAtomC{},
      make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}, Int<kSmemLayoutCBatch>{})));

  static_assert(cute::cosize_v<SmemLayoutA>/Stages >= cute::cosize_v<SmemLayoutC>,
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  TORCH_CHECK(a.options().dtype() == torch::kHalf, "a must be a half tensor");
  TORCH_CHECK(b.options().dtype() == torch::kHalf, "b must be a half tensor");
  TORCH_CHECK(b_col_major.options().dtype() == torch::kHalf, "b_col_major must be a half tensor");
  TORCH_CHECK(c.options().dtype() == torch::kHalf, "c must be a half tensor");
  
  const int M = a.size(0);
  const int K = a.size(1);
  const int N = c.size(1);
  
  TORCH_CHECK(a.dim() == 2 && a.size(0) == M && a.size(1) == K, "Input A has wrong size");
  TORCH_CHECK(b.dim() == 2 && b.size(0) == K && b.size(1) == N, "Input B (row-major) has wrong size");
  TORCH_CHECK(b_col_major.dim() == 2 && b_col_major.size(0) == K && b_col_major.size(1) == N, "Input B (col-major) has wrong size");
  TORCH_CHECK(c.dim() == 2 && c.size(0) == M && c.size(1) == N, "Output C has wrong size");
