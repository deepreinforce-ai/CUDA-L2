...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  int num_tiles_x = (n + BN - 1) / BN;
  int num_tiles_y = (m + BM - 1) / BM;
  int total_tiles = num_tiles_x * num_tiles_y;
  
  for (int tile_id = blockIdx.x + blockIdx.y * gridDim.x; 
       tile_id < total_tiles; 
       tile_id += gridDim.x * gridDim.y) {
    
    int ix = tile_id % num_tiles_x;
    int iy = tile_id / num_tiles_x;
...
#pragma unroll
      for (int ik = 0; ik < nk; ik += 2) {
        ...
        if (has_next) {
          ...
          cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik_next), tCrB(_, _, ik_next), tCrD);
        }
      }
    }
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_tn_cute<half, 6>(
