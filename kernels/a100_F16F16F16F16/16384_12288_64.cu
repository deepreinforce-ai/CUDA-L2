...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  constexpr int step = size<3>(tCsC_r2s);
  constexpr int total_iters = size<1>(tCrC_r2sx);
  
  constexpr int half_iters = total_iters / 2;
  
#pragma unroll
  for (int i = 0; i < half_iters; i += step) {
#pragma unroll
...
#pragma unroll
  for (int i = half_iters; i < total_iters; i += step) {
#pragma unroll
    for (int j = 0; j < step; ++j) {
      cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, i + j), tCsC_r2s(_, 0, 0, j));
    }
