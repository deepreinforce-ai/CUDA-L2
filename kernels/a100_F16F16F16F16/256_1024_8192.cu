...
__global__ void cuda_l2_a100_fp16_kernel(const T * __restrict__ Aptr, 
                                                   const T * __restrict__ Bptr,
                                                   T * __restrict__ Dptr, 
                                                   const int m, const int n, const int k) {
  using namespace cute;
  
  extern __shared__ __align__(16) T shm_data[];
...
#pragma unroll
  ...
  const int total_iterations = size<1>(tCrC_r2sx);
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_block_swizzle_tn_cute<half, 7, true>(
