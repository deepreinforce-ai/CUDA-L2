...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert((BM * BK) >= cute::cosize(SmemLayoutC{}),
                "C shared memory request is larger than one pipe of A's shared memory");
  ...
  if (shm_size > (48 * 1024)) {
      cudaFuncSetAttribute(cuda_l2_a100_fp16_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle>, cudaFuncAttributeMaxDynamicSharedMemorySize, shm_size);
  }
