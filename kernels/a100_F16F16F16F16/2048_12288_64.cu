...
#pragma unroll
  ...
  if (ntile > 1) {
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, kStage - 1),
               tAsA_copy(_, _, _, (kStage - 1) % kStage));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, kStage - 1),
               tBsB_copy(_, _, _, (kStage - 1) % kStage));
    cp_async_fence();
  }
