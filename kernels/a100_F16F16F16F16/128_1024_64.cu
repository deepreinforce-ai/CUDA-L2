...
template <typename T, int BM, int BN, int BK, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, 
                                                   int m, int n, int k) {
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      cute::copy(s2r_tiled_copy_a, tAsA_load(_, _, ik, 0), tCrA_view(_, _, ik));
      cute::copy(s2r_tiled_copy_b, tBsB_load(_, _, ik, 0), tCrB_view(_, _, ik));
...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  ...
  using s2r_copy_op = UniversalCopy<int>;
  ...
  static constexpr int shm_size = (BM * BK + BN * BK) * sizeof(T);
