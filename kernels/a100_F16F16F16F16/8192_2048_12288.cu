...
cuda_l2_a100_fp16_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_view_next = s2r_thr_copy_a.retile_D(tCrA_next);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_view_next = s2r_thr_copy_b.retile_D(tCrB_next);
...
#pragma unroll
  ...
  int ik_next = 1;
  cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read), tCrA_view_next(_, _, ik_next));
  cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read), tCrB_view_next(_, _, ik_next));
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (use_primary) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_view_next(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_view_next(_, _, ik_next));
      } else {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
