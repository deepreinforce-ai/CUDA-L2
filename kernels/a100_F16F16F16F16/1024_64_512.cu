...
template <typename T, const int Stages = 10>
void launch_hgemm_mma_stages_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_cute<half, 10>(
