...
void launch_hgemm_triple_swizzle_ultimate(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  using namespace cute;
...
void cuda_l2_a100_fp16(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  int swizzle_stride = 32;
  
  launch_hgemm_triple_swizzle_ultimate<half, 5, true>(
