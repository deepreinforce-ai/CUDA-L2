...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  #pragma unroll
  for (int i = 0; i < min(kStage - 1, ntile); ++i) {
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, i), tAsA_copy(_, _, _, ismem_write));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, i), tBsB_copy(_, _, _, ismem_write));
    cp_async_fence();
    ismem_write = (ismem_write + 1) % kStage;
  }
  
  int itile_to_read = min(kStage - 1, ntile);
  ...
  for (int i = 0; i < num_elements; i += 8) {
    if (i + 8 <= num_elements) {
      float4 f0 = *reinterpret_cast<float4*>(&tCrD(i));
      float4 f1 = *reinterpret_cast<float4*>(&tCrD(i + 4));
      ...
      *reinterpret_cast<half2*>(&tCrD_half(i)) = h0;
      *reinterpret_cast<half2*>(&tCrD_half(i + 2)) = h1;
      *reinterpret_cast<half2*>(&tCrD_half(i + 4)) = h2;
      *reinterpret_cast<half2*>(&tCrD_half(i + 6)) = h3;
    } else {
      for (int j = i; j < num_elements; ++j) {
        tCrD_half(j) = __float2half(tCrD(j));
      }
    }
  }
