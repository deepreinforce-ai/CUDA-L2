...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  for (int tile_id = blockIdx.x; tile_id < total_tiles; tile_id += gridDim.x) {
    int ix = tile_id % num_tiles_n;
    int iy = tile_id / num_tiles_n;
...
void launch_hgemm_optimized_persistent_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  cuda_l2_3090_fp32_kernel<
      ...
      S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K, total_tiles);
}
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_optimized_persistent_tn_cute<half, 5>(
