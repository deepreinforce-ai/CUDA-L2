...
cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, 
                                  int m, int n, int k) {
  ...
  if (nk > 1) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, ismem_read), tCrA_next_view(_, _, 1));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, ismem_read), tCrB_next_view(_, _, 1));
  }
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik < nk - 2) {
        if (ik % 2 == 0) {
          ...
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next2, ismem_read),
                     tCrA_next_view(_, _, ik_next2));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next2, ismem_read),
                     tCrB_next_view(_, _, ik_next2));
        }
      } else if (ik == nk - 2) {
