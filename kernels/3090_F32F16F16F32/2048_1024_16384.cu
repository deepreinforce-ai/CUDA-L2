...
#pragma unroll 1
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    int next_tile = itile + 1;
    if (next_tile < ntile) {
      int next_stage = (ismem_write + 1) % kStage;
...
#pragma unroll
  ...
  auto tCrD_output = make_tensor_like<T>(tCrD_fp32);
#pragma unroll
  for (int i = 0; i < size(tCrD_fp32); ++i) {
    tCrD_output(i) = T(tCrD_fp32(i));
  }

  auto sC = make_tensor(sA(_, _, ismem_read).data(), SmemLayoutC{});
  auto r2s_tiled_copy_c = make_tiled_copy_C(R2SCopyAtomC{}, tiled_mma);
  auto r2s_thr_copy_c = r2s_tiled_copy_c.get_slice(idx);
  auto tCrC_r2s = r2s_thr_copy_c.retile_S(tCrD_output);
