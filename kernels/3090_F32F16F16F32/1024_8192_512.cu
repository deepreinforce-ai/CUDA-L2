...
cuda_l2_3090_fp32_kernel(const T * __restrict__ Aptr,
  ...
  const Tensor A = make_tensor(make_gmem_ptr(Aptr), make_shape(m, k),
                                make_stride(k, Int<1>{}));
  const Tensor B = make_tensor(make_gmem_ptr(Bptr), make_shape(n, k),
                                make_stride(k, Int<1>{}));
  const Tensor D = make_tensor(make_gmem_ptr(Dptr), make_shape(m, n),
                                make_stride(n, Int<1>{}));

  const Tensor gA = local_tile(A, make_tile(Int<BM>{}, Int<BK>{}),
                                make_coord(iy, _));
  const Tensor gB = local_tile(B, make_tile(Int<BN>{}, Int<BK>{}),
                                make_coord(ix, _));
  const Tensor gD = local_tile(D, make_tile(Int<BM>{}, Int<BN>{}),
                                make_coord(iy, ix));

  const auto sA = make_tensor(make_smem_ptr(Ashm), SmemLayoutA{});
  const auto sB = make_tensor(make_smem_ptr(Bshm), SmemLayoutB{});

  TiledMMA tiled_mma;
  const auto thr_mma = tiled_mma.get_slice(idx);
  ...
  const auto g2s_thr_copy_a = g2s_tiled_copy_a.get_slice(idx);
  const auto tAgA_copy = g2s_thr_copy_a.partition_S(gA);
  const auto tAsA_copy = g2s_thr_copy_a.partition_D(sA);

  G2SCopyB g2s_tiled_copy_b;
  const auto g2s_thr_copy_b = g2s_tiled_copy_b.get_slice(idx);
  const auto tBgB_copy = g2s_thr_copy_b.partition_S(gB);
  const auto tBsB_copy = g2s_thr_copy_b.partition_D(sB);

  const auto s2r_tiled_copy_a = make_tiled_copy_A(S2RCopyAtomA{}, tiled_mma);
  const auto s2r_thr_copy_a = s2r_tiled_copy_a.get_slice(idx);
  const auto tAsA = s2r_thr_copy_a.partition_S(sA);
  const auto tCrA_view = s2r_thr_copy_a.retile_D(tCrA);

  const auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  const auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  const auto tBsB = s2r_thr_copy_b.partition_S(sB);
  const auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik + 2 < nk) {
        const int ik_pf = ik + 2;
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_pf, ismem_read),
                   tCrA_view(_, _, ik_pf));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_pf, ismem_read),
                   tCrB_view(_, _, ik_pf));
      }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  auto kernel_func = cuda_l2_3090_fp32_kernel<
  ...
  kernel_func<<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}
