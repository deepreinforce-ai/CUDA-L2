...
cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_next = make_fragment_like(tCrA);
  auto tCrB_next = make_fragment_like(tCrB);
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      int ik_next = (ik + 1) % nk;
      bool is_last_k = (ik == nk - 1);
      bool has_next_tile = (itile + 1 < ntile);
      ...
      if (!is_last_k) {
        auto tmp_a = tCrA(_, _, ik_next);
        auto tmp_b = tCrB(_, _, ik_next);
        cute::copy(tCrA_next(_, _, ik_next), tmp_a);
        cute::copy(tCrB_next(_, _, ik_next), tmp_b);
      } else if (has_next_tile) {
        auto tmp_a = tCrA(_, _, 0);
        auto tmp_b = tCrB(_, _, 0);
        cute::copy(tCrA_next(_, _, 0), tmp_a);
        cute::copy(tCrB_next(_, _, 0), tmp_b);
      }
    }
  }
