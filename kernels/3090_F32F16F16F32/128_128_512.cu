...
cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  #pragma unroll
  for (int i = 0; i < cute::min(kStage, ntile); ++i) {
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, i), tAsA_copy(_, _, _, i));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, i), tBsB_copy(_, _, _, i));
    cp_async_fence();
  }

  #pragma unroll 1
  for (int itile = 0; itile < ntile; ++itile) {
    int istage = itile % kStage;
    int istage_next = (itile + kStage) % kStage;

    if (itile + kStage < ntile) {
      cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, itile + kStage), 
                 tAsA_copy(_, _, _, istage_next));
      cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, itile + kStage), 
                 tBsB_copy(_, _, _, istage_next));
      cp_async_fence();
    }
    ...
    for (int ik = 0; ik < nk; ++ik) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, istage), tCrA_view(_, _, ik));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, istage), tCrB_view(_, _, ik));
...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  ...
  cudaFuncSetAttribute(
      cuda_l2_3090_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB,
