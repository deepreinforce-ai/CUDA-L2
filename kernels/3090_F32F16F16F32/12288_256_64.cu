...
template <typename T, int BM, int BN, int BK, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 3) cuda_l2_3090_fp32_kernel(
...
#pragma unroll
  for (int ik = 0; ik < nk; ++ik) {
    if (ik + 3 < nk) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + 3), tCrA_view(_, _, ik + 3));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + 3), tCrB_view(_, _, ik + 3));
    }
  ...
  constexpr int kEpilogueStages = 2;
...
#pragma unroll
    for (int j = 0; j < step; ++j) {
      cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, i + j),
                 tCsC_r2s(_, stage, 0, j));
    }
    __syncthreads();

#pragma unroll
    for (int j = 0; j < step; ++j) {
      cute::copy(s2g_tiled_copy_c, tCsC_s2g(_, stage, 0, j),
                 tCgC_s2gx(_, i + j));
    }

    stage = (stage + 1) % kEpilogueStages;
...
template <typename T>
void launch_hgemm_bk64_optimized(T *a, T *b, T *c, int M, int N, int K,
                                  int swizzle_stride) {
  using namespace cute;
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_bk64_optimized<half>(
