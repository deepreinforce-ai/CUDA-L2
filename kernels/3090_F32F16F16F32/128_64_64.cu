...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr,
  ...
  for (int ktile = 0; ktile < ntile; ++ktile) {
    cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, ktile), tAsA_copy(_, _, _));
    cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, ktile), tBsB_copy(_, _, _));
    cp_async_fence();
    cp_async_wait<0>();
    __syncthreads();
...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  ...
  cuda_l2_3090_fp32_kernel<T, BM, BN, BK, MMA, G2SCopyA, G2SCopyB,
