...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  ...
  constexpr int kMmaPM = 1 * 2 * get<0>(mma_atom_shape{});
  constexpr int kMmaPN = 2 * 2 * get<1>(mma_atom_shape{});
  constexpr int kMmaPK = 1 * 1 * get<2>(mma_atom_shape{});
  ...
  using SmemLayoutC = decltype(tile_to_shape(
      SmemLayoutAtomC{}, make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}, Int<2>{})));
