...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  auto tCrA_double = make_tensor_like(tCrA);
  auto tCrB_double = make_tensor_like(tCrB);
  ...
  auto tCrA_double_view = s2r_thr_copy_a.retile_D(tCrA_double);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_double_view = s2r_thr_copy_b.retile_D(tCrB_double);
...
#pragma unroll
  for (int istage = 0; istage < min(kStage - 1, ntile); ++istage) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if ((ik & 1) == 0) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_double_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_double_view(_, _, ik_next));
      } else {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
