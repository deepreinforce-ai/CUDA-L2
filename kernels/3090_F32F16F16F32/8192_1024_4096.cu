...
cuda_l2_3090_fp32_kernel(const T * __restrict__ Aptr, 
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik_prefetch < nk) {
        int prefetch_buffer = ik_prefetch % 3;
        if (prefetch_buffer == 0) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch, ismem_read),
                     tCrA0_view(_, _, ik_prefetch));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch, ismem_read),
                     tCrB0_view(_, _, ik_prefetch));
        } else if (prefetch_buffer == 1) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch, ismem_read),
                     tCrA1_view(_, _, ik_prefetch));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch, ismem_read),
                     tCrB1_view(_, _, ik_prefetch));
        } else {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch, ismem_read),
                     tCrA2_view(_, _, ik_prefetch));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch, ismem_read),
                     tCrB2_view(_, _, ik_prefetch));
        }
      }
      ...
      if (buffer_idx == 0) {
        ...
        cute::gemm(tiled_mma, tCrD, tCrA1(_, _, ik), tCrB1(_, _, ik), tCrD);
      } else {
        cute::gemm(tiled_mma, tCrD, tCrA2(_, _, ik), tCrB2(_, _, ik), tCrD);
      }
      ...
      if (ik == nk - 1) {
        ...
        if (itile + 1 < ntile) {
          ...
          if (nk > 2) {
            cute::copy(s2r_tiled_copy_a, tAsA(_, _, 2, ismem_read),
                       tCrA2_view(_, _, 2));
            cute::copy(s2r_tiled_copy_b, tBsB(_, _, 2, ismem_read),
                       tCrB2_view(_, _, 2));
          }
        }
      }
