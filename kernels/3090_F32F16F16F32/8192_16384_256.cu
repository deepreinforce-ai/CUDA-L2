...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik + 1 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read), tCrA_prefetch_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read), tCrB_prefetch_view(_, _, ik_next));
      }
      
      cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik), tCrB(_, _, ik), tCrD);
      
      if (ik + 1 < nk) {
        cute::copy(tCrA_prefetch(_, _, ik_next), tCrA(_, _, ik_next));
        cute::copy(tCrB_prefetch(_, _, ik_next), tCrB(_, _, ik_next));
      }
    }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  ...
  cudaFuncSetAttribute(
      cuda_l2_3090_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle>,
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  LAUNCH_HGEMM_MMA_STAGES_CUTE_SWIZZLE_TN(2, 2048);
}