...
#pragma unroll
    for (int prefetch_k = 0; prefetch_k < 4 && prefetch_k < nk; ++prefetch_k) {
      cute::copy(s2r_tiled_copy_a, tAsA(_, _, prefetch_k, ismem_read), 
                 tCrA_view(_, _, prefetch_k));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, prefetch_k, ismem_read), 
                 tCrB_view(_, _, prefetch_k));
    }
...
#pragma unroll
  for (int j = 0; j < step; ++j) {
    cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, j), tCsC_r2s(_, 0, 0, j));
  }
  __syncthreads();
...
#pragma unroll
      for (int j = 0; j < step; ++j) {
        cute::copy(r2s_tiled_copy_c, tCrC_r2sx(_, i + step + j), 
                   tCsC_r2s(_, 0, 0, j));
      }
      __syncthreads();
    }
