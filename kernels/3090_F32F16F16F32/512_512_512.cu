...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  int total_tiles = tiles_n * tiles_m;
  ...
  for (int tile_idx = blockIdx.x + blockIdx.y * gridDim.x + blockIdx.z * gridDim.x * gridDim.y; 
    ...
    int ix = tile_idx % tiles_n;
    int iy = tile_idx / tiles_n;
...
template <typename T>
void launch_hgemm_persistent_rectangular(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_persistent_rectangular<half>(
