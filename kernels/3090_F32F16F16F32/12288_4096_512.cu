...
cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    const int smem_read = itile & 1;
    const int smem_write = (itile + 1) & 1;
    const int next_tile = itile + 1;
    ...
    if (next_tile < ntile) {
      cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, next_tile), 
                 tAsA_copy(_, _, _, smem_write));
      cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, next_tile), 
                 tBsB_copy(_, _, _, smem_write));
      cp_async_fence();
    }
    
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, smem_read), tCrA_view(_, _, 0));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, smem_read), tCrB_view(_, _, 0));
    
    const int nk = size<2>(tCrA);
    
    #pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      const int ik_next = ik + 1;
      
      if (ik_next < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, smem_read), 
                   tCrA_view(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, smem_read), 
