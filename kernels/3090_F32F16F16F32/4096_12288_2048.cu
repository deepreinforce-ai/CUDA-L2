...
#pragma unroll 1
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (; ik + kUnroll - 1 < nk; ik += kUnroll) {
      #pragma unroll
      for (int u = 0; u < kUnroll; ++u) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik + u, ismem_read),
                   tCrA_view(_, _, ik + u));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik + u, ismem_read),
                   tCrB_view(_, _, ik + u));
      }

      #pragma unroll
      for (int u = 0; u < kUnroll; ++u) {
        cute::gemm(tiled_mma, tCrD, tCrA(_, _, ik + u), tCrB(_, _, ik + u), tCrD);
      }
    }
