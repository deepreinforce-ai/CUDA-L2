...
cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    int itile_next = itile + 1;

    cp_async_wait<0>();
    __syncthreads();

    if (itile_next < ntile) {
      int ismem_write_next = (ismem_write + 1) % kStage;
      cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, itile_next),
                 tAsA_copy(_, _, _, ismem_write_next));
      cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, itile_next),
                 tBsB_copy(_, _, _, ismem_write_next));
      cp_async_fence();
      ismem_write = ismem_write_next;
    }
...
template <typename T>
void launch_hgemm_bank_conflict_optimized(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b,
  ...
  launch_hgemm_bank_conflict_optimized<half>(
