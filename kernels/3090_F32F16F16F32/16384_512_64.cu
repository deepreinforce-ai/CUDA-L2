...
#pragma unroll
  for (int i = 0; i < num_vec; ++i) {
    int base = i * vec_size;
    tCrD_half(base + 0) = static_cast<T>(tCrD(base + 0));
    tCrD_half(base + 1) = static_cast<T>(tCrD(base + 1));
    tCrD_half(base + 2) = static_cast<T>(tCrD(base + 2));
    tCrD_half(base + 3) = static_cast<T>(tCrD(base + 3));
    tCrD_half(base + 4) = static_cast<T>(tCrD(base + 4));
    tCrD_half(base + 5) = static_cast<T>(tCrD(base + 5));
    tCrD_half(base + 6) = static_cast<T>(tCrD(base + 6));
    tCrD_half(base + 7) = static_cast<T>(tCrD(base + 7));
  }
  
#pragma unroll
  for (int i = num_vec * vec_size; i < num_elements; ++i) {
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  using SmemLayoutAtom = decltype(composition(
      Swizzle<2, 3, 3>{},
      make_layout(make_shape(Int<4>{}, Int<BK>{}),
