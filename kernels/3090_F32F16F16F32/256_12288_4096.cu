...
#pragma unroll
  ...
  constexpr int kConvUnroll = 8;
  int total_size = size(tCrD);
  int unroll_limit = (total_size / kConvUnroll) * kConvUnroll;
  
#pragma unroll
  for (int i = 0; i < unroll_limit; i += kConvUnroll) {
...
#pragma unroll
  for (int i = unroll_limit; i < total_size; ++i) {
