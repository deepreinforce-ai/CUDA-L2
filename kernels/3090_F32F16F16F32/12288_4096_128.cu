...
#pragma unroll
    for (int j = 0; j < step; ++j) {
      auto tCrC_fp16 = make_tensor_like<T>(tCrC_r2sx(_, i + j));
      cute::copy(tCrC_r2sx(_, i + j), tCrC_fp16);
      cute::copy(r2s_tiled_copy_c, tCrC_fp16, tCsC_r2s(_, 0, 0, j));
    }
    __syncthreads();
