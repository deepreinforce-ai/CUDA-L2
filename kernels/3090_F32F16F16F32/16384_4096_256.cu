...
#pragma unroll 1
  for (int itile = 0; itile < ntile; ++itile) {
    if (itile + kStage - 1 < ntile) {
      cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, itile + kStage - 1),
                 tAsA_copy(_, _, _, ismem_write));
      cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, itile + kStage - 1),
...
#pragma unroll
    for (int j = 0; j < step; ++j) {
      auto tCrD_half = make_tensor_like<T>(tCrC_r2sx(_, i + j));
      cute::copy(tCrC_r2sx(_, i + j), tCrD_half);
      cute::copy(r2s_tiled_copy_c, tCrD_half, tCsC_r2s(_, 0, 0, j));
    }
    __syncthreads();
