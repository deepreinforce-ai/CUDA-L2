...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  const int tiles_per_block = 2;
  
  for (int tile_batch = 0; tile_batch < tiles_per_block; ++tile_batch) {
    int block_idx = blockIdx.x * tiles_per_block + tile_batch;
    int ix = ((int)BlockSwizzle) * blockIdx.z * (gridDim.x * tiles_per_block) + block_idx;
    int iy = blockIdx.y;
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  const int tiles_per_block = 2;
  int BX = (N + BN - 1) / BN;
  BX = (BX + tiles_per_block - 1) / tiles_per_block;
  int BY = (M + BM - 1) / BM;
