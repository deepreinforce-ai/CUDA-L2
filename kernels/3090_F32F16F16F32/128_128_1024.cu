...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
    int iy = tile_idx / num_tiles_n;
    int ix = tile_idx % num_tiles_n;
...
void launch_hgemm_mma_stages_persistent_cute(T *a, T *b, T *c, int M,
                                             int N, int K) {
  using namespace cute;
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_mma_stages_persistent_cute<half, 4>(
