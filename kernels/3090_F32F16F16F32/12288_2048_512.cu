...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik == nk - 1) {
        ...
        int ismem_read_next = (ismem_read + 1) % kStage;
        
        if (itile + 1 < ntile && nk > 0) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, 0, ismem_read_next),
                     tCrA_view(_, _, 0));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, 0, ismem_read_next),
                     tCrB_view(_, _, 0));
        }
        
        ismem_read = ismem_read_next;
      }
    }
  }
