...
template <typename T>
void launch_hgemm_single_tile_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  ...
  using SmemLayoutAtom = decltype(composition(
      Swizzle<3, 4, 3>{}, make_layout(make_shape(Int<8>{}, Int<64>{}),
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  launch_hgemm_single_tile_tn_cute<half>(
