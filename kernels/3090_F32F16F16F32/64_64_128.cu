...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void __launch_bounds__(128, 10) cuda_l2_3090_fp32_kernel(
  ...
  auto tCrA_double = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_double = thr_mma.partition_fragment_B(gB(_, _, 0));
  ...
  auto tCrA_view_double = s2r_thr_copy_a.retile_D(tCrA_double);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_view_double = s2r_thr_copy_b.retile_D(tCrB_double);
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (use_primary) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read), tCrA_view_double(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read), tCrB_view_double(_, _, ik_next));
      } else {
      ...
      if (use_primary) {
        ...
        cute::gemm(tiled_mma, tCrD, tCrA_double(_, _, ik), tCrB_double(_, _, ik), tCrD);
      }
    }
  }
...
void launch_hgemm_extreme_pipeline(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b, 
  ...
  launch_hgemm_extreme_pipeline<half, 7>(
