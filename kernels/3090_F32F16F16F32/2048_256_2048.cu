...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  constexpr int kPadB = 4;
  
  T *Ashm = shm_data;
  T *Bshm = shm_data + (cute::cosize(SmemLayoutA{}) + kPadA * BM * kStage);
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static constexpr int shm_size_AB =
      cute::cosize(SmemLayoutA{}) + kPadA * BM * KStage +
      cute::cosize(SmemLayoutB{}) + kPadB * BN * KStage;
