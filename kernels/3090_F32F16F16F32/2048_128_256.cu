...
void launch_hgemm_deep_pipeline_cute(T *a, T *b, T *c, int M, int N, int K,
                                     int swizzle_stride) {
  using namespace cute;
...
#define LAUNCH_HGEMM_DEEP_PIPELINE_CUTE_SWIZZLE_TN(stages, stride)            \
  launch_hgemm_deep_pipeline_cute<half, (stages), true>(                      \
...
void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  ...
  LAUNCH_HGEMM_DEEP_PIPELINE_CUTE_SWIZZLE_TN(4, swizzle_stride);
}