...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M, int N, int K, int swizzle_stride) {
  ...
  using SmemLayoutAtomC = decltype(composition(Swizzle<2, 3, 3>{}, make_layout(make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}), make_stride(Int<kMmaPN>{}, Int<1>{}))));
  ...
  cudaFuncSetAttribute(cuda_l2_3090_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle>, cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);

  cuda_l2_3090_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyB, SmemLayoutA, SmemLayoutB, SmemLayoutC, S2RCopyAtomA, S2RCopyAtomB, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC, BlockSwizzle><<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}

#include <torch/extension.h>
#include <torch/types.h>

#define CHECK_TORCH_TENSOR_DTYPE(T, th_type) if (((T).options().dtype() != (th_type))) { throw std::runtime_error("dtype mismatch"); }
#define CHECK_TORCH_TENSOR_SHAPE(T, S0, S1) if (((T).size(0) != (S0)) || ((T).size(1) != (S1))) { throw std::runtime_error("shape mismatch"); }
