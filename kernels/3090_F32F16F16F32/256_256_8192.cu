...
cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, 
                                  int m, int n, int k, int num_tile_n) {
  ...
  for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
    int ix = tile_idx % num_tile_n;
    int iy = tile_idx / num_tile_n;
...
void launch_hgemm_mma_stages_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  ...
  int num_tile_m = (M + BM - 1) / BM;
  int num_tile_n = (N + BN - 1) / BN;
  ...
  cuda_l2_3090_fp32_kernel<
      ...
      S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K, num_tile_n);
}
