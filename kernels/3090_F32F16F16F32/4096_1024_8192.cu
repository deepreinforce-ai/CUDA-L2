...
cuda_l2_3090_fp32_kernel(T* __restrict__ Aptr,
                                  T* __restrict__ Bptr,
                                  T* __restrict__ Dptr,
                                  int m, int n, int k)
{
...
void launch_hgemm(T* a, T* b, T* c, int M, int N, int K, int swizzle_stride)
{
  ...
  using BN = Int<BN_val>;
  using BK = Int<BK_val>;
  using KStage = Int<Stages>;
  using kSmemLayoutCBatch = Int<4>;

  using SmemLayoutAtom = decltype(composition(
      Swizzle<2, 4, 3>{},
      make_layout(make_shape(Int<16>{}, BK{}),
  ...
  using SmemLayoutC = decltype(tile_to_shape(
      SmemLayoutAtomC{},
      make_shape(Int<kMmaPM>{}, Int<kMmaPN>{}, kSmemLayoutCBatch{})));
  ...
  cudaFuncSetAttribute(
      cuda_l2_3090_fp32_kernel<
          T, BM_val, BN_val, BK_val, Stages, MMA,
          G2SCopyA, G2SCopyB,
