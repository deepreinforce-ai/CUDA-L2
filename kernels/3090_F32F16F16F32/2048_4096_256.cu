...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  auto tCrA_db = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_db = thr_mma.partition_fragment_B(gB(_, _, 0));
  ...
  auto tCrA_view_db = s2r_thr_copy_a.retile_D(tCrA_db);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_view_db = s2r_thr_copy_b.retile_D(tCrB_db);
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (!use_db) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
                   tCrA_view_db(_, _, ik_next));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next, ismem_read),
                   tCrB_view_db(_, _, ik_next));
      } else {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next, ismem_read),
      ...
      if (!use_db) {
        ...
        cute::gemm(tiled_mma, tCrD, tCrA_db(_, _, ik), tCrB_db(_, _, ik), tCrD);
      }
    }
  }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static_assert(size<0>(SmemLayoutA{}) * size<1>(SmemLayoutA{}) >=
                    size(SmemLayoutC{}),
                "C shared memory larger than A pipe");
