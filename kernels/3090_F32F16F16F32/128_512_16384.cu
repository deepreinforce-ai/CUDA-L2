...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  for (; tile_idx < total_tiles; tile_idx += gridDim.x) {
    int ix = tile_idx % ((n + BN - 1) / BN);
    int iy = tile_idx / ((n + BN - 1) / BN);
...
void launch_hgemm_mma_stages_tn_cute(T *a, T *b, T *c, int M, int N, int K) {
  ...
  using SmemLayoutAtom = decltype(composition(
      Swizzle<2, 3, 4>{}, make_layout(make_shape(Int<8>{}, Int<BK>{}),
  ...
  cuda_l2_3090_fp32_kernel<
      ...
      S2GCopyC><<<grid, block, shm_size>>>(a, b, c, M, N, K, total_tiles);
}
