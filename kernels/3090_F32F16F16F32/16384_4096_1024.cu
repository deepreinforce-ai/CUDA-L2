...
cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  auto tCrA_prefetch = thr_mma.partition_fragment_A(gA(_, _, 0));
  auto tCrB_prefetch = thr_mma.partition_fragment_B(gB(_, _, 0));
  ...
  auto tCrA_prefetch_view = s2r_thr_copy_a.retile_D(tCrA_prefetch);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_prefetch_view = s2r_thr_copy_b.retile_D(tCrB_prefetch);

  int itile_to_read = 0;
  int ismem_read = 0;
  int ismem_write = 0;

  cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, 0),
             tAsA_copy(_, _, _, 0));
  cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, 0),
             tBsB_copy(_, _, _, 0));
  cp_async_fence();
  ++itile_to_read;
  ++ismem_write;
  ...
  int nk = size<2>(tCrA);
  if (nk > 1) {
    cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, ismem_read), tCrA_prefetch_view(_, _, 1));
    cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, ismem_read), tCrB_prefetch_view(_, _, 1));
  }
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik < nk - 2) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_nextnext, ismem_read),
                   tCrA_prefetch_view(_, _, ik_nextnext));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_nextnext, ismem_read),
                   tCrB_prefetch_view(_, _, ik_nextnext));
      }
      ...
      if (ik == nk - 1) {
        ...
        if (nk > 1) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, 1, ismem_read),
                     tCrA_prefetch_view(_, _, 1));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, 1, ismem_read),
                     tCrB_prefetch_view(_, _, 1));
        }
      }
    }
  }
