...
__global__ void __launch_bounds__(128, 2)
    cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n,
                                      int k) {
  ...
  auto tCrA_db = make_tensor_like(tCrA);
  auto tCrB_db = make_tensor_like(tCrB);
  ...
  auto tCrA_db_view = s2r_thr_copy_a.retile_D(tCrA_db);

  auto s2r_tiled_copy_b = make_tiled_copy_B(S2RCopyAtomB{}, tiled_mma);
  auto s2r_thr_copy_b = s2r_tiled_copy_b.get_slice(idx);
  auto tBsB = s2r_thr_copy_b.partition_S(sB);
  auto tCrB_view = s2r_thr_copy_b.retile_D(tCrB);
  auto tCrB_db_view = s2r_thr_copy_b.retile_D(tCrB_db);
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      auto& tCrA_current = (ik % 2 == 0) ? tCrA : tCrA_db;
      auto& tCrB_current = (ik % 2 == 0) ? tCrB : tCrB_db;
      auto& tCrA_next_view = (ik % 2 == 0) ? tCrA_db_view : tCrA_view;
      auto& tCrB_next_view = (ik % 2 == 0) ? tCrB_db_view : tCrB_view;
      ...
      cute::gemm(tiled_mma, tCrD, tCrA_current(_, _, ik), tCrB_current(_, _, ik), tCrD);
    }
  }
