...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik_next == 0) {
        ...
        cute::copy(tCrA_next(_, _, ik_next), tCrA(_, _, ik_next));
        cute::copy(tCrB_next(_, _, ik_next), tCrB(_, _, ik_next));
        
        if (ik_next + 1 < nk) {
          cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_next + 1, ismem_read),
                     tCrA_next_view(_, _, ik_next + 1));
          cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_next + 1, ismem_read),
                     tCrB_next_view(_, _, ik_next + 1));
        }
      }
