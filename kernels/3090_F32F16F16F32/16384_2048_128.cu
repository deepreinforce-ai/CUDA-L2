...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      int ik_prefetch1 = ik + 1;
      int ik_prefetch2 = ik + 2;
      int ik_prefetch3 = ik + 3;
      int ik_prefetch4 = ik + 4;
      
      if (ik_prefetch1 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch1, ismem_read), 
                   tCrA_view(_, _, ik_prefetch1));
      }
      if (ik_prefetch2 < nk) {
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch2, ismem_read), 
                   tCrB_view(_, _, ik_prefetch2));
      }
      
      if (ik_prefetch3 < nk) {
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_prefetch3, ismem_read), 
                   tCrA_view(_, _, ik_prefetch3));
      }
      if (ik_prefetch4 < nk) {
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_prefetch4, ismem_read), 
                   tCrB_view(_, _, ik_prefetch4));
      }
