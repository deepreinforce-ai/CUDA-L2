...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  int total_m_tiles = (m + BM - 1) / BM;
  int total_n_tiles = (n + BN - 1) / BN;
  int total_tiles = total_m_tiles * total_n_tiles;
  
  for (int tile_idx = blockIdx.x; tile_idx < total_tiles; tile_idx += gridDim.x) {
    int iy = tile_idx / total_n_tiles;
    int ix = tile_idx % total_n_tiles;
