...
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
  ...
  T *Ashm = shm_data;
  T *Bshm = shm_data + cute::cosize(SmemLayoutA{}) + 16;
  ...
  for (int itile = 0; itile < ntile; ++itile) {
    ...
    for (int ik = 0; ik < nk; ++ik) {
      ...
      if (ik < nk - 2) {
        int ik_early = (ik + 2) % nk;
        cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik_early, ismem_read),
                   tCrA_view(_, _, ik_early));
        cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik_early, ismem_read),
                   tCrB_view(_, _, ik_early));
      }
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static constexpr int shm_size_AB =
      cute::cosize(SmemLayoutA{}) + cute::cosize(SmemLayoutB{}) + 16;
