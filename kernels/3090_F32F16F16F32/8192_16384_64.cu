...
cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, int m, int n, int k) {
  ...
  constexpr int padding_per_stage = 8;
  
  T *Ashm = shm_data;
  T *Bshm = shm_data + (cute::cosize(SmemLayoutA{}) + padding_per_stage * kStage);
...
void launch_hgemm_mma_stages_block_swizzle_tn_cute(T *a, T *b, T *c, int M,
  ...
  static constexpr int shm_size_AB =
      cute::cosize(SmemLayoutA{}) + cute::cosize(SmemLayoutB{}) + padding_per_stage * KStage;
