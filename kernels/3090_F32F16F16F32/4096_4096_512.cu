...
__global__ void __launch_bounds__(128, 3) cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr,
                                                  T *Dptr, int m,
                                                  int n, int k) {
...
#pragma unroll
    for (int ik = 0; ik < nk; ++ik) {
      if (ik == 0) {
        if (itile + 1 < ntile) {
          int write_stage = (itile + 1) % kStage;
          cute::copy(g2s_tiled_copy_a, tAgA_copy(_, _, _, itile + 1),
                     tAsA_copy(_, _, _, write_stage));
          cute::copy(g2s_tiled_copy_b, tBgB_copy(_, _, _, itile + 1),
                     tBsB_copy(_, _, _, write_stage));
          cp_async_fence();
        }
      }

      cute::copy(s2r_tiled_copy_a, tAsA(_, _, ik, read_stage), tCrA_view(_, _, ik));
      cute::copy(s2r_tiled_copy_b, tBsB(_, _, ik, read_stage), tCrB_view(_, _, ik));
