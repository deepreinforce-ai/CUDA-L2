...
template <typename T, int BM, int BN, int BK, int kStage, typename TiledMMA,
          ...
          typename S2GCopyC>
__global__ void cuda_l2_3090_fp32_kernel(T *Aptr, T *Bptr, T *Dptr, 
                                                   int m, int n, int k) {
...
void launch_hgemm_optimized(T *a, T *b, T *c, int M, int N, int K) {
  using namespace cute;
  auto BM = Int<32>{}; auto BN = Int<32>{}; auto BK = Int<64>{}; auto KStage = Int<5>{};
  
  using SmemLayoutAtom = decltype(composition(Swizzle<2, 3, 3>{}, make_layout(make_shape(Int<8>{}, Int<BK>{}), make_stride(Int<BK>{}, Int<1>{}))));
  ...
  using MMA = decltype(make_tiled_mma(mma_atom{}, make_layout(make_shape(Int<2>{}, Int<2>{}, Int<1>{})), Tile<Int<32>, Int<32>, Int<16>>{}));
  
  using g2s_copy_atom = Copy_Atom<Copy_Traits<SM80_CP_ASYNC_CACHEGLOBAL<cute::uint128_t>>, T>;
  using G2SCopyA = decltype(make_tiled_copy(g2s_copy_atom{}, make_layout(make_shape(Int<32>{}, Int<4>{}), make_stride(Int<4>{}, Int<1>{})), make_layout(make_shape(Int<1>{}, Int<8>{}))));
  
  using s2r_copy_atom = Copy_Atom<Copy_Traits<SM75_U32x4_LDSM_N>, T>;
  
  using SmemLayoutAtomC = decltype(composition(Swizzle<2, 3, 3>{}, make_layout(make_shape(Int<32>{}, Int<32>{}), make_stride(Int<32>{}, Int<1>{}))));
  using SmemLayoutC = decltype(tile_to_shape(SmemLayoutAtomC{}, make_shape(Int<32>{}, Int<32>{}, Int<1>{})));
  
  using R2SCopyAtomC = Copy_Atom<UniversalCopy<int>, T>;
  using S2GCopyAtomC = Copy_Atom<UniversalCopy<cute::uint128_t>, T>;
  using S2GCopyC = decltype(make_tiled_copy(S2GCopyAtomC{}, make_layout(make_shape(Int<32>{}, Int<4>{}), make_stride(Int<4>{}, Int<1>{})), make_layout(make_shape(Int<1>{}, Int<8>{}))));

  dim3 block(size(MMA{})); 
  dim3 grid((N + BN - 1) / BN, (M + BM - 1) / BM);
  
  int kShmSize = cute::max(cute::cosize(SmemLayoutA{}) + cute::cosize(SmemLayoutB{}), cute::cosize(SmemLayoutC{})) * sizeof(T);
  
  cudaFuncSetAttribute(cuda_l2_3090_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyA, SmemLayoutA, SmemLayoutB, SmemLayoutC, s2r_copy_atom, s2r_copy_atom, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC>, cudaFuncAttributeMaxDynamicSharedMemorySize, kShmSize);
  
  cuda_l2_3090_fp32_kernel<T, BM, BN, BK, KStage, MMA, G2SCopyA, G2SCopyA, SmemLayoutA, SmemLayoutB, SmemLayoutC, s2r_copy_atom, s2r_copy_atom, R2SCopyAtomC, S2GCopyAtomC, S2GCopyC><<<grid, block, kShmSize>>>(a, b, c, M, N, K);
}

void cuda_l2_3090_fp32(torch::Tensor a, torch::Tensor b, torch::Tensor b_col_major, torch::Tensor c) {
  const int M = a.size(0), K = a.size(1), N = b.size(1);
  launch_hgemm_optimized<half>(reinterpret_cast<half *>(a.data_ptr()), reinterpret_cast<half *>(b_col_major.data_ptr()), reinterpret_cast<half *>(c.data_ptr()), M, N, K);
}